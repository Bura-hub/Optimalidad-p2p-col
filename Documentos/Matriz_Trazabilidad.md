...
| **4** — Sensibilidad y análisis de optimalidad | **4.1** — Análisis de sensibilidad mediante simulaciones | `analysis/global_sensitivity.py`; `analysis/sensitivity.py` | `graficas/fig7_sensibilidad_pgb.png`; `graficas/fig8_sensibilidad_pv.png`; `graficas/fig10_sensibilidad_ppa.png`; `graficas/fig11_sensibilidad_pgs.png` | parcial — índices Sobol fiables requieren n_base ≥ 64 | SALib 1.5.2 instalado. Análisis Saltelli (7 parámetros, 3 outputs) implementado en `analysis/global_sensitivity.py`. Smoke test n_base = 4 exitoso. Run n_base ≥ 64 (~75 min, 8 workers) pendiente de autorización. Barridos paramétricos SA-1 (PGB) y SA-2 (PV) completados en `analysis/sensitivity.py` |
...
| **4** — Sensibilidad y optimalidad | 1 (4.2) | 1 (4.1) | 2 |
| **TOTAL** | **8 / 10 (80 %)** | **2 / 10 (20 %)** | **10** |

**Actividades parciales y sus condiciones de cierre:**

- **3.2:** Requiere `python main_simulation.py --data real --full --analysis` (~20–30 min) para
  generar resultados sobre el horizonte completo (5 160 h). Resultado parcial disponible con
  perfil diario promedio en `REPORTE_AVANCES.md`.

- **4.1:** Requiere confirmación del usuario para lanzar GSA con n_base ≥ 64 (~75 min con 8
  workers). El módulo `analysis/global_sensitivity.py` está implementado y probado (smoke test
  n_base = 4 exitoso). Ejecutar: `python main_simulation.py --gsa --n-base 64`.
