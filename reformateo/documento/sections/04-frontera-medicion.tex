...
% Parte I. El dato
%
% ENCARGO: dos fronteras, dos preguntas. La bifurcacion es en si misma
...
bifurcación entre las fronteras M1 y M3 ocurre en la primera etapa del
pipeline y en ninguna otra. Este capítulo desarrolla esa bifurcación, y lo
hace en capítulo aparte por una razón sencilla: no se trata de dos
versiones del mismo cálculo, ni de un análisis de sensibilidad
...
La frontera \textbf{M1} pregunta: \emph{¿qué le aporta este mercado a
una carga grande, frente a la cual la generación es escasa?} La frontera
...
\subsection{Cobertura de cada frontera}
\label{sub:frontera-cobertura}

La diferencia entre las dos fronteras se resume en un hecho que desactiva
de entrada una posible objeción: \textbf{la generación es idéntica en las
dos}. El pipeline designa el mismo inversor
...
\begin{figure}[H]
 ...
 \caption[Generación y demanda en cada frontera]{Energía medida en
...
\notafig{La demanda comunitaria medida es de \uni{318045}{kWh} bajo M1
y de \uni{66734}{kWh} bajo M3, mientras que la generación es de
\uni{63574}{kWh} en ambos casos, es decir, exactamente la misma cifra. De
ahí las dos razones, \pct{20.0} y \pct{95.3}, que no reflejan dos
instalaciones distintas sino dos maneras de delimitar la carga.
Elaboración propia desde el caché de estados intermedios.}

\begin{lecturabox}
M3 no «tiene más generación», aunque lo parezca: tiene la misma. Lo que M3 hace es comparar esa generación contra
una carga cinco veces menor, porque solo mira un circuito secundario de
cada institución. Una generación que equivale al \pct{20} del consumo del
circuito principal puede equivaler al \pct{95} del consumo de un circuito
...
las dos fronteras en un mismo par de paneles.
...
\begin{figure}[H]
 ...
 del día de la comunidad en cada frontera. La curva verde es la misma
...
Figura~\ref{fig:frontera-inversion} cruza las dos fronteras conservando
...
instituciones», «ninguna») puede escribirse sin decir de qué frontera
...
Queda una magnitud que ninguna de las figuras anteriores muestra: de las
\num{6144} horas del horizonte, en muy pocas llega a haber mercado. La
...
\notafig{El recorrido es el mismo en las dos fronteras hasta el
segundo escalón, porque hay generación en \num{3354} horas y esa cifra no
depende de la frontera. La separación empieza en el tercero: bajo M1
quedan \num{1131} horas con excedente y se transa en \num{1029}; bajo M3,
\num{2567} y \num{1752}. El último escalón procede de la corrida
canónica, anterior a las correcciones de este capítulo, de modo que
crecerá con ella. Elaboración propia desde el caché del preprocesamiento y
los archivos de flujos.}
...
\subsection{Una salvedad del escenario secundario}
\label{sub:frontera-mariana}

La frontera M3 no es perfectamente homogénea, y la excepción se dice aquí
y no al pie. Cuatro de las cinco instituciones tienen un
...
fronteras. Su efecto sobre el resultado agregado se examina en el
...
\subsection{La consecuencia: el reporte doble}
...
fronteras, siempre, y sin elegir una como principal.}

La tentación de elegir una sola es real. M1 es la frontera en
...
\pct{95.3}; y es también la frontera en la que, como se verá, el
...
cuando las dos fronteras dan lecturas opuestas, se dicen las dos y se
discute por qué difieren.

Con la energía así delimitada queda pendiente la otra mitad del dato, los
precios, que el capítulo siguiente construye desde sus fuentes primarias.
