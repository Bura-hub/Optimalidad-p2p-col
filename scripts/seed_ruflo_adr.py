...
ADR_SUMMARIES = {
    ...
    "0021-cal21-c2-f-split-sensibilidad": (
        ...
        "scripts/study_f_split.py 168h ago-2025 split ilustrativo 3 "
        "prosumers Udenar Mariana Cesmag + 2 consumers UCC HUDN. "
        "Tres hipotesis ratificadas H1 invarianza total_net_benefit "
        "= 791289 COP para 7 valores f delta < 1e-13 % ratifica teorema "
        "notas §3.8. H2 Gini no invariante 0.436 -> 0.495 monotono "
        "creciente justifica reporte SA-3. H3 egalitaridad simetrica "
        "f=0.5 mediana exacta consumer mean entre f=0 y f=1 cada "
        "delta f 0.1 transfiere ~1700 COP. Sustento triple invarianza "
        "egalitaridad estabilidad Gini. Alternativas descartadas f de "
        "PPAs reales rango disperso [-3.08, +0.029] no representativo "
        "MTE. f=0 rompe incentivo prosumer f=1 rompe incentivo consumer. "
        "f por agente complejidad sin beneficio. main_simulation.py "
        "extrae F_PPA_DEFAULT=0.5 con cita ADR-0021. Test "
        "tests/test_cal21_f_sensibilidad.py 4 verdes. Graficas "
        "sensibilidad_f csv mat png actividad 3.1 3.2 3.3"
    ),
    "0022-cal22-mem-costs-validacion": (
        "CAL-22 validacion trazable mem_costs_no_regulado.csv ADR-0022 "
        "Sprint 3.2 cierre item validacion MEM costs plan radiant-sleeping. "
        "Trazabilidad celda-fuente formal CSV introducido en CAL-16. "
        "FAZNI 1.90 COP/kWh Ley 1715/2014 art 19 + MinMinas Resolucion "
        "4 0067/2024. Contribucion 4 porciento sobre Gm Ley 1117/2006 "
        "art 2 prorrogada Ley 2099/2021 art 45 aplicable usuario "
        "no-regulado Ley 142/1994 art 89. Comision representante "
        "2.00 COP/kWh CREG 156/2012 + ASOCODIS 2024 mediana mercado "
        "rango [1.5, 3.0] COP/kWh para usuarios 50-200 MWh/mes. "
        "Total estimado 15.9 COP/kWh con G=300 ~3 porciento del G "
        "~5 porciento del CU material no dominante bienestar agregado C2. "
        "Cobertura 13/13 meses CSV total 9/9 horizonte --full. "
        "Decision Accepted: aceptar valores referenciales defendibles "
        "para 2025-2026. Riesgos abiertos FAZNI puede revisarse "
        "anualmente comision estimacion mercado Ley 2099 puede expirar. "
        "data/mem_costs_audit.md trazabilidad detallada celda-fuente. "
        "Test tests/test_cal22_mem_costs.py 11 verdes schema cobertura "
        "rangos FAZNI comision contrib literal Gm fuente legal "
        "coherencia helper actividad 3.1 3.2 3.3"
    ),
    "0023-cal23-c2-cxc-cargo-confiabilidad": (
        "CAL-23 CXC cargo confiabilidad opt-in en C2 ADR-0023 Sprint 3.3 "
        "ambiguedad regulatoria resuelta consulta usuario opcion "
        "parametrizable cxc_alpha. Tres normas concurrentes Decreto "
        "2236/2023 autogeneracion colectiva no menciona CXC; CREG "
        "071/2006 mod 099/2007 define CXC universal; CREG 086/1996 "
        "no exime explicitamente. Practica industrial PPAs corporativos "
        "auditados CXC sigue cobrando bajo PPA. Decision Accepted: "
        "implementar CXC parametrizable analogo a cot_alpha CAL-20 "
        "default 0.0 cota conservadora opuesto a cot_alpha=1.0. "
        "scenarios/scenario_c2_bilateral.py nuevos parametros "
        "cxc_component cxc_alpha=0.0 default. Linea savings_CXC[i] "
        "+= e * cxc_v[i,k] * cxc_alpha. data/cxc_costs.csv 13 filas "
        "10.00 COP/kWh constante mediana ASOCODIS 2024 rango [5,15]. "
        "scenarios/_c2_cxc.py helper opt-in load_cxc_monthly "
        "cxc_per_agent_hourly NO integrado main_simulation. "
        "Test tests/test_cal23_cxc.py 9 verdes schema rangos "
        "compatibilidad default sin cxc no cambia resultado linealidad "
        "savings_CXC en cxc_alpha inercia consumer_ids vacio default=0.0. "
        "Sensibilidad triple disponible f cot_alpha cxc_alpha cubre "
        "incertidumbre regulatoria C2 actividad 3.1 3.2 3.3"
    ),
    ...
    "0025-cal25-modo-paper": (
        "CAL-25 modo paper IEEE WEEF homogeneizacion comercial filtrado "
        "escenarios Sprint 6.1 reunion 2026-05-01 Pantoja Obando "
        "scripts/run_paper_iter.py orquestador standalone NO toca "
        "main_simulation.py homogeneiza INSTITUTION_PROFILE en memoria "
        "comercial uniforme A1 evita multi-equilibrios solver Stackelberg "
        "con tarifas heterogeneas pi_hat replicator_buyers ventana comun "
        "admisibilidad B filtra solo C1 CREG 174 + P2P + C2 (renaming C4 "
        "= CREG 101 072) omite C2 PPA bilateral C3 spot G horizonte "
        "mensual default agosto-2025 744h tag y out-dir parametrizables "
        "outputs/paper/ resultados_paper_*.xlsx perfiles_*.png separado "
        "tesis full mantiene heterogeneidad oficial vs comercial. "
        "Test tests/test_run_paper_iter.py 7 verdes homogeneizacion "
        "horizonte_mensual paper_renaming smoke xlsx pde capacity "
        "y excedentes. Tesis 117/117 tests no afectados base post-CAL-23 "
        "intacto. ADR-0025 Accepted Sprint 6.1 2026-05-02 actividad 4.1 4.2"
    ),
    "0026-cal26-pde-excedentes-proportional": (
        "CAL-26 PDE excedentes_proportional opt-in C4 CREG 101 072 ADR-0026 "
        "Sprint 6.2 reunion 2026-05-01 punto C-sensibility Pantoja Obando "
        "compute_pde_weights extiende method capacity_proportional default "
        "art 5 y excedentes_proportional alternativa admisible CREG 101 072 "
        "ponderadores acordados entre miembros helper compute_excedentes_acumulados "
        "G D suma surplus por agente sobre horizonte invariantes Sigma=1 "
        "no-negatividad fallback 1/N si total=0. Comparativa C2-cap vs "
        "C2-exc lado a lado en run_paper_iter.py reporta robustez ranking "
        "vs metodo PDE. Test tests/test_cal26_pde_excedentes.py 11 verdes "
        "metodos invariantes degeneracion fallback. Default sigue capacity "
        "proportional CAL-15 baseline post-CAL-23 invariante. Sin impacto "
        "tesis. ADR-0026 Accepted Sprint 6.2 2026-05-03 actividad 4.1 4.2 3.1 3.2"
    ),
    "0027-cal27-c4-mensual-hx": (
        "CAL-27 C4-mensual con cruce Hx ADR-0027 Sprint 6.3 cierra TODO "
        "CAL-15 reunion 2026-05-01 punto F Pantoja Obando "
        "scenarios/scenario_c4_creg101072.py modo monthly_hx analogo "
        "_run_c4_creg174_inheritance pero con agregacion mensual + cruce "
        "Hx heredado de C1 art 22-23 month_labels parametriza periodos "
        "facturacion default (None) periodo unico igual a perfil diario. "
        "Hipotesis CAL-27 verificada empiricamente total_net_benefit "
        "monthly_hx >= total_net_benefit creg174_inheritance en agregado "
        "0-5 porciento mejora. mode dispatcher acepta creg174_inheritance "
        "(default CAL-15) y monthly_hx (opt-in nuevo). Figura comparativa "
        "horario vs mensual graficas/fig_c4_horario_vs_mensual csv mat png "
        "Test tests/test_cal27_c4_monthly.py 6 verdes acepta modo sin "
        "labels n_periods=1 con labels n_periods=k hipotesis no-peor "
        "default sigue creg174 keys per_agent compatibles. Tesis intacta "
        "default sin cambios. ADR-0027 Accepted Sprint 6.3 2026-05-03 "
        "actividad 4.1 4.2 3.1 3.2 3.3"
    ),
    "0028-cal28-paper-medidor-puntual": (
        "CAL-28 seleccion medidor puntual por institucion paper IEEE WEEF "
        "ADR-0028 Sprint 6.X-bis observacion usuario 2026-05-03 P2P sin "
        "excedentes con M1 totalizador genera mercado P2P inactivo. "
        "data/paper_meter_config.csv 5 instituciones M3 sub-medidor por "
        "default Mariana M1 escalado factor 0.3 (M3 vacio). "
        "scripts/run_paper_iter.py cargar_mte_paper helper nuevo lee "
        "csv config localiza tz America/Bogota resamplea horario reindex "
        "horizonte clip no-negativos D paper distinto de tesis (M1 "
        "totalizador). Cobertura PV agregada 96 porciento vs 19 porciento "
        "M1. resultado mercado P2P activo 221/744 horas (29.7%) 525.88 "
        "kWh transados ago-2025. Flag --no-paper-meters revierte M1 "
        "totalizador comparacion baseline tesis. Test "
        "tests/test_cal28_meter_selection.py 9 verdes config csv 5 "
        "instituciones columnas requeridas Mariana factor 0.3 "
        "_read_meter_csvs tz horaria cargar_mte_paper shapes cobertura "
        "no-negatividad smoke CLI no-paper-meters. Tesis 117/117 intacta "
        "ADR-0028 Accepted Sprint 6.X-bis 2026-05-03 actividad 4.1 4.2"
    ),
    "0029-cal29-p2p-revenue-canonica": (
        "CAL-29 formula canonica net_benefit P2P paper-only ADR-0029 "
        "Sprint 6.6-A audit usuario 2026-05-03 'tenemos tanta diferencia "
        "y desbeneficio al mercado P2P estoy simulando mal hay algo que "
        "deba calibrar Modelo P2P original'. Auditoria 3 fases A lectura "
        "formulas B diagnostico empirico C decision fix. "
        "Documentos/audit_p2p_decomposition.md scripts/audit_p2p_paper.py "
        "Hallazgos H1 asimetria comparison_engine._p2p_monetary_benefit "
        "(pi_star - pi_gb) * P_sold incremental sobre baseline omite "
        "revenue completo trade + residual surplus. C1/C4 reportan "
        "revenue total E_t1 (pi_gs - pi_C) + E_t2 pi_bolsa horario. "
        "Verificacion empirica delta = 958255 COP coincide exacto "
        "pi_bolsa_mean E_surplus_total = 234.5 x 4085.6. Bug 2 "
        "_p2p_decomposed paper script autoconsumo solo en horas activas "
        "if r.P_star is None continue omite 523/744 horas ~961K COP. "
        "Total subreporte P2P paper 1.92M COP. "
        "Fix CAL-29 paper-only NO toca comparison_engine reescribe "
        "scripts/run_paper_iter.py::_p2p_decomposed bug 1+2 fix "
        "autoconsumo todas las horas mercado_seller revenue completo "
        "sum_t pi_star * P_sold + sum_t pi_bolsa * residual_nk "
        "mercado_buyer (pi_gs - pi_star) P_bought. Acepta pi_bolsa "
        "horario fallback pi_gb escalar. "
        "Resultados ago-2025 pre-CAL-29 P2P 3.03M C1 4.95M C2 4.58M "
        "post-CAL-29 P2P 4.81M (entre C1 y C2 -2.9% vs C1 +5.0% vs C2). "
        "Per-agente 3 de 5 prefieren P2P sobre C1 (Udenar HUDN Cesmag). "
        "Sweep PV pre rank 3 todos los factores post P2P rank 1 "
        "factor >= 1.5x cruce optimalidad 1.0-1.5x. "
        "Ambito limitado tesis intacta 117/117 tests. CAL-30 Sprint 7 "
        "post-paper migrara engine canonical mode default. "
        "Test tests/test_cal29_p2p_canonical.py 4 verdes invariantes "
        "autoconsumo todas horas residual pi_bolsa simetria C1 fallback. "
        "ADR-0029 Accepted 2026-05-03 actividad 4.1 4.2"
    ),
    "0030-cal30-engine-canonical": (
        "CAL-30 migracion engine tesis a formula canonica P2P ADR-0030 "
        "Sprint 7 cierre auditoria. Promueve fix CAL-29 paper-only al "
        "engine de tesis. scenarios/comparison_engine.py::_p2p_monetary_benefit "
        "firma extendida mode={canonical,premium} default canonical "
        "pi_bolsa Optional ndarray. Modo canonical autoconsumo + revenue "
        "completo trade pi_star * P_sold + residual surplus pi_bolsa[k] "
        "simetrico C1/C2/C3/C4. Modo premium legacy (pi_star - pi_gb) "
        "P_sold opt-in reproducibilidad. run_comparison pasa "
        "mode=canonical y pi_bolsa. main_simulation._compute_daily_series "
        "pasa pi_bolsa[sl]. analysis/monthly_report._p2p_benefit_month "
        "parametro pi_bolsa_m analogo. "
        "Validacion gate plan §6.6.5: pytest tests/ -q 310/310 verdes "
        "cero tests asertaban valores P2P especificos. RPE perfil diario "
        "delta +0.02% dentro gate +-0.5%. P2P canonical 210496 vs premium "
        "210450 COP. Per-agente Udenar+172 Mariana-51 UCC-64 HUDN+1 "
        "Cesmag+35 vs C4 RPE total +0.0004 P2P>C4. "
        "Por que delta pequeno: tesis usa M1 totalizador cobertura 19% "
        "surplus residual marginal premium cancela pi_gb*P_sold contra "
        "baseline implicito canonical anade pi_bolsa*residual ambos "
        "compensan en 0.02%. Paper CAL-28 cobertura 96% delta era 38% "
        "pero ya aplicado via CAL-29. CAL-30 unifica ambos. "
        "Pendiente autor: ejecutar --full --analysis ~30min capturar RPE "
        "canonica horizonte completo 5160h actualizar §4.9 cap4 "
        "REPORTE_AVANCES.md valores especificos. Conclusion cualitativa "
        "preservada solo magnitudes en COP. "
        "ADR-0030 Accepted 2026-05-03 actividad 4.1 4.2"
    ),
    "0031-cal31-renumeracion-art-creg-101072": (
        "CAL-31 renumeracion articulos CREG + aclaratoria terminologica "
        "ADR-0031 Sprint 8 auditoria regulatoria integral C1-C4. Solicitud "
        "usuario 2026-05-03 audita C1 C2 C3 C4 con superpowers ruflo "
        "investiga internet valida regulacion. WebFetch a "
        "gestornormativo.creg.gov.co verifico 9 fuentes oficiales. "
        "Hallazgos: G-01 C4 cita CREG 101 072 art 5 PDE correcto art 19 "
        "(4 lugares). G-02 C4 cita art 3 capacidad 100kW correcto art 20 "
        "caso 1. G-03 C1 cita CREG 174 art 22-23 mecanica Tipo 1/2 "
        "correcto art 25 creditos energia + valoracion horaria. G-04 C2 "
        "cita CREG 086 art 1 mod 039/2001 correcto art 3 mod 039 art 1. "
        "G-05 modificacion 2025 NO trackeada CREG 101-087/2025 art 13 "
        "modifica CREG 101 072 art 20 caso 4 amplia no-FNCER + CREG 174 "
        "art 24 definiciones precio. NULO impacto MTE solar todas FNCER. "
        "G-06 terminologia Tipo 1 Tipo 2 hora Hx NO literal CREG 174 son "
        "didacticas sector EDEQ Solsta. CREG 174 art 25 habla credito "
        "energia y valoracion horaria. Cero gaps criticos cero "
        "parametros incorrectos. "
        "Fixes: 9 ediciones docstrings (3 c1 + 1 c2 + 5 c4) replace_all "
        "art 5 -> art 19 en c4. Lógica matematica intacta. "
        "Validacion: pytest 310/310 verdes (cambios solo doc). CAL-24 "
        "swarm validator PASS 15/15. main_simulation --data real RPE "
        "delta=0. Doc auditoria Documentos/audit_regulatorio_C1_C4.md "
        "~12 paginas. "
        "Pendientes autor humano: 1) umbral no-regulado 2025-2026 cita "
        "(Concepto CREG 14534/2025); 2) prorroga 4% Ley 2099/2021 art 45; "
        "3) lectura CREG 101 028/2023 mod CREG 119/2007. "
        "ADR-0031 Accepted 2026-05-03 actividad 3.1 3.2 3.3"
    ),
