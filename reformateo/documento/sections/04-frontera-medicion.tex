...
Cada institución de la comunidad tiene más de un medidor. El totalizador
de campus registra todo lo que la institución consume; los submedidores
registran ramales concretos, y entre ellos está el circuito que alimenta
el sistema fotovoltaico. Leer uno u otro no es una preferencia técnica:
...
\begin{figure}[H]
 ...
 \node[inv] (inv) at (7.3,-2.55) {Inversor};
 ...
 \node[et, text=sectioncolor, right=2.2cm of m1, anchor=west,
 text width=4.4cm]
 {\textbf{M1} mide todo el campus:\\
 la generación cubre el 19,1 \%};
 \node[et, text=warningcolor, right=0.55cm of inv, anchor=west,
 text width=3.6cm]
 {\textbf{M3} mide solo el ramal\\
 que el inversor alimenta:\\
 la generación cubre el 91,2 \%};
 \end{tikzpicture}
...
La cobertura \textbf{M1} pregunta: \emph{¿qué le aporta este mercado a la
institución entera?} La cobertura \textbf{M3} pregunta: \emph{¿qué le
aporta al circuito que efectivamente está electrificado con sol?} La
primera es la pregunta del rector que firma la factura; la segunda, la del
ingeniero que dimensionó la instalación. Ninguna es más legítima que la
otra.
...
\begin{figure}[H]
 ...
 demanda por institución. Las cifras se miden del caché del
...
una carga cinco veces menor, porque solo mira el ramal que el inversor
alimenta. Un sistema fotovoltaico que cubre el \pct{19} de un campus puede
cubrir el \pct{91} de un edificio de ese campus sin que se haya instalado
un solo panel adicional.
\end{lecturabox}
...
submedidor que aísla el circuito fotovoltaico y que el pipeline lee
directamente. Mariana no lo tiene. Para representarla en M3, el pipeline
reutiliza su totalizador de campus escalado por un factor de $0{,}3$, bajo
...
concluyente. M3 es la frontera con cobertura de medición más alta y, por
tanto, la que un revisor consideraría más representativa del sistema
fotovoltaico instalado; y es también la frontera en la que, como se verá,
el mecanismo colectivo supera al mercado entre pares. Presentar solo una
