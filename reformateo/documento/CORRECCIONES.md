...
## C-189 · Las dos palancas del acoplado, apagadas por defecto (D35, D26)

**2026-09-14 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

La sonda de H-79 corre en el servidor antes de la matriz de trece corridas y decide, hora por hora, si el reparto o el excedente dependen del integrador. El autor ya decidió qué hacer en cada caso: si la palanca es la tolerancia relativa, producción la aprieta a 1e-7 (D35); si es el horizonte, el acoplado para por estacionario en vez de horizonte fijo (D26). Hasta esta tarea ninguna de las dos existía como opción del motor: `solve_coupled_for_hour` ya aceptaba `rtol`, pero `SolverParams` no lo exponía, y el horizonte del acoplado (`t_span_acoplado`) era siempre fijo. Para que la cadena del servidor aplique el veredicto de la sonda sola, sin volver a tocar el código entre la sonda y el lanzamiento de la matriz, las dos palancas tenían que existir ya, apagadas por defecto e idénticas al bit a como estaba.

### Qué cambia

- `core/ems_p2p.py`: `SolverParams.rtol_acoplado` (1e-6 por omisión, con un comentario D35) y `SolverParams.horizonte_max_acoplado` (`None` por omisión, con un comentario D26). La tolerancia absoluta no se expone (regla principal de `CLAUDE.md`, H-51): bajarla de 1e-6 cuelga el integrador.
- La parada por estacionario queda en un ayudante nuevo, `_resuelve_acoplado`: con `horizonte_max_acoplado=None` resuelve una sola vez con `t_span_acoplado`, idéntico a antes; con la palanca activa, dobla el horizonte desde `t_span_acoplado` y **vuelve a resolver desde el arranque** en cada vuelta (no hace falta arranque en caliente, y el resultado es determinista) hasta que el precio se mueva menos del 1 % de su recorrido en el último décimo de la trayectoria (`TOL_ESTACIONARIO = 0.01`, constante de módulo, la misma medida que el motor ya guarda como `norm_rel_final`), o hasta pasar el tope. Solo pagan más las horas que no llegaban al estacionario. `HourlyResult.horizonte_usado` guarda el horizonte con que de verdad se resolvió cada hora (0,0 en la vía alternada o sin mercado esa hora).
- `_run_hour_worker` recibe las dos palancas al final de su tupla de argumentos; se agregaron en los tres sitios de producción donde esa tupla se construye (el lote de horas de `EMSP2P.run`, la llamada recursiva de la restricción de participación de H-43, y `EMSP2P.run_single_hour`), y una tupla de la longitud anterior (26 campos) sigue aceptándose, con las dos palancas apagadas por el mismo mecanismo de compatibilidad que ya usaban CAL-48 y H-49.
- `main_simulation.py`: opciones `--rtol-acoplado` (1e-6 por omisión; rechaza valores no finitos o no positivos, y avisa si se pide por encima de 1e-6, que aflojaría la tolerancia de producción) y `--horizonte-max-acoplado` (0 por omisión, que la apaga; si se da, tiene que ser mayor o igual que `--t-span-acoplado`), hasta el `SolverParams` de `main`, en las dos llamadas a `main`. Al final del mercado, si la parada por estacionario está activa, la consola imprime cuántas horas usaron cada horizonte; con los valores por defecto esa línea no aparece y la salida de la corrida no cambia en nada.

Prueba nueva: `tests/test_palancas_acoplado.py`, con una hora del caso sintético de la compuerta C-165 (`gate_c165_desglose_horario._caso`) y la vía acoplada. Tres casos: con los valores por defecto, el resultado es idéntico (`np.array_equal`) al de la tupla de 26 campos de antes de esta tarea, y `horizonte_usado` es el horizonte de producción; con `rtol_acoplado=1e-7`, el resultado de la hora es idéntico al de llamar directo a `solve_coupled_for_hour(..., rtol=1e-7)`; y con `horizonte_max_acoplado` activo y la tolerancia de estacionario forzada a cero mediante el argumento `tol_estacionario` de `_resuelve_acoplado` (nunca mutando la constante de módulo), la hora se resuelve con el horizonte doble y su resultado es idéntico al de resolverla directamente con ese horizonte.

### Estado

**Corregido en el código, sin corrida oficial.** Las tres pruebas nuevas en verde; las siete de `tests/test_plazo_por_hora.py` en verde; la dorada en 7 de 7; el preflight filtrado (`-k "suma or banner or propaga or construye or pasa"`) en 6 de 6; y la compuerta C-165 con 48 horas en verde, sin cambios frente a antes de esta tarea (las dos palancas quedan apagadas por defecto en esa corrida).

---

## C-190 · La hora del acoplado que revienta deja motivo, no se traga muda (H-51 y regla principal)

**2026-09-14 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

En `core/ems_p2p.py`, la rama acoplada de `_run_hour_worker` envolvía la resolución de la hora en `try: ... except Exception: return res`, sin capturar la excepción en ninguna variable. Si `_resuelve_acoplado` (o `solve_coupled_for_hour` por debajo) reventaba por cualquier causa, la hora volvía tal cual se había construido al entrar a la función: `P_star=None`, `motivo=""`. Esa hora quedaba indistinguible de una sin vendedores o sin compradores, o de una que el guardia de NaN descartó: un fallo mudo, exactamente el patrón que la regla principal de `CLAUDE.md` («cifras que salen sin error pero son falsas») fija por resolver en voz alta. La vía alternada, en el mismo `else:`, no tiene ningún `try/except`: una excepción ahí se propaga tal cual, no es un fallo mudo.

### Qué cambia

- El `except` de la rama acoplada ahora captura la excepción (`except Exception as e:`) y escribe `res.motivo = f"excepcion del acoplado: {type(e).__name__}: {e}"`, recortado a 200 caracteres, y deja `res.horizonte_usado = 0.0`. Una hora que revienta no tumba la corrida (una hora no puede matar las otras 5159), pero ya no queda muda.
- El almacén (`core/almacen.py::anota_agentes`) ya trataba cualquier `motivo` no vacío como «sin resolver» (o «inactivo» sin sobrante ni faltante), sin mirar su texto: no hizo falta tocarlo. La hora con excepción llega ahí exactamente igual que la que vence el plazo por hora (D24).
- `main_simulation.py`: el resumen de horas vencidas por plazo (`[D24]`) ahora filtra el motivo por el prefijo `"vencio el plazo"`, en vez de contar cualquier motivo no vacío; antes de esta tarea eso no importaba porque el único motivo posible era el del plazo, pero con el motivo de excepción habría mezclado las dos causas en un solo conteo. Se añade un resumen nuevo, `[C-190]`, con cuántas horas terminaron con excepción y las primeras cinco (hora y motivo); solo aparece si hay al menos una, de modo que la salida de una corrida sin excepciones no cambia en nada.
- De paso, la recursión de la restricción de participación de H-43 (`if sub.P_star is None: return res`) también deja `res.horizonte_usado = 0.0`, igual que el `elif fuera:` de C-189: sin mercado, el horizonte del intento con el conjunto completo que esa restricción descartó no significa nada.
- Esa misma rama de H-43 tenía el mismo fallo mudo un nivel más arriba. Si el reintento con el conjunto reducido (`sub = _run_hour_worker(...)`) reventaba, su propio `motivo` («excepcion del acoplado: ...») se perdía, porque el marco exterior devolvía su propio `res`, con `motivo=""` de fábrica: la hora llegaba al resumen `[C-190]` y al almacén como hora sin mercado, muda. Ahora, antes de ese `return res`, si `sub.motivo` no está vacío se copia a `res.motivo`.

Pruebas nuevas: `tests/test_palancas_acoplado.py::test_excepcion_en_acoplado_no_tumba_la_hora_y_deja_motivo`, que monkeypatchea `_resuelve_acoplado` para que reviente (sin integrar de verdad) y comprueba que la hora vuelve con `P_star is None`, `motivo` empezando por «excepcion del acoplado» y nombrando el tipo de la excepción, y `horizonte_usado == 0.0`; y `tests/test_palancas_acoplado.py::test_excepcion_en_reintento_de_h43_propaga_el_motivo`, que fuerza la retirada de un vendedor con pisos no uniformes y hace que el reintento del conjunto reducido reviente, comprobando que el motivo sube al marco exterior. Se comprobó que esta segunda prueba falla sin el arreglo (`res.motivo == ""`) y pasa con él.

### Estado

**Corregido en el código, sin corrida oficial.** Las seis pruebas de `tests/test_palancas_acoplado.py` en verde; las siete de `tests/test_plazo_por_hora.py` en verde; la dorada en 7 de 7; y la compuerta C-165 con 48 horas en verde, sin la línea `[C-190]` (cero horas con excepción en ese caso sintético).

---

## C-191 · La parada por estacionario conserva su última vuelta buena, y la corrida sale con código si pierde horas (D36, D37, D38)

**2026-09-14 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

La revisión final del conjunto encontró que, con la matriz de trece corridas encadenada sola en el servidor, la parada por estacionario de D26 (dobla el horizonte del acoplado hasta 0,4) choca con el plazo de 15 (min) por hora de D24. Una hora que necesita 0,2 o 0,4 vence, y el trabajador que el plazo mata no tiene canal para devolver la solución que ya tenía de las vueltas anteriores: la hora se pierde entera. Medido en la hora 4184 con la generación por siete: 142 (s) al horizonte 0,05 y 367 (s) al 0,1; llegar a 0,4 costaría de 45 a 65 (min). Con el crecimiento medido en esta tarea (2,56 por doblez), la cuenta da unos 64 (min) acumulados hasta 0,4, dentro de esa estimación.

Había tres huecos más. Una vuelta que no terminaba con éxito dejaba la hora sin mercado y sin motivo: la rama acoplada de `_run_hour_worker` hacía `if not bool(getattr(tr, "success", True)): return res` (`core/ems_p2p.py`, líneas 802 y 803 antes de esta tarea), indistinguible de una hora sin vendedores. Las horas con excepción (C-190) o vencidas (D24) no llegaban al código de salida, de modo que la cadena del lanzador seguía con un caso vacío. Y el `import` de `solve_coupled_for_hour` vivía dentro de `_resuelve_acoplado`, es decir dentro del `try` de C-190: un módulo ausente o viejo se habría convertido en una excepción por hora en vez de tumbar la corrida al arrancar.

### Qué cambia

- **D36, el presupuesto.** `solve_coupled_for_hour` devuelve además `nfev` y `njev` del integrador, dos campos nuevos de `CoupledTrajectory` con valor por defecto cero; el resto de su salida no cambia. Con la parada activa, `_resuelve_acoplado` guarda la última vuelta que resolvió bien, suma las evaluaciones gastadas y, antes de doblar, estima el coste de la vuelta siguiente: las evaluaciones de la última por el cociente medido entre las dos últimas (nunca menor que uno) o, si solo hay una, por la constante de módulo `FACTOR_CRECIMIENTO_DOBLEZ`. Si lo gastado más lo estimado pasa del presupuesto, no la empieza y se queda con la última vuelta buena. El presupuesto es `SolverParams.presupuesto_eval_acoplado` (None: la constante `PRESUPUESTO_EVAL_ACOPLADO`; lo que no sea un entero positivo se rechaza al construir) y la opción `--presupuesto-eval-acoplado` de `main_simulation.py`. Solo actúa con la parada activa: con `horizonte_max_acoplado=None` hay una sola resolución, idéntica al bit.
- **La calibración**, medida en la máquina de trabajo sobre la hora 4184 de M1 con la generación por siete (`paso_a_paso.carga`), en primer plano, con los parámetros de producción (tolerancias relativa y absoluta en 1e-6, 150 puntos, un solo proceso) y una resolución por horizonte: al 0,05, 141,4 (s) y 1 292 398 evaluaciones (34 564 jacobianas); al 0,1, 365,9 (s) y 3 310 324 evaluaciones (87 627 jacobianas). El factor de crecimiento por doblez es su cociente, 2,56. La velocidad es de unas 9 073 evaluaciones por segundo, de modo que un presupuesto de 5 300 000 evaluaciones son unos 584 (s), el 65 % del plazo por hora. Es un conteo y no un reloj: corta en el mismo sitio en cualquier máquina, aunque los segundos cambien con ella. Una observación de la medición: al 0,05 esa hora ya es estacionaria según la medida del motor (el precio se mueve el 0,13 % de su recorrido en el último décimo, por debajo del 1 %), de modo que con la parada activa se detiene en la primera vuelta; la señal de H-79 sobre esa hora es la del excedente, que es otro criterio.
- **D37, la vuelta que falla.** Si una vuelta devuelve `success=False`, o termina con precios o cantidades no finitos (un recorrido no finito pasaba por estacionario y desplazaba a la vuelta buena), la hora se queda con la vuelta anterior. Si falla la primera, la hora queda sin mercado con el motivo «integrador sin exito al horizonte <h>», también con la parada apagada: los números no cambian, porque ya quedaba sin mercado, pero ahora dice por qué. Una excepción no se atrapa en la parada: sube al trabajador, que la anota como en C-190.
- `HourlyResult.parada_acoplado` dice por qué paró cada hora: «estacionario», «tope», «presupuesto», «fallo_vuelta» o, con la parada apagada, «una_vuelta» (vacío en la vía alternada). `horizonte_usado` es el de la vuelta que se conservó. La recursión de la restricción de participación de H-43 recibe solo lo que queda del presupuesto de la hora, `max(1, presupuesto - gastado)`, porque `_resuelve_acoplado` devuelve también las evaluaciones que gastó: con un presupuesto entero, una hora que gastó casi todo en el conjunto completo gastaría dos (unos 1168 (s) a la velocidad medida) y vencería el plazo con las dos soluciones dentro, y la recursión puede anidarse. La primera vuelta del conjunto reducido corre siempre, de modo que el peor caso es un presupuesto más una vuelta al horizonte de producción; con la parada apagada, el presupuesto sigue inerte. La recursión sube además la parada del conjunto reducido. La tupla del trabajador gana un campo (29), en los tres sitios de producción que la construyen, y una de 28 campos sigue aceptándose.
- El `import` de `solve_coupled_for_hour` pasa a nivel de módulo en `core/ems_p2p.py`.
- **D38, el código de salida.** `main_simulation.py` gana dos funciones puras: `cuenta_para_salida`, que saca de las horas del mercado cuántas hubo de mercado, cuántas vencieron y cuántas terminaron con excepción, y `codigo_de_salida`, que devuelve 3 si hubo alguna excepción o si las vencidas pasan del 1 % de las horas de mercado, y 0 si no. El denominador son las horas en que el juego de verdad se planteó, las que traen vendedores y compradores más las vencidas, que el plazo devuelve sin ids: todas las horas pasan por el trabajador, pero la que no tiene los dos lados sale en su primera línea y no puede vencer. «Más del 1 %» es estricto: justo el 1 % sale con 0. Solo el bloque de la línea de órdenes lo aplica, después de que `main` escribió todo, e imprime una línea `[D38]` con el código y su causa; las llamadas a `main` desde pruebas y compuertas no cambian.
- Los resúmenes: con la parada activa, `[D26]` dice también cuántas horas con mercado pararon por cada motivo; `[D37]` cuenta las horas sin mercado por una primera vuelta fallida, solo si hay alguna. Con la parada activa, y solo entonces, `horizonte_usado` y `parada_acoplado` llegan como dos columnas más a la tabla de horas del almacén, que no tiene esquema fijo.
- El lanzador: `recoger` sabe qué espera según la corrida (`recoger matriz`, el almacén de cada uno de los trece casos y las figuras de E0; `recoger oficial`, las rutas de siempre), lo comprueba antes y verifica después que quedó en el fichero comprimido, leyendo la lista sin tubería (con `pipefail`, un `grep -q` que corta la tubería daría un aviso falso); `sonda79` borra el veredicto de cada caso antes de correr su sonda; `humo_linux` gana un cuarto paso, el mismo día de E5 con las dos palancas, cuyo código 3 se imprime sin detener el humo; y `matriz`, si un caso sale con 3, se detiene con un mensaje que manda mirar `[D24]`, `[C-190]` y `[D38]` de su registro y retomar con `DESDE=<caso>`. Toda escritura nueva pasa por una función que respeta `SECO`.
- `MONTAJE_SERVIDOR.md`: el tiempo esperado de la matriz se corrige con su fuente. Los 89 (min) eran las dos fronteras de 6144 horas de la corrida oficial, no una; una frontera son unos 44,5 (min) de mercado a ese ritmo. Se añade el efecto del presupuesto.

Prueba nueva: `tests/test_presupuesto_acoplado.py`, con 24 casos y sin integrar de verdad (`solve_coupled_for_hour` se sustituye en el módulo del motor por trayectorias fabricadas, cada una con su `nfev`), añadida a `compuertas` y a `COMPUERTAS` de `modelo_base/empaquetar.py`. Cubre el presupuesto pequeño, que para tras la primera vuelta y la conserva idéntica; el cociente medido, que manda sobre la constante; el presupuesto inerte con la parada apagada; la segunda vuelta fallida, que conserva la primera; la primera fallida, sin mercado y con motivo, con la parada activa y con la apagada; estacionario y tope, con su motivo; el presupuesto que viaja desde `SolverParams` por `run_single_hour` y por el lote de `EMSP2P.run`; el rechazo de presupuestos que no son enteros positivos; el reintento de H-43, que recibe solo lo que queda del presupuesto y para en «presupuesto» donde con uno entero habría doblado, que hace una sola vuelta si el conjunto completo lo agotó, y que recibe el presupuesto intacto con la parada apagada; la vuelta no finita, que no desplaza a la buena y, si es la primera, deja la hora sin mercado con su motivo; y `codigo_de_salida` (0 sin nada, 3 con una excepción, 3 por encima del 1 %, 0 justo en el 1 %, rechazo de conteos imposibles) con `cuenta_para_salida`. En `tests/test_oraculo_anexo4.py`, un segundo caso con datos reales y la generación por siete (numeral 2 y tramo del exceso), que se salta si faltan las mediciones. `tests/test_palancas_acoplado.py` se ajusta a la terna que ahora devuelve `_resuelve_acoplado`.

### Estado

**Corregido en el código, sin corrida oficial.** Tras la re-revisión (el reintento de H-43 con lo que queda del presupuesto, la vuelta no finita, la comprobación del tar sin tubería y el horizonte de 6144 horas en `MONTAJE_SERVIDOR.md`) se repitieron las pruebas nuevas, la dorada, las de las palancas y las del plazo por hora, `bash -n` y la comprobación en seco de `recoger matriz` y `matriz`, todo en verde y sin crear nada en `modelo_base/` ni en `SALIDAS_SERVIDOR/`. Las 24 pruebas nuevas en verde; las seis de `tests/test_palancas_acoplado.py`; las siete de `tests/test_plazo_por_hora.py`; la dorada en 7 de 7, que prueba que con los valores por defecto nada cambió; las seis de `tests/test_oraculo_anexo4.py`, con las dos pasadas de datos reales; las doce de `tests/test_veredicto_sonda.py`; el preflight filtrado en 6 de 6; la compuerta C-165 con 48 horas; y las dos del almacén, con el intérprete del sistema, porque el entorno virtual de la máquina de trabajo no tiene `pyarrow` y con él fallan al arrancar por la guarda de C-171, no por este cambio. `bash -n` sin errores, y la comprobación en seco de `humo_linux`, `sonda79`, `matriz` y `recoger` sin crear nada en `modelo_base/` ni en `SALIDAS_SERVIDOR/` (listado recursivo idéntico antes y después). Queda por ver en el servidor el cuarto paso del humo, con sus líneas `[D24]`, `[D26]` y `[D38]`.

---

## C-192 · El piso de P en el acoplado y el corte al primer valor no finito (H-84; D40, D41, D42)

**2026-09-14 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

Tres defectos del solucionador acoplado (`core/coupled_ode_convergence.py`), diagnosticados en H-84 sobre la hora 4184 de la frontera M1 con la generación por siete:

- **P sin piso dentro de la dinámica.** `_rhs` leía la oferta P de cada vendedor a cada comprador tal cual venía en el estado. La dinámica de replicador (dP = P·(F − F̄)) conserva P ≥ 0 solo en aritmética exacta. El comprador 0 tiene un déficit de 0,1218 (kWh) y el arranque le asigna 7,77 (kWh); su multiplicador de demanda se dispara (`VEL_GRAD` = 1e6) y empuja su columna de P a cero con una fuerza del orden de 1e7. El integrador cruza el cero y, con P negativa, el replicador se alimenta a sí mismo hasta desbordar. Que la hora explote o no lo decidía el último decimal de la entrada: en la máquina de trabajo explotaban seis de las siete perturbaciones de un ulp que se pudieron leer, y en el servidor nueve de doce corridas.
- **La entrega inflada.** La P negativa que sobrevivía la borraba después el recorte a cero de la salida (`P_t` recortado en 0). La base, que en la máquina de trabajo sobrevive, le entregaba al comprador 0 0,2319 (kWh) frente a su déficit de 0,1218, es decir 90 % más de lo que pidió. El precio de equilibrio no cambiaba, de modo que el defecto era mudo: toda hora que terminara con alguna P negativa podía liquidar energía inflada.
- **El integrador que sigue tras el NaN.** Con scipy 1.17, LSODA no se detiene al aparecer un NaN en el lado derecho: en el servidor siguió unos catorce millones de evaluaciones en 15 (min) sin avanzar en el tiempo, y en la máquina de trabajo una perturbación terminó con «éxito» y el estado entero en NaN. En la matriz, cada hora así ocupaba un trabajador hasta que la cortaba el plazo por hora (D24), y como vencida contaba para el código 3 de D38.

### Qué cambia

- **D40, el piso.** En `_rhs`, `P = np.maximum(X[idx0_P:idx0_lam].reshape(J, I), 1e-10)`, exactamente la línea del experimento de H-84; ninguna otra línea de la dinámica cambia, y el estado integrado no se toca: solo se lee con piso. Es el mismo piso de 1e-10 del arranque. No es cero a propósito: con piso cero la columna sería absorbente (P = 0 da dP = 0 para siempre) y el comprador no podría volver a recibir. El comentario lleva el aviso de fidelidad: `JoinFinal.m` (`join`, líneas 146 a 166) no tiene este piso; es una salvaguarda de la traducción.
- **D41, el corte.** Una excepción interna propia, `_LadoDerechoNoFinito`, que `_rhs` lanza si recibe un estado no finito o produce una salida no finita, y que atrapa solo `solve_coupled_for_hour`, alrededor de `solve_ivp`; ningún `except` genérico. La función devuelve entonces una `CoupledTrajectory` con `success=False`, el mensaje «lado derecho no finito en t=<t> (estado o salida de la evaluacion <n>)», `nfev` igual a las evaluaciones hechas hasta el corte (esa incluida; `_rhs` las cuenta, porque la cuenta de `solve_ivp` no llega), `njev` cero y solo el punto inicial, con las formas de siempre: `t` (1,), `pi_t` (I, 1), `P_t` (J, I, 1), el bienestar (1,), `pi_star` (I,), `P_star` (J, I) y, si se piden, los multiplicadores con una columna. La trayectoria normal pasa por el mismo posproceso, sin cambio. El motor no se toca: `_resuelve_acoplado` y `_run_hour_worker` ya tratan `success=False` por D37, de modo que la hora cortada queda sin mercado con el motivo «integrador sin exito al horizonte <h>» o, con la parada activa, con la vuelta anterior buena. La excepción nunca llega al `except` de C-190: la hora cortada no cuenta como excepción. Por D44 sí cuenta para el umbral del 1 % de D38, sumada a las vencidas (abajo).
- **D42.** La vía alternada (`core/replicator_sellers.py`, `core/replicator_buyers.py`) queda como está.
- **D44 (decisión del autor, 2026-09-14): D38 cuenta también las horas sin éxito.** Con D41, la hora que explota ya no vence el plazo: se corta y queda sin mercado con el motivo «integrador sin exito», y `cuenta_para_salida` solo contaba «vencio el plazo» y «excepcion», de modo que ya no habría detenido la matriz. `cuenta_para_salida` (`main_simulation.py`) devuelve ahora cuatro conteos (horas de mercado, vencidas, excepciones y sin éxito); `codigo_de_salida` recibe la cuenta de sin éxito como argumento obligatorio, para que una llamada con la cuenta vieja falle en voz alta, y aplica el umbral del 1 % a la SUMA de vencidas y sin éxito: dos cantidades que por separado no lo pasan pueden pasarlo juntas. Una sola excepción sigue bastando para salir con 3, y el denominador es el mismo (las horas con los dos lados más las que traen motivo). La línea `[D38]` da las dos cifras por separado, la suma y, si sale con 3, por qué, y manda mirar `[D24]`, `[D37]`, `[D26]` y `[C-190]`. Las pruebas de `codigo_de_salida` en `tests/test_presupuesto_acoplado.py` se amplían: 0 sin nada; 3 con una excepción; 3 con las vencidas o con las sin éxito por encima del 1 %; 3 cuando la suma pasa del 1 % aunque ninguna de las dos lo pase sola (una y una de cien; seis y cinco de mil); 0 justo en el 1 %, también repartido entre las dos; el rechazo de conteos imposibles, también cuando la suma pasa de las horas de mercado, y el de la llamada sin la cuenta de sin éxito. `cuenta_para_salida` da (4, 1, 1, 1) sobre las cinco horas de su prueba. `MONTAJE_SERVIDOR.md` lo dice en el «Qué mirar» del humo y de la matriz.
- **Los llamadores, con una trayectoria de un solo punto** (revisados con Grep). El motor (`_resuelve_acoplado`, `_run_hour_worker` y `run_convergence`) mira `success` antes de usar la trayectoria. Ninguno se rompe con un punto: los que miden la cola del precio (`converge_acoplado.py`, `coste_acoplado.py`, `smoke_equivalence.py`) toman la cola con `max(1, n // 10)` o `max(int(0,9·n), n − 5)`, válidos con n = 1, y los demás leen `pi_star`, `P_star` o el último punto del bienestar. `paso_a_paso.py`, `reparto_vs_integrador.py`, `coste_acoplado.py`, `smoke_equivalence.py` y `smoke_solver_robustness.py` registran o filtran `success`; no lo miran, y con una hora cortada leerían el punto inicial como resultado, seis sondas (`alternado_vs_acoplado.py`, `caso_publicado.py`, `competencia_real.py`, `error_centralizado.py`, `eficiencia.py`, `converge_acoplado.py`), `scripts/demo_invariancia_c_lambda.py`, `scripts/plot_price_curves_equilibrio.py` (la llamada de su línea 82), `tests/gate_validacion_horaria.py` (la de su línea 53) y tres compuertas de `compuertas`: `tests/gate_cal49_competencia.py` (función `acoplado`, líneas 59 a 65), `tests/gate_h45_techo_por_comprador.py` (`corre`, líneas 62 a 65) y `tests/gate_h46_peso_virtual.py` (`corre`, líneas 54 a 57). No es nuevo (antes leían el último punto que alcanzaba el integrador), pero con un corte sus comprobaciones podían pasar en falso: la revisión encontró que las comprobaciones 1 a 3 de `gate_h45` pasarían comparando arranques. Por eso las tres compuertas ganan, en esa función y sin ningún otro cambio, un fallo explícito (`AssertionError` con el caso y el mensaje del integrador) si la trayectoria no terminó con éxito. Las sondas, los guiones y `gate_validacion_horaria.py` quedan anotados, sin tocar.
- **Las compuertas.** `test_piso_P` entra en el bucle de `compuertas` de `modelo_base/run_servidor.sh` y en `COMPUERTAS` de `modelo_base/empaquetar.py`. `modelo_base/MONTAJE_SERVIDOR.md` gana la sección «El entorno exacto (H-84)», con cómo se rehizo el entorno del servidor (Python 3.13.7 con `uv` y `requirements-lock.txt`) y la regla de comparar cifras entre máquinas solo con el mismo entorno, y la cuenta de pruebas de `compuertas` pasa de dieciséis a diecisiete.

Prueba nueva: `tests/test_piso_P.py`, con 18 casos y sin datos reales. Las entradas de la hora son literales hexadecimales (`float.fromhex`), sacados una vez con `paso_a_paso.carga("m1", factor_generacion=7)` y la receta de los guiones de H-84; su huella SHA-256 es `678c7a72873ca47b`, la de la máquina de trabajo y la del servidor, y una prueba la comprueba.

- **Rápida, la que decide.** Al horizonte 0,002, que cubre el evento de t ≈ 1,06e-4, la base y cuatro perturbaciones de un ulp (`G_net[0]` y `D_net[0]`, arriba y abajo) terminan con éxito, sin valores no finitos, y con el mismo precio final dentro de una tolerancia relativa de 1e-6, la del integrador; la mayor diferencia medida es de 3,0e-9.
- **Lenta** (unos 3 (min) en la máquina de trabajo). La base al horizonte de producción, 0,05, termina con precio 711,1344 · 456,5695 · 254,5658 y entregas 0,121831 · 9,63799 · 13,558479 (kWh), con 1 155 804 evaluaciones, las mismas del experimento en memoria; ningún comprador recibe más que su déficit más 1e-6.
- **D41.** Con `monkeypatch` sobre `solve_ivp` del módulo, el lado derecho verdadero recibe en la evaluación k un estado con un NaN, o un estado finito con el precio del comprador 0 en −1, que anula el denominador de sus pagos y hace no finita la salida. Para k igual a 1, 7 y 50, la función vuelve sin ninguna evaluación más, con `success=False`, el mensaje del corte, `nfev` igual a k y las formas coherentes. Un corte no contamina la resolución siguiente en el mismo proceso (la misma trayectoria, al bit, antes y después). Y por `_run_hour_worker`: la primera vuelta cortada deja la hora sin mercado con el motivo «integrador sin exito», con la parada apagada y con la activa; la segunda vuelta cortada conserva la primera, idéntica a resolverla directamente.

**La roja.** Con la línea del piso devuelta a la original un momento (D41 activo), la prueba rápida falla en 36 (s): 4 casos fallan y 2 pasan. `G_net[0]+1ulp`, `G_net[0]-1ulp` y `D_net[0]+1ulp` explotan y D41 los corta en las evaluaciones 16 477, 16 851 y 16 982 (t = 1,057e-4, 1,106e-4 y 1,089e-4), y la comparación de precios falla con una diferencia relativa de 0,245 (533,35 frente a 534,36 (COP/kWh)); pasan la base y `D_net[0]-1ulp`. Registro: `outputs/run_2026-09-14_piso_P_roja_sin_piso.log`.
...
## C-194 · El arranque factible del acoplado, como opción apagada, y la herramienta que compara los dos arranques (H-85; D45, D46)

**2026-09-15 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

`solve_coupled_for_hour` (`core/coupled_ode_convergence.py`) arrancaba la oferta P repartiendo a partes iguales, como `JoinFinal.m`: si la oferta cubre la demanda, el déficit de cada comprador a partes iguales entre los vendedores; si no, el excedente de cada vendedor a partes iguales entre los compradores. En la hora 753 de E4 (generación por siete) el comprador 1, con un déficit de 1,90 (kWh), arranca con 10,45; su multiplicador de demanda crece en proporción a sí mismo, desborda el mayor número de coma flotante hacia t ≈ 7,2e-6 y el corte de D41 deja la hora sin mercado. Es una de las nueve horas «sin éxito del integrador» de la matriz de trece corridas (H-85). No había forma de pedir otro arranque ni de medir su efecto sobre las horas que ya resolvían, y en la hora 4184 el experimento en memoria de H-85 mostró que el arranque puede mover el reparto del precio entre dos compradores con las mismas entregas.

### Qué cambia

- **D45, la opción.** `core/coupled_ode_convergence.py` gana `ARRANQUES` y la función `_arranque_P0(G_net_j, D_net_i, regla)`, y `solve_coupled_for_hour` el parámetro `arranque="iguales"`, que usa esa función en lugar de las dos líneas del arranque. «iguales» son esas dos líneas tal cual, con el mismo recorte a 1e-10. «factible» es `np.outer(G_net_j / sum_G, D_net_i)` si `sum_G >= sum_D` (cada comprador recibe su déficit completo; cada vendedor ofrece `G_net_j · sum_D / sum_G`, que no pasa de su excedente) y `np.outer(G_net_j, D_net_i / sum_D)` si no (cada vendedor ofrece su excedente completo; cada comprador recibe `D_net_i · sum_G / sum_D`, que no pasa de su déficit), con el mismo recorte. Es decir, las sumas por columna no pasan del déficit ni las sumas por fila del excedente. Cualquier otro valor lanza `ValueError`, también en la hora sin mercado (se valida antes del caso degenerado); el factible con una suma que no es positiva también, en vez de devolver un NaN. El comentario lleva el aviso de fidelidad: `JoinFinal.m` reparte a partes iguales, y el arranque factible es una extensión de esta traducción.
- `core/ems_p2p.py`: `SolverParams.arranque_acoplado = "iguales"`, validado en `__post_init__`; `_resuelve_acoplado` gana `arranque` y lo pasa en cada vuelta de la parada por estacionario; la tupla de `_run_hour_worker` gana el campo 30, con un shim desde 29 campos que pone «iguales», y una regla desconocida en una tupla armada a mano falla en voz alta antes de resolver, no como excepción del acoplado por hora (C-190). Los tres sitios que arman la tupla la llevan: el lote de `EMSP2P.run`, la recursión de H-43 (el conjunto reducido se resuelve con el mismo arranque) y `run_single_hour`. Revisados con Grep: ningún otro sitio de producción la arma; los que la arman a mano (pruebas anteriores y `banda_precios.py`) pasan por los shims, con el arranque de siempre.
- `main_simulation.py`: la opción `--arranque-acoplado {iguales,factible}` (defecto `iguales`) llega hasta `SolverParams` en las dos llamadas a `main`; con `factible`, una línea `[D45]` al principio del mercado, y un aviso si se pide sin `--metodo acoplado`.
- **D46, la herramienta.** `reformateo/documento/scripts/sonda/compara_arranque.py`, nueva, compara dos almacenes de la misma ventana, uno con cada arranque (A el de hoy, B el factible). Una función pura, `compara`, recibe los `DataFrame` de las tablas de horas y de flujos y no necesita pyarrow; el cargador, `carga`, lee el almacén con `core.almacen.lee`. La entrega de un comprador es la suma de `kwh` de sus pares, su precio el `precio` de sus pares (se comprueba que es el mismo para todos sus vendedores) y su pago, precio por entrega. Por hora alineada informa: las horas de mercado y las sin resolver, con su motivo, en cada almacén; la clase de cada hora (igual, precio indeterminado, entregas distintas, compradores distintos, resuelta con un solo arranque, sin resolver en las dos, sin mercado); las horas cuyo resultado cambia (alguna entrega por comprador que no coincide, es decir cuya diferencia pasa de max(1e-4 (kWh), 1e-5 por la mayor de sus dos entregas), o algún precio por comprador con diferencia mayor que 1e-3 (COP/kWh), o la hora resolvió con un solo arranque); el máximo y la media de la diferencia de precio, de entrega y de pago por comprador, y el pago de cada comprador sumado sobre las horas resueltas con los dos; y las horas de precio indeterminado (las entregas de todos los compradores coinciden y algún precio difiere más de 1e-3 (COP/kWh)), con lo que se mueve el pago de cada comprador y si la suma de los pagos se conserva con el mismo criterio combinado, es decir si su diferencia no pasa de max(1e-3 (COP/kWh) por la energía de la hora, 1e-5 por la mayor de las dos sumas). Da también la diferencia de la suma de los precios, que en la hora 4184 es la que se conserva aunque la de los pagos no, porque los dos compradores reciben energías distintas. Escribe el CSV hora a hora y el detalle por hora y comprador, imprime un resumen y sale con 0; falla en voz alta si las dos ventanas no coinciden, si una hora resuelta no trae flujos o si hay valores no finitos. La tolerancia de las entregas es combinada, y no los 1e-6 (kWh) absolutos del brief, por decisión del controlador: el almacén guarda en float32, cuyo paso cerca de 20 (kWh) es de unos 1,9e-6 (kWh), y con 1e-6 la hora 4184, justo el caso que D46 quiere contar, podría salir como «entregas distintas» por un redondeo. La del precio sigue en 1e-3 (COP/kWh). El resumen y la primera línea de cada CSV (precedida de «#») dicen qué tolerancias se usaron, y `--tol-kwh`, `--tol-rel` y `--tol-precio` permiten repetir la comparación con otras.
- `modelo_base/run_servidor.sh`: la acción `arranque`. Un `bash -n` de cortesía (las compuertas no se repiten); cuatro corridas de la semana del 5 al 11 de mayo de 2025 (`--desde 2025-05-05 --hasta 2025-05-12`, que contiene la hora 753), E0 y E4 (`--factor-generacion 7`) con `--arranque-acoplado iguales` y con `factible`, las opciones de la matriz sin el análisis (`--data real --full --include-c5 --no-regulado --metodo acoplado --plazo-hora 15 --horizonte-max-acoplado 0.4`), cada una con su almacén y su `--out-dir` en `SALIDAS_SERVIDOR/arranque/<caso>_<regla>` y `PROCS` del entorno, con `PARA_EN_FALLO=0` para que el código 3 de D38 de E4 con el arranque de hoy no la detenga; la comparación de E0 y de E4, con su CSV en `SALIDAS_SERVIDOR/arranque/`; y la recogida. `corre()` deja el código de su última orden en `CODIGO_CORRE` («seco» en seco), y la acción lo imprime por corrida y todos juntos al final. `recoger arranque` espera los cuatro almacenes y los dos CSV hora a hora; la comprobación previa pasa de `-d` a `-e` y la del fichero comprimido acepta la forma de fichero además de la de carpeta, lo que no cambia nada para `matriz` ni `oficial`. Las dos pruebas nuevas entran en `compuertas`. Toda escritura de la acción pasa por `crea_dir` o `corre()`.
- `modelo_base/MONTAJE_SERVIDOR.md`: la sección «La medición del arranque (H-85, D45 y D46)», con la orden, qué mirar y el tiempo esperado, y la cuenta de pruebas de `compuertas`, que pasa a diecinueve. `modelo_base/empaquetar.py`: la herramienta en `SONDAS` y las dos pruebas en `COMPUERTAS`.

Pruebas nuevas, sin datos reales:

- `tests/test_arranque_acoplado.py`, 21 casos. La regla: el factible cumple las dos capacidades en cuatro casos (la oferta cubre la demanda, con y sin un comprador de déficit pequeño; al revés; y la hora 753) y agota el lado corto; «iguales» y el defecto reproducen al bit las dos líneas de antes; el estado inicial que recibe el integrador, capturado sin integrar, lleva con el defecto la oferta de siempre al bit y con «factible» la del reparto factible, con el resto del estado idéntico; y una regla desconocida se rechaza en `_arranque_P0`, en `solve_coupled_for_hour` (también sin mercado), en `SolverParams` y en la tupla. El motor: con el defecto, `_run_hour_worker` da lo mismo que con la tupla vieja de 29 campos; con «factible», `run_single_hour` y el lote de `EMSP2P.run` dan lo mismo, al bit, que llamar directamente a `solve_coupled_for_hour(..., arranque="factible")`, y la prueba separa, porque en esa hora los dos arranques dan distinto; y la regla viaja por los tres sitios, la recursión de H-43 incluida. La hora 753, con sus entradas como literales hexadecimales sacados una vez con `paso_a_paso.carga("m1", factor_generacion=7)` y la receta `entradas` de los guiones de H-85, y su huella SHA-256 `3d05c2c225d2e5c2` comprobada: al horizonte 0,001, en la máquina de trabajo, el arranque de hoy desborda y D41 lo corta («lado derecho no finito en t=7.305893e-06», evaluación 15 329; al horizonte de producción el corte fue en t = 7,19e-6, evaluación 15 144, porque el horizonte cambia los pasos del integrador), y el factible termina sin valores no finitos (6 919 evaluaciones). En el servidor ese tramo corto termina también con el arranque de hoy, de modo que la prueba fijaba algo que depende de la plataforma: corregida en C-195. La lenta, al horizonte 0,05, termina con 1 579 181 evaluaciones, precio 412,9468 · 318,18 (COP/kWh) y entregas 1,898547 · 18,998732 (kWh), por debajo de los déficits 1,898567 · 19,028067: las mismas cifras del experimento en memoria. El segundo precio es el piso de la hora, 318,18 (COP/kWh).
- `tests/test_compara_arranque.py`, 14 casos: la función pura con dos tablas sintéticas de siete horas (una igual; dos de precio indeterminado, una que conserva la suma de los pagos y otra que no, como la 4184; una con entregas distintas; una resuelta con un solo arranque; una sin mercado; una sin resolver en las dos), el borde de las tolerancias, absoluto y relativo; una entrega cerca de 20 (kWh) que difiere en una unidad de redondeo de float32 (`np.nextafter`) con el precio movido 0,4 (COP/kWh), que sale como precio indeterminado, y otra que difiere 1e-3 (kWh), que sale como entregas distintas; el comprador que falta en un lado, los tres fallos en voz alta, y la línea de órdenes con el cargador sustituido, que escribe los dos CSV con las tolerancias en su primera línea, las pasa desde `--tol-kwh` y `--tol-rel` y sale con 0.

### Estado

**Aplicada en el código, apagada por defecto y sin corrida en el servidor.** Las 21 pruebas de `tests/test_arranque_acoplado.py` en verde (la lenta en 242 (s), registro `outputs/run_2026-09-15_arranque_lenta_h753.log`; las otras 20 en 17 (s)) y las 14 de `tests/test_compara_arranque.py`. Con el defecto nada cambió: la dorada en 7 de 7, las seis de `tests/test_palancas_acoplado.py`, las 17 de `tests/test_piso_P.py` sin la lenta, las 26 de `tests/test_presupuesto_acoplado.py`, las siete de `tests/test_plazo_por_hora.py`, el preflight filtrado en 6 de 6 y la compuerta C-165 con 48 horas (`outputs/run_2026-09-15_arranque_gate_c165_48h.log`). El cargador se probó con el intérprete del sistema (el entorno virtual no tiene pyarrow) sobre el almacén de E4 de la matriz comparado consigo mismo: 1 182 horas iguales, ninguna que cambie, y las tres sin resolver en los dos (753, 5631 y 5632). `bash -n` sin errores, y `SECO=1 bash modelo_base/run_servidor.sh arranque` imprime las cuatro corridas, las dos comparaciones y la recogida sin crear nada en `modelo_base/` ni en `SALIDAS_SERVIDOR/` (listado recursivo, con tamaño y fecha, idéntico antes y después); `recoger matriz` y `recoger oficial` en seco siguen igual. La decisión de adoptar el arranque factible por defecto y repetir la matriz queda para después de la medición en el servidor (D45).

---

## C-195 · La prueba rápida del arranque de siempre fijaba un desborde que depende de la máquina (H-84, H-85)

**2026-09-15 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

`test_rapida_el_arranque_de_siempre_desborda_y_d41_lo_corta`, de `tests/test_arranque_acoplado.py` (C-194), afirmaba que con el arranque de siempre y el horizonte corto de 0,001 la hora 753 no termina y el corte de D41 la detiene. Es cierto en la máquina de trabajo y falso en el servidor, donde el mismo tramo termina con 40 464 evaluaciones (compuertas del 2026-09-15, registro `modelo_base/logs/pytest_test_arranque_acoplado_2026-09-15_1503.log`). La compuerta falló con el motor en orden: la hora está mal condicionada y el instante del desborde lo decide el último decimal de cada plataforma, que es la lección de H-84.

### Qué cambia

La prueba pasa a llamarse `test_rapida_el_arranque_de_siempre_le_cuesta_mucho_mas_que_el_factible` y fija lo que no cambia de una máquina a otra: si la integración no termina, es por el corte de D41 y la trayectoria se queda en el punto inicial; si termina, es finita y le cuesta más del triple de evaluaciones que al arranque factible (40 464 frente a 6 919 en el servidor). Imprime el resultado de las dos ramas, para que el registro diga en cuál cayó. Al horizonte de producción la hora falla en las dos máquinas, que es como salió en la matriz. El dato queda también en H-85.

### Estado

**Aplicada.** En la máquina de trabajo, 34 pruebas en verde en 16 (s): las 20 de `tests/test_arranque_acoplado.py` sin la lenta y las 14 de `tests/test_compara_arranque.py`. La rama del servidor se comprueba al repetir `compuertas` allí, que es donde falló.

---

## C-196 · El criterio de parada mira el reparto, el arranque factible pasa a defecto, y la campaña que mide su costo (H-86; D45 resuelta, D47)

**2026-09-16 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

La vía acoplada declaraba estacionaria una hora cuando **el precio** se movía menos del 1 % de su recorrido en el último décimo de la trayectoria (`TOL_ESTACIONARIO`, D26), y esa era la única medida que miraba. La medición del arranque corrida en el servidor el 2026-09-15 encontró que en muchas horas el precio ya está quieto mientras el reparto entre compradores sigue moviéndose entero (H-86): de las 15 541 horas de mercado de la matriz de trece corridas, 2 556 pararon por tope y 2 475 por presupuesto, es decir una de cada tres nunca llegó al criterio; y llegar tampoco basta, porque la hora 109 de E0 paró por estacionario con residuo 0,0065 y su reparto estaba lejos del punto final. Integrada hasta el horizonte 2,0 esa hora converge a darle todo el excedente a un solo comprador, mientras el horizonte de producción lo reparte casi por igual entre cuatro. Con el arranque factible, en siete horas de E0 medidas, el reparto de producción se aparta del convergido entre el 16 % y el 53 % de la energía de la hora.

Además, la medida del reparto no se guardaba en ninguna parte, de modo que no había forma de contar cuántas horas habían quedado con la energía todavía cambiando de manos sin volver a resolver la corrida entera. Y el arranque factible (C-194), que la misma medición dejó decidido, seguía siendo una opción apagada.

### Qué cambia

- **D47, la medida.** `core/ems_p2p.py` gana `_movimiento_relativo(traj, normaliza)`, que extrae la cuenta que estaba escrita dos veces en línea (en `_resuelve_acoplado` y en el trabajador) y sirve a las dos magnitudes: admite la forma del precio, (I, n_t), y la del reparto, (J, I, n_t), que aplana a una fila por pareja. Con `normaliza="recorrido"` reproduce la medida del precio operación por operación, de modo que el criterio de hoy queda **idéntico al bit**; con `"energia"` divide por la **energía transada de la hora**, la suma de la última columna, y no por el recorrido propio del reparto. Esa elección va comentada en el código y es lo que hace comparable la medida entre horas: el reparto no tiene un recorrido propio que signifique nada, porque una pareja que nunca recibió nada tiene recorrido cero y otra que se lo lleva todo lo tiene enorme, mientras que lo que decide si la hora está hecha es cuánta energía se sigue moviendo frente a la que se reparte. Con la suma casi nula el divisor se acota a 1e-12, en vez de dividir por cero.
- **El criterio, como opción.** `SolverParams.criterio_estacionario` («precio», el defecto, o «precio_y_reparto») y `SolverParams.tol_reparto` (0,01), los dos validados en `__post_init__`; la constante de módulo `TOL_REPARTO` y la tupla `CRITERIOS_ESTACIONARIO`. `_resuelve_acoplado` recibe los dos y, con el criterio nuevo, una hora es estacionaria solo si lo son las dos medidas. El resto de la maquinaria de D26 y D36 (doblar el horizonte, tope, presupuesto, vuelta buena) no cambia: lo único que cambia es cuándo se declara estacionaria una vuelta. **El defecto no cambia en esta tarea.**
- **El residuo del reparto se guarda siempre.** `HourlyResult.residuo_reparto` lleva la medida del reparto de la vuelta que se conservó, se use o no en el criterio; vale 0,0 por la vía alternada o sin mercado, y en la recursión de H-43 es el de la vuelta que produjo el resultado, la del conjunto reducido. Viaja por donde viajaba `norm_rel_final` hasta la columna `residuo`, y `main_simulation.py` lo escribe en la tabla de horas del almacén junto a ella, en toda corrida y no solo con la parada activa. Es barato y sin él no se puede censar nada sin repetir la corrida.
- **D45, resuelta: el arranque factible pasa a ser el defecto.** `SolverParams.arranque_acoplado` pasa a «factible», con el porqué en el comentario y el aviso de fidelidad: es un apartamiento declarado de `JoinFinal.m`, sostenido en que el arranque a partes iguales cae en algunas horas sobre un conjunto simétrico del que la dinámica no sale (hora 152 de E0) y cuesta hasta mil veces más. `solve_coupled_for_hour`, la función de bajo nivel, **conserva su propio defecto «iguales»**, de modo que quien la llama directa (las compuertas que comparan contra el modelo base y las sondas) sigue recibiendo el arranque del modelo base sin pedirlo. El shim de la tupla de 29 campos también sigue rellenando con «iguales», que era el defecto cuando esas tuplas se escribieron.
- **La línea de órdenes.** `main_simulation.py` gana `--criterio-estacionario {precio,precio_y_reparto}` y `--tol-reparto`, que llegan hasta `SolverParams` en las dos llamadas a `main`; con el criterio nuevo imprime una línea `[D47]` al principio del mercado, como la `[D45]`, y avisa si se pide sin `--horizonte-max-acoplado`, donde no tiene efecto. La línea `[D26]` que resume las horas por motivo de parada informa además **cuántas pararon con el reparto todavía moviéndose**. El defecto de `--arranque-acoplado` pasa a «factible» para que producción y `SolverParams` digan lo mismo, y su aviso de «no tiene efecto sin `--metodo acoplado`» pasa a salir solo cuando el operador la pide, no por su valor: si no, saltaría en toda corrida alternada, incluido el caso sintético de siempre.
- **La campaña.** `modelo_base/run_servidor.sh` gana la acción `convergencia`, que respeta `SECO` como todas: una semana, la del 5 al 11 de mayo de 2025, para E0 y K1 (el caso con menos horas estacionarias de la matriz, el 33 %), y para cada caso tres configuraciones, la de hoy al horizonte 0,4 y el criterio nuevo a los horizontes 0,4 y 2,0. Las dos últimas con `--plazo-hora 60` y `--presupuesto-eval-acoplado 100000000`, porque la campaña existe **para ver el costo, no para acotarlo**: una hora que hoy se corta por presupuesto es justo la que hay que medir. Las seis con `--arranque-acoplado factible`, su propio almacén y su `--out-dir` en `SALIDAS_SERVIDOR/convergencia/<caso>_<configuración>`, `PROCS` del entorno, `PARA_EN_FALLO=0` y el código de cada una impreso. Después, la comparación de los repartos dos a dos con la herramienta de D46 (`compara_arranque.py`), «hoy» contra «reparto a 2,0» y «reparto a 0,4» contra «reparto a 2,0» en cada caso; el censo; y `recoger convergencia`, que espera los seis almacenes, los cuatro CSV y el censo, sin tocar lo que esperan `matriz`, `oficial` ni `arranque`.
- **El resumen del costo.** `reformateo/documento/scripts/sonda/censo_convergencia.py`, nuevo, lee los almacenes de una carpeta de campaña y da, por caso y configuración, las horas de la ventana, las de mercado, las resueltas y las que no con su motivo, cuántas pararon por cada motivo del acoplado, el horizonte usado, el residuo del precio y el del reparto (mediana, percentil 90 y máximo) y cuántas horas quedaron con el reparto por encima del umbral. La función `censa` es pura, como en `compara_arranque.py`, y sale con 0: es una medición. Resume además **lo que costó cada hora**, que es la otra mitad de la decisión de D47: los segundos de reloj y las evaluaciones del integrador (mediana, percentil 90, máximo y suma). Y cuando una medida no está, porque el almacén es anterior a D47 o porque se corrió sin la parada por estacionario, **lo dice**: cada columna ausente sale con su bandera `tiene_*`, sus estadísticas en `nan` y un aviso de «NO MEDIDO» en el resumen, para que un cero no se lea como una medición.

- **El costo por hora, en el almacén.** `HourlyResult` gana `segundos` (el tiempo de reloj de resolver esa hora por la vía acoplada, medido en el trabajador alrededor de la resolución) y `evaluaciones` (las del integrador sumadas sobre todas las vueltas, el `gastado` de `_resuelve_acoplado`); con la restricción de participación las dos suman lo del conjunto completo más lo del reducido, porque esa hora pagó las dos resoluciones. Las dos viajan al almacén junto a `residuo` y `residuo_reparto`, en toda corrida, y valen 0.0 y 0 por la vía alternada o sin mercado. Los segundos dependen de la máquina y por eso no deciden nada dentro del motor; las evaluaciones no dependen de ella, y son la misma cuenta con la que D36 hace su presupuesto.
- **Los documentos.** `modelo_base/MONTAJE_SERVIDOR.md` gana la sección «La campaña de convergencia (H-86, D47)», con la orden, qué mirar y el tiempo esperado, que es **del orden de una noche** y se dice así, con las cifras de H-86 que lo sostienen (horas de hasta 3 609 (s) con 28,7 millones de evaluaciones), y su cuenta de pruebas de `compuertas` pasa a veinte. `modelo_base/empaquetar.py` lleva el censo en `SONDAS` y la prueba nueva en `COMPUERTAS`.

Pruebas nuevas, sin datos reales:

- `tests/test_criterio_reparto.py`, 13 casos. La medida: un reparto quieto mide cero; uno que se mueve en una sola pareja mide lo que esa pareja mueve frente a la energía de la hora, y el mismo movimiento sobre una hora que reparte el doble pesa menos; con la energía nula no divide por cero y con un movimiento diminuto sale grande pero finita; la medida del precio coincide **al bit** con la de siempre sobre trayectorias al azar, sobre un precio de recorrido cero y sobre la trayectoria de un solo punto que corta D41; y una normalización desconocida se rechaza. El criterio: con «precio», la tupla de 30 campos y la de 32 dan la misma hora, y la vía de producción reproduce la hora anterior a D47 con el residuo del precio idéntico a la medida de siempre sobre la trayectoria conservada. Con el integrador sustituido, una hora de precio quieto y reparto en marcha para en la primera vuelta con el criterio de hoy y, con el nuevo, dobla el horizonte hasta que el reparto se quieta, conservando esa vuelta; si no se quieta nunca, para por tope con su residuo por encima del umbral; y el umbral manda sobre la parada. Y el residuo viaja hasta el resultado de la hora resuelta de verdad, vale cero por la vía alternada, y un criterio o un umbral desconocidos se rechazan en `SolverParams` y en la tupla.
- `tests/test_arranque_acoplado.py` gana dos casos del defecto (23 en total): `SolverParams()` trae «factible» y la función de bajo nivel sigue trayendo «iguales»; y una hora resuelta con el defecto da lo mismo que pidiendo «factible» expresamente, y distinto que pidiendo «iguales», que es lo que hace que la prueba separe.

Las pruebas que pasaron a pedir **«iguales» de forma explícita**, con su comentario, son dos: `tests/gate_almacen_cruzado.py`, porque compara el motor contra el camino independiente de la sonda (`paso_a_paso.resuelve`), que llama a `solve_coupled_for_hour` sin pedir arranque y recibe por tanto el del modelo base, de modo que con el defecto nuevo los dos caminos arrancarían distinto y la comparación mediría el arranque en vez del almacén; y la rama «iguales» de `test_la_regla_viaja_por_los_tres_sitios_que_arman_la_tupla`, en `tests/test_arranque_acoplado.py`, que afirmaba el defecto viejo. Además, `tests/test_palancas_acoplado.py` pide ahora el arranque de producción en la llamada directa contra la que compara, porque lo que esa prueba mide son las palancas y no el arranque. Las demás compuertas de la vía acoplada llaman a `solve_coupled_for_hour` directamente y no cambian de arranque; `tests/gate_c146_techo_en_el_juego.py`, `tests/gate_c151_participacion_motor.py` y `tests/gate_c165_desglose_horario.py` toman el defecto nuevo, porque lo que comprueban son propiedades y sumas, no cifras históricas.

**Ronda de revisión (2026-09-16).** La revisión de esta corrección dejó diez arreglos, todos aplicados y todos en la lista de arriba:

1. la acción `convergencia` decía, heredado de `arranque`, que las compuertas no se repiten, y **en este paquete es al revés**: el defecto del arranque se movió y `tests/gate_almacen_cruzado.py` se editó después de la última corrida de compuertas, que es justo la que se enteraría si el motor y la sonda dejaran de arrancar igual. El lanzador y el montaje dicen ahora lo contrario, con el porqué;
2. `_trayectoria_finita` mira también `P_t`, que desde D47 no es un adorno: decide la parada y viaja al almacén. Un no finito ahí daría una medida `nan`, que no es menor ni mayor que el umbral, y la hora quemaría el presupuesto entero sin que nada dijera por qué;
3. el comentario de la prueba de la energía casi nula prometía una guarda que no existe: lo que la cota de 1e-12 asegura es que la medida salga **finita**, no que sea grande, y esa hora sí se declara estacionaria, que es lo correcto cuando no hay energía que repartir;
4. los dos `getattr` que arman la tupla respaldan con «iguales», no con «factible», por la misma razón que el shim: lo que se rellena por compatibilidad es el defecto de cuando se escribió ese camino, no el de hoy;
5. `_resuelve_acoplado` rechaza un criterio desconocido en voz alta, en vez de dejar que una comparación con un valor mal escrito degrade en silencio al criterio flojo;
6. y 7. el censo distingue «cero» de «no medido»: cada columna ausente sale con su bandera `tiene_*`, sus estadísticas en `nan` y su aviso, también en el CSV, y los nombres del costo son fijos en vez de una lista de candidatos;
8. `compara_arranque.py` gana `--etiqueta-a` y `--etiqueta-b`, con los rótulos de siempre por defecto, y la campaña pasa el nombre de cada configuración;
9. `tests/test_censo_convergencia.py`, 17 casos, con la misma forma que `test_compara_arranque.py`, incluidos los tres casos de «no medido»;
10. el costo por hora en el almacén, `segundos` y `evaluaciones`, que es lo que D47 tiene que poner en la balanza y hasta ahora no se podía leer.

**Re-revisión (2026-09-16).** Cuatro observaciones menores, las cuatro aplicadas. El guardia de NaN era el único retorno que volvía sin fijar el costo de la hora, y ahora lo fija como sus cuatro hermanos: una hora que revienta no puede decir que costó cero, que es el mismo cero que el censo acaba de dejar de escribir. El comentario del campo describía mal sus propios valores, porque tres de los caminos sin mercado guardan el costo de verdad a propósito, y dice además que **ese costo no tiene consumidor hoy**, porque esas horas se anotan por `sin_resolver`, que escribe el motivo y nada más: no se perdió, nunca se escribió. `segundos` queda definido por lo que de verdad mide, **tiempo de pared del trabajador con `PROCS` horas corriendo a la vez**, con sus dos consecuencias prácticas (la mediana sale inflada frente a una hora aislada, y la suma de la columna no es la duración de la corrida sino del orden de `PROCS` veces), en el campo, en el montaje y en la cabecera del censo, dejando dicho que entre configuraciones de la misma campaña la comparación sí es justa y que la cifra que no depende de la máquina es `evaluaciones`. Y el costo por hora gana cuatro pruebas **en el motor**: hasta ahora solo estaba probado sobre las tablas sintéticas del censo, de modo que si el motor dejara de escribir esas columnas, las diecisiete del censo seguirían en verde y se sabría después de gastar la noche de servidor.

### Estado

**Aplicada en el código y sin corrida en el servidor.** El horizonte de producción **no cambia** en esta tarea: se fija con el costo medido por la campaña (D47), y la matriz no se repite.

En la máquina de trabajo, tras las dos rondas, **126 pruebas en verde**: `tests/test_criterio_reparto.py` (17, con los cuatro casos del costo en el motor) y `tests/test_censo_convergencia.py` (17), 34 en 26 (s); `tests/test_arranque_acoplado.py` sin la lenta (22) en 18 (s); `tests/test_compara_arranque.py` (14); `tests/test_presupuesto_acoplado.py` (26) y `tests/test_palancas_acoplado.py` (6); y `tests/test_piso_P.py` sin la lenta (17) con `tests/test_plazo_por_hora.py` (7), que se corrieron porque el punto 2 de la revisión toca `_trayectoria_finita`, que es lo que esas dos ejercen. Antes de la ronda pasaron además las 7 de `tests/golden_test_sofia.py` y las 6 del preflight con su filtro. Las trayectorias fabricadas de tres pruebas llevan ahora `P_t`, como las de verdad, porque el motor mide sobre ella el residuo del reparto; ninguna de sus cifras cambia.

**Ninguna cifra publicada se movió con el defecto nuevo del arranque.** Las tres compuertas que lo toman dan el mismo veredicto que con el arranque anterior, comprobado corriendo cada una dos veces, la segunda forzando el arranque de antes: C-146 en verde con cero compradores de ahorro nulo en sus 6 horas; C-151 en verde con los mismos 6 retiros en 6 horas, cero energía evaporada y cero vendedores a pérdida; C-165 en verde con el desglose sumando el total por agente con diferencias relativas del orden de 1e-16. Lo único que se movió es **el tiempo**, y a favor: el bloque acoplado de C-146 tardó 2 min 57 s con el arranque nuevo frente a 4 min 58 s con el de antes, y el segundo bloque de C-151, 5 min 00 s frente a 5 min 09 s. En C-165 la diferencia relativa de la suma del P2P pasó de 4,49e-16 a 3,39e-16, es decir el último dígito de una comprobación que trabaja en el límite de la máquina, y sus 48 horas tardaron 4 min 06 s frente a 4 min 09 s.

Las compuertas que llaman a `solve_coupled_for_hour` directamente, y que por tanto siguen recibiendo el arranque del modelo base, se corrieron para confirmarlo: `tests/gate_h45_techo_por_comprador.py` y `tests/gate_h46_peso_virtual.py`, las dos en verde.

`bash -n` del lanzador sin errores, y `SECO=1 bash modelo_base/run_servidor.sh convergencia` imprime las seis corridas, las cuatro comparaciones, el censo y la recogida **sin crear nada** en `modelo_base/` ni en `SALIDAS_SERVIDOR/` (listado recursivo, con tamaño y fecha, idéntico antes y después). Repetido tras la ronda de revisión: el aviso de las compuertas sale en el paso 1 y cada comparación lleva sus rótulos (`--etiqueta-a hoy --etiqueta-b reparto20`, y los suyos las demás).

---

## C-197 · El núcleo del mercado en el reposo del juego regularizado, en forma cerrada, y sus pruebas (H-87, H-88, H-89, H-90; D48 a D55, D61 y D62; CAL-53, ADR 0060)

**2026-09-16 y 2026-09-17 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

El mercado de cada hora se resolvía integrando la dinámica de replicador por la vía acoplada (`core/coupled_ode_convergence.py`, `solve_coupled_for_hour`) hasta un horizonte, y lo que se guardaba era el estado en ese instante, es decir una foto. H-86 mostró que el horizonte de producción no fija el reparto entre compradores; H-87, que con un vendedor y varios compradores no hay horizonte que lo fije, porque la dinámica oscila alrededor de un reposo que no alcanza, y que el precio medio guardado es un invariante del arranque; y H-88, que el nivel del precio del modelo base no tiene ancla dentro de la banda colombiana. En la hora 109 de la semana de E0 del 5 de mayo, que es la 853 de la matriz (2025-05-09 13:00; las dos numeraciones, en H-90), la parte del vendedor valía 0,695, 0,618 y 0,526 a t = 0,05, 0,4 y 2 (`evaluador2-report.md` §2.3). Ninguna función calculaba el reposo directo, de modo que el reparto y el precio por institución dependían del instante en que se detenía el cálculo.

### Qué cambia

Esta corrección es solo el núcleo. Su conexión con el motor (la vía `reposo`, la participación de los vendedores, los campos del almacén y la línea de órdenes) es la tarea 2 y se registra aparte (C-198).

- **`core/reposo_mercado.py`, nuevo.** Funciones puras, sin estado, sin E/S y sin dependencia del motor, que calculan el mercado de una hora en el reposo del juego regularizado con la especificación de la síntesis final (`.superpowers/sdd/2026-09-16-sonda-equilibrio/fable-report.md`, §5.1), pasos 1 a 6, 8 y 9. El paso 7, la participación de los vendedores (C-151), queda para el motor, que usará `ingreso_por_vendedor`.
  - `resuelve_reposo(s, d, b, techo, piso_j, *, modo_presupuesto="sigma", sigma=None, pi_gs=None, regla_precio="uniforme", despacho_vendedores="merito")` devuelve un `ReposoHora` congelado con el reparto P, los precios del reposo y los liquidados, el régimen, el presupuesto S, el precio común ℓ, el precio uniforme p_u, los compradores excluidos (aparte, los que quedan bajo el piso del juego), los vendedores excluidos, el orden de mérito, el número de reposos encontrados (`n_soluciones`) y la parte del vendedor. Los regímenes son `interiores`, `topados`, `excluidos`, `mixto`, `suma_no_cabe`, `compradores_cortos`, `un_comprador`, `sin_mercado` y `sin_ganancia`.
  - `presupuesto_precios`, con tres modos (D50): `sigma`, el de producción, S = Σ_i [piso + σ_I·(techo_i − piso)] con σ_I = (I − 1)/I, o con un σ explícito para el barrido; `algoritmo3`, el literal del documento extenso de la autora (H-61), para reproducir el caso de Chacón; y `c136`, copia literal del arranque de hoy del acoplado con su interruptor, para comparar con la dinámica. En los tres manda D54: con un comprador, el piso.
  - El paso 3 enumera la posición de cada comprador (en su techo, interior o en el piso) y aplica la prioridad por precio y el llenado por niveles del grupo marginal; con más de un reposo publica el de más compradores interiores y lo anota. El paso 4 empareja con rango uno. El paso 5 aplica la regla de saturación de «la suma no cabe sobre el piso» (D53 corregida), que conserva la suma de precios: si hay reposos y la regla no cae en ninguno, lanza `ValueError`; si no hay ninguno, la publica como regla declarada con `n_soluciones = 0`. El paso 6, con compradores cortos, despacha por `merito` (D52, por defecto) o por `llenado`, para que la medición M-B decida sin cambiar código (H-89). El paso 8 liquida por `uniforme` (D51, solución exacta por tramos) o por `puja`.
  - **D61.** Si el piso de todos los vendedores activos supera el techo de todos los compradores activos, la hora sale `sin_ganancia`, anotada y sin error. Un comprador activo con techo bajo el piso del juego sale de la hora, sin energía y fuera del presupuesto. Un vendedor activo cuyo piso supera el techo mayor de los compradores activos queda fuera antes del orden de mérito (`vendedores_excluidos`).
  - **D62.** Con compradores cortos se despacha primero y se toma después el piso de los vendedores que despachan, en un lazo a punto fijo de a lo sumo I vueltas.
  - `ingreso_por_vendedor`, para la participación del motor; `cotas_optimalidad`, el óptimo de transporte y el peor reparto del mismo volumen (D55); y `captura`, que falla en voz alta con excedente negativo y óptimo positivo.
  - `_comprueba` falla en voz alta, antes de devolver, si se rompe alguna identidad: ningún valor no finito; ninguna energía negativa; ningún vendedor despacha más que su excedente ni ningún comprador recibe más que su déficit; las filas iguales a lo despachado; Σq = E y las columnas iguales a lo recibido; la suma de los precios del reposo de los compradores en el juego igual a S; los precios liquidados de los servidos dentro de [piso, techo]; y, con la liquidación uniforme, el ingreso del reposo conservado al 1e-9 relativo. La identidad del excedente, Σ (techo_i − piso_j)·P_ji al 1e-6, no la mira `_comprueba` (el núcleo define así el excedente): la comprueba `test_identidades`. Toda entrada no finita o negativa lanza `ValueError`, y no hay ningún `except` genérico.
- **Lo que el núcleo no usa.** El segundo lazo del prototipo de la segunda evaluación, que pone al grupo marginal en el piso y a los demás en su techo sin conservar S. Los valores esperados que el encargo traía para las horas 3086 y 877 salían de ese lazo y estaban mal. El núcleo aplica la regla del paso 5 y, con el presupuesto `c136`, reproduce al centésimo los promedios de la dinámica en las horas 540, 2317 y 3062 de la matriz de E0, y a 1,4 (COP/kWh) en la 3086, donde la dinámica oscila; el segundo lazo quedaba a entre 24 y 44 (COP/kWh) (H-90).
- **`tests/test_reposo_mercado.py`, nuevo.** Sin datos reales: las entradas de catorce horas son literales hexadecimales (`float.fromhex`) sacados una vez de los almacenes de la matriz del 15 de septiembre (`SALIDAS_SERVIDOR/entrega_matriz_2026-09-15/SALIDAS_SERVIDOR/matriz/<caso>/almacen/m1/agentes`), con su huella SHA-256 `fe4488168ced426c` comprobada antes de usarlos. Son, en la numeración de la matriz, las horas 853, 874, 2120, 3804, 3086, 877, 12, 58 y 540 de E0, con los vendedores que quedaron tras el retiro de la corrida guardada, y las horas 4766 y 226 de E0 y 133, 5631 y 5632 de E4, con los retirados incluidos, que es lo que recibirá el motor antes de su participación. Los grupos:
  - la huella de los literales;
  - las horas de E0 y la evidencia de la dinámica: S de 2 913,70 (COP/kWh) con `c136` en la 853 (la 109 de la semana), los libres a 702,70 (COP/kWh) en la 874 (la 130 de la semana), y las horas 540 y 3086 contra los promedios de `valida_reposo_E0.log`;
  - las horas reales de compradores cortos, de un comprador, sin ganancia y del piso de los que despachan. En la 4766 de E0 (piso 616,46, S = 1 332,08 y p_u = 641,64 (COP/kWh)) el mérito despacha 1,7224 (kWh) solo de Cesmag, con parte del vendedor 0,254, y el llenado, 0,5741 (kWh) de cada vendedor, con parte −0,262 antes de la participación. En la 226 de E0 el único comprador paga el piso, 695,68 (COP/kWh), con parte 0. Las 5631 y 5632 de E4 salen `sin_ganancia`, con piso 791,36 frente a techos de 720,12 (COP/kWh). En la 133 de E4 el mérito despacha solo Cesmag y el piso del juego pasa de 147,46 a 330,15 (COP/kWh), con parte 0; con llenado despachan los cuatro 1,0481 (kWh) y la parte sale −0,4406;
  - los sintéticos: σ = 0 deja todo en el piso con parte 0; un comprador paga el piso en los cuatro modos, con vendedores cortos y con compradores cortos; con bandas iguales, parte 1/2 con dos compradores y 3/4 con cuatro; con vendedores cortos el reparto no depende de b; con compradores cortos, mérito frente a llenado, también entre vendedores de igual costo; la multiplicidad anotada; D61 parcial y del lado de los vendedores; D62, incluida la oferta de los que despachan igual a la demanda; `c136` con dos compradores lanza por H-32; y las guardas de `_comprueba` con `monkeypatch`;
  - las identidades sobre todas las horas y las combinaciones de modo, regla y despacho (`test_identidades`, que añade a las de `_comprueba` la del excedente al 1e-6 y las cotas de optimalidad);
  - las entradas inválidas, la hora sin mercado, el presupuesto y las cotas calculadas a mano;
  - el caso de Chacón a las 22:00 con el Algoritmo 3: S = 5 000, precios 833,33 × 3 y 1 250 × 2 (COP/kWh) y reparto [0,3037 × 3; 0,262; 0,208] (kWh).

### Estado

**Aplicada en el código, sin commit y sin conexión con el motor.** `tests/test_reposo_mercado.py`: 81 pruebas en verde en 1,36 (s), sin `xfail` (comprobado por el controlador); la prueba dorada, 7 de 7. Ninguna corrida con datos reales completos.

La revisión, sin críticos, pidió tres arreglos importantes: que las horas sin ganancia posible no lanzaran error (D61), que el piso del juego fuera el de los vendedores que despachan (D62) y horas reales de compradores cortos y de un comprador; y siete menores, entre ellos documentar `c136` con dos compradores como H-32, que `_comprueba` verificara el ingreso, la suma de precios y las filas, documentar que la parte del vendedor puede ser negativa antes de la participación y que `captura` fallara con excedente negativo. Todo se atendió en una ronda, que retiró también los dos `xfail` estrictos con los esperados equivocados, y un arreglo corto añadió la exclusión de los vendedores sin ganancia posible. La re-revisión lo aprobó con seis menores o cosméticos (el orden de mérito en las horas sin ganancia, un parámetro muerto, el docstring de E, los excluidos que mezclaban dos causas, la cifra de 464 a 451 horas, y líneas largas y un nombre de prueba), cerrados antes de la tarea 2.

La exploración de la revisión: la guarda del paso 5 no lanzó en 130 000 horas sintéticas (40 000 realistas, 60 000 adversarias y 30 000 de compradores cortos empatados) ni en ninguna hora real. Con la participación emulada encima, ninguna parte del vendedor queda negativa y ningún vendedor despachado queda bajo su piso, que pasa a ser compuerta de la tarea 2. La re-revisión no encontró ningún invariante roto en los trece casos por siete combinaciones ni en 60 000 horas hostiles, y comprobó que las nueve horas de E0, `c136` y el caso de Chacón son idénticos al bit al núcleo anterior a los arreglos. Cuesta unos 5 (ms) por hora: los trece casos, en un minuto.

El censo del implementador de los trece casos con los retirados, en las combinaciones por defecto, `llenado`, σ = 0, 1/2 y 1 y `puja`, no da ningún `ValueError`; solo `c136` lanza, en 451 horas de ocho casos, por H-32 (la revisión había contado 464 antes de D62; H-90). El censo de E0 por régimen, repetido el 2026-09-17 con el núcleo revisado, da los mismos porcentajes que la primera corrida (H-90).

Queda por medir en el servidor la validación contra la dinámica regularizada, por régimen, y el orden de mérito (mediciones M-A a M-H; H-89 y H-90).

---

## C-198 · El motor resuelve el mercado de cada hora en el reposo, con la participación sobre el precio liquidado, y el reposo pasa a defecto con datos reales (H-90; D48 a D62; CAL-53, ADR 0060)

**2026-09-17 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

Con datos reales, el motor (`core/ems_p2p.py`) resolvía cada hora por la vía acoplada: integraba la dinámica de replicador de las dos poblaciones hasta un horizonte (CAL-48, con la parada por estacionario de D26) y guardaba el estado en ese instante, es decir una foto. H-86 y H-87 mostraron que esa foto no es un equilibrio: el reparto entre compradores sigue moviéndose al parar y, con un vendedor y varios compradores, la dinámica oscila alrededor de un reposo que no alcanza. El núcleo que calcula ese reposo en forma cerrada existía desde C-197, pero nada lo llamaba: no había vía para pedirlo, la participación de los vendedores (C-151) solo sabía trabajar con el precio del acoplado, el almacén no tenía dónde guardar el régimen ni los precios del reposo, y `main_simulation.py --data real --full` sin `--metodo` fallaba por CAL-48.

### Qué cambia

- **El tercer ramal.** `core/ems_p2p.py` gana `METODOS = ("alternado", "acoplado", "reposo")`. `_run_hour_worker`, con `metodo == "reposo"`, llama a `resuelve_reposo` (C-197) con **las mismas entradas que recibe la vía acoplada** en esa hora: el excedente neto de cada vendedor, el déficit neto de cada comprador, el costo b_j, el techo de cada comprador (C-146) y el piso de cada vendedor (H-49), o el escalar si no hay vector. La única diferencia está dentro del núcleo: el piso del juego es el mínimo de los vendedores que despachan (D62) y no el de todos los de la hora. `P_star` es el reparto del reposo y `pi_star` el precio liquidado (D51); el del reposo, el pago según puja, va a `pi_reposo`. Sin nada que integrar, los contadores del integrador quedan en cero y la parada se anota «reposo». Las horas `sin_mercado` y `sin_ganancia` (D61) vuelven sin `P_star`, con la causa en `regimen`, para que la liquidación las trate como cualquier hora sin mercado.
- **Fallar en voz alta.** Un `ValueError` del núcleo deja la hora sin mercado con su motivo, que empieza por «excepcion del reposo» (`PREFIJO_EXCEPCION_REPOSO`); la línea `[C-190]` la cuenta y la corrida sale con código 3 (D38). Solo se atrapa `ValueError`: cualquier otra excepción es un defecto del motor y sube.
- **`SolverParams`** gana `modo_presupuesto` («sigma»), `sigma_nivel` (None, la sigma base), `regla_precio` («uniforme») y `despacho_vendedores` («merito»), validados en `__post_init__`, y valida también `metodo`: antes, un nombre desconocido caía en la vía alternada sin avisar. `sigma_nivel` tiene que ser None o un número finito en [0, 1], y solo vale con «sigma». El defecto de la clase sigue siendo «alternado».
- **La tupla** del trabajador pasa de 32 a 36 campos en los tres sitios que la arman (el lote de `EMSP2P.run`, la recursión de la participación y `run_single_hour`), con un shim de 32 a 36 que pone los defectos. Las opciones se validan en toda hora, también sin mercado.
- **`HourlyResult`** gana `regimen`, `presupuesto`, `precio_comun`, `precio_uniforme`, `pi_reposo`, `piso_juego`, `excluidos`, `excluidos_bajo_piso`, `vendedores_excluidos`, `orden_merito`, `n_soluciones`, `excedente_optimo`, `excedente_peor` y `captura`, con valores neutros por defecto, de modo que las otras vías no cambian. Las listas llevan identificadores de agente.
- **La participación, con el precio liquidado** (paso 7 de la síntesis, sin cambio de criterio). Un vendedor despachado se retira si su ingreso medio con el precio liquidado queda bajo su piso. El conjunto reducido vuelve a pasar por el núcleo, porque sin el retirado cambian el piso del juego y el presupuesto; la recursión conserva la vía y las cuatro opciones y copia del conjunto reducido todos los campos del reposo (`CAMPOS_REPOSO`). Los retirados siguen en `seller_ids` con su fila a cero, que es la trampa de H-43.
- **Las cotas, después de la participación y sobre quienes pueden comerciar.** El óptimo de transporte, el peor reparto y la captura (D55) se calculan sobre el conjunto final, y antes se ponen a cero, en copias, los vendedores de `vendedores_excluidos` y los compradores de `excluidos_bajo_piso`. Sin eso, el peor reparto contaba a quien no puede comerciar: con dos vendedores de 5 (kWh) y pisos 300 y 1 300 (COP/kWh) frente a un comprador de techo 1 250, daba −250 (COP) en vez de 4 750. El óptimo y la captura no cambian.
- **CAL-32 se exige.** El núcleo supone a_j = 0. Si algún vendedor trae un costo cuadrático distinto de cero, la hora lanza `ValueError` con «CAL-32» (`_entradas_reposo`); `main` lo rechaza antes de resolver (`error_costo_cuadratico`) y la línea de órdenes, con `ap.error`, en el caso sintético, antes de cargar nada. Con datos reales `main` arma a = 0.
- **El polvo de los netos.** `classify_agents` puede dejar netos en [−1e-9, 0) por redondeo; con `TOL_NETO_REPOSO = 1e-9` se llevan a cero, en copias. Un neto más negativo lanza.
- **El defecto con datos reales.** `resuelve_metodo`: sin `--metodo`, con `--data real` o `--day` la vía es **reposo**, y la corrida lo avisa; con el caso sintético o con `--gsa`, la alternada de siempre. Un `--metodo` explícito no cambia nada, y `--gsa --metodo reposo` da un error propio, porque el GSA usa su mercado por la vía alternada. El defecto vive en la línea de órdenes: `main(use_real_data=True)` sin `metodo` sigue en la alternada. El mensaje de D45 nombra la vía resuelta, y el de CAL-48 recomienda `--metodo reposo` o `--metodo acoplado`.
- **La línea de órdenes** gana `--modo-presupuesto`, `--sigma-nivel` (un número o «base»), `--regla-precio` y `--despacho-vendedores`, con `ap.error` si la sigma no es un número finito en [0, 1] o va con otro presupuesto, y un aviso si las opciones del reposo se piden con otra vía.
- **Las dos líneas `[D48]`.** Al empezar el mercado, las reglas con que se resuelve cada hora. Al final, las horas y la energía de cada régimen (también los que dan cero), las horas del reposo que acabaron en excepción, fuera de los regímenes, los excluidos, los excluidos bajo el piso y los vendedores excluidos, la energía liquidada al techo (ahorro cero) y al piso, la topada en el reposo, la de las horas con un solo comprador y los retirados por la participación con **la energía que ofrecían** (su excedente neto) y **la comerciable** (la que algún comprador con déficit sin cubrir y techo sobre el piso del retirado habría comprado). `resumen_reposo` salta las horas con motivo, que ya cuenta `[C-190]`.
- **El almacén.** La tabla de horas lleva las columnas del reposo en todas las vías, con los valores neutros en las otras dos; las listas van como texto separado por punto y coma. `core/almacen.py`: `anota_flujos` escribe siempre `precio_reposo` (0,0 en las otras vías) y comprueba que sea finito. Las horas `sin_mercado` y `sin_ganancia` del reposo se anotan como «sin mercado esa hora» con su régimen; sin esa rama, la guarda de C-160 las habría anotado como solución no finita.

**Pruebas nuevas.**

- **`tests/test_reposo_motor.py`, 68 casos.** `run_single_hour` igual al núcleo, con cotas escalares y por agente; las opciones llegan; el lote de 24 (h) igual a `run_single_hour` con cotas al azar; el retiro con fila a cero, con el vendedor en la lista, con excluidos de D61 en la vuelta reducida y con las cotas del conjunto reducido; la recursión conserva la vía y las opciones; las cotas sobre quienes pueden comerciar, con el ejemplo de arriba y con D61 parcial, calculadas a mano; la captura después de la participación; `sin_ganancia`; el camino de la excepción (una hora de H-32 con `c136`, la vuelta reducida que revienta, y una corrida sintética de `main` con la hora forzada a lanzar, que sale con código 3 y deja una sola fila sin resolver en el almacén); el rechazo de a_j ≠ 0 en la tupla, en `main` y en la línea de órdenes; el polvo de 1e-9; los rechazos de `SolverParams`, de la tupla y de la línea de órdenes (las de `--sigma-nivel` con `--data real` y un texto que la línea de uso no contiene); `resuelve_metodo`; `resumen_reposo` con la energía ofrecida y comerciable de los retirados, calculada a mano; y una corrida sintética de `main` con almacén, por reposo y por la alternada.
- **La identidad al bit de las otras dos vías, contra el código base.** Los literales de una hora por la vía alternada y la acoplada, con y sin pisos por vendedor, se calcularon con el código del commit a392cbd (`git archive`, de solo lectura) y se comparan en hexadecimal con tuplas de 32 y de 36 campos. Los generadores están en `tests/literales/` (`genera_literales.py`, `literales_ronda1.py`, `verifica_huella.py` y `literales_base.py`, con su `README.md`), fuera de lo que pytest recoge. La prueba corre en Windows con las versiones de numpy y scipy de `requirements-lock.txt`, y ahí falla si los literales no coinciden; fuera de ese entorno se salta con su motivo (H-84). La huella de las entradas corre siempre.
- **La compuerta de la participación sobre los trece almacenes** de la matriz del 15 de septiembre: el motor, hora a hora sobre las entradas guardadas (demanda, generación, techo y piso de cada agente), por la vía por reposo con las opciones de producción. Cero excepciones, cero vendedores despachados bajo su piso, cero partes del vendedor negativas, captura siempre en [0, 1] y a lo sumo una vuelta de retiro. Horas con retiro: CV2 190, E0 113, E1 721, E2 821, E3 611, E4 302, E5 119, I1 1 032, K1 1, N1 507, P1 380, P2 32 y SINU 57. Los recuentos no cambiaron en las dos rondas de arreglos.

### Estado

**Aplicada en el código, sin commit y sin corrida en el servidor.** En la máquina de trabajo, **162 pruebas en verde** en 398 (s): las 68 de `tests/test_reposo_motor.py`, las de `tests/test_reposo_mercado.py` (solo se tocó su cabecera, sin cambiar ninguna afirmación), `tests/golden_test_sofia.py` y `tests/test_palancas_acoplado.py`; el preflight filtrado en 6; y, en la primera entrega, la compuerta C-165 con 48 horas en verde. Las vías acoplada y alternada dan lo mismo al bit que antes, contra el código base. `outputs/` y `graficas/` de la raíz no cambiaron.

La revisión, sin críticos, pidió un arreglo importante (las cotas sobre quienes pueden comerciar) y ocho menores: el régimen de una hora con excepción en las cotas, la línea `[D48]` con las horas en excepción y la energía topada, el rechazo de a_j ≠ 0, las pruebas del camino de la excepción, la identidad al bit contra el código base, los mensajes de D45 y CAL-48, la lista de llamadores (que pasó a la tarea de las acciones del servidor) y el polvo de los netos. La re-revisión encontró vacías las cuatro pruebas de `--sigma-nivel`, porque CAL-32 salía antes y el texto buscado estaba en la línea de uso, y pidió que la clave del salto no dependiera del parche de Python, llevar los generadores de literales al repositorio, una prueba del retiro con excluidos de D61 en la vuelta reducida y la del piso también con llenado. La segunda ronda lo atendió y añadió a `[D48]` la energía ofrecida y la comerciable de los retirados. La última re-revisión la **aprobó**, con tres menores aparcados (registro del controlador, `.superpowers/sdd/2026-09-16-reposo/progress.md`): `literales_ronda1.py` escribe su registro dentro de `tests/literales/`, la clave del salto no fija la versión menor de Python (fallaría en voz alta) y tres guiones no tienen la guarda `__main__`.

**Lo que se encontró por el camino: con D62, el piso del juego nunca sube tras un retiro.** La síntesis (§5.1, paso 7) decía que, tras un retiro, «el piso del juego puede subir». **Con D62 es falso: solo puede bajar.** El argumento: con el emparejamiento de rango uno, todos los vendedores despachados cobran el mismo precio medio, y ningún comprador servido paga menos que el piso del juego; de modo que el vendedor de piso mínimo entre los que despachan nunca se retira. En el conjunto reducido ese vendedor sigue despachando, porque la demanda no baja y ya no tiene más oferta por delante. Por inducción sobre el lazo de D61 parcial y D62, el piso del conjunto reducido es a lo sumo el suyo. La re-revisión lo dio por correcto, y válido también sin D62, salvo el polvo por debajo de 1e-9. Medido en 40 000 horas al azar: 25 545 vueltas de retiro, ninguna subida del piso y 462 bajadas. Lo fijan dos pruebas: una hora hecha a mano en que el piso pasa de 500 a 200 (COP/kWh) tras dos retiros, y 3 000 horas al azar en que, con mérito, el piso nunca sube y alguna vez baja, y con llenado nunca cambia.

**Lo que queda abierto: la participación retira energía comerciable.** Medido con el motor sobre las entradas de los almacenes de la matriz del 15 de septiembre, con la línea `[D48]`; no es la matriz con el reposo. La participación deja sin transar energía que algún comprador habría pagado por encima del piso del retirado: en E1, 5 977 (kWh) frente a 10 625 (kWh) transados; en E2, 6 489 (kWh) frente a 10 783 (kWh). En las horas de un solo comprador toda la energía se paga al piso (D54), con parte del vendedor cero. Se está midiendo la causa antes de correr la matriz con el reposo. **Lo abierto se resolvió con D63 a D67 (C-199, H-91).**

---

## C-199 · El piso del juego pasa a ser el del vendedor marginal, con la caminata competitiva en el núcleo, el motor, la compuerta y el lanzador (H-91; D63 a D70; CAL-53 enmendado, ADR 0060)

**2026-09-17 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

El núcleo del reposo (C-197) y el motor (C-198) resolvían cada hora con **un solo piso de juego, el mínimo de los vendedores que despachan** (D62), y despachaban los vendedores sobrantes **por orden de mérito sobre el costo nivelado b_j** (D52). Con pisos heterogéneos por vendedor (CAL-47, H-49) y emparejamiento de rango uno, todos los vendedores cobran el mismo ingreso medio, de modo que el vendedor de piso alto quedaba por debajo de su propia alternativa y la restricción de participación (C-151) lo retiraba con toda su energía. Medido sobre las entradas de los trece almacenes de la matriz del 15 de septiembre: 5 977 (kWh) perdidos frente a 10 625 transados en E1 y 6 489 frente a 10 783 en E2, y en las 4 886 horas con retiro existía siempre un precio uniforme que cubría a todos (H-91). Con un solo comprador (D54) el precio se ponía en el piso mínimo, lo que retiraba en la primera vuelta a todos los demás. La vía acoplada perdía lo mismo, porque piso_j no aparece en ninguna aptitud de su dinámica.

### Qué cambia

- **El núcleo, `core/reposo_mercado.py`.** Paso 1 nuevo, `despacho_competitivo(s, d, techo, piso_j, despacho="piso", *, b=None)`, que decide quién entra de cada lado, la energía E y el piso del juego antes de repartir los precios.
  - **La caminata competitiva (D65), `_caminata`.** Los niveles son los pisos distintos de los vendedores con ganancia posible, agrupados por `TOL_PISO = 1e-9`, y el nivel vale el mayor piso de su grupo. Para cada nivel p, la oferta es la de los vendedores con piso_j ≤ p y la demanda la de los compradores con techo_i ≥ p; **p\* es el menor nivel que maximiza el mínimo de las dos**, con tolerancia relativa para que el polvo de redondeo no mueva el nivel. E = mín(oferta(p\*), demanda(p\*)). Si la oferta no supera a la demanda despachan todos los de piso ≤ p\*; si la supera, se despacha por piso creciente con llenado por niveles dentro del grupo empatado. Por la minimalidad de p\*, el último despachado tiene piso p\*.
  - **El piso del juego es p\*** (D63), y `orden_merito` se ordena por piso_j (D64). `DESPACHOS_VENDEDORES = ("piso", "costo", "llenado")`, con defecto `"piso"`; el núcleo **no admite** `"merito"`, cuya traducción vive en el motor y en la línea de órdenes.
  - **Las dos opciones de comparación**, `"costo"` y `"llenado"`, usan `_lazo_piso_maximo`: el piso es el máximo de los despachados y el lazo de exclusión se recalcula desde todos los activos. **No es monótono:** si vuelve a un conjunto ya visto, o si pasa de `MAX_VUELTAS_LAZO = 64`, lanza `ValueError` con «oscila», en voz alta.
  - `_paso1` conserva D61: si el volumen máximo de la caminata es 0, la hora sale `sin_ganancia` con todos los vendedores en `vendedores_excluidos`.
  - **Campos nuevos** en `ReposoHora`: `piso_marginal`, `vendedores_no_despachados`, `renta_inframarginal` y `parte_juego`. **D69:** renta = Σ_j (p\* − piso_j)·s_j y parte del juego = ingreso − p\*·E, que suman exactamente la prima porque lo despachado suma E.
  - **`_comprueba` gana tres identidades**: que el piso del juego no quede bajo el mayor piso de los que despachan, con la tolerancia del piso relativa al marginal; que ese piso sea el de algún vendedor que puede vender; y el teorema de cobertura, ingreso_j/s_j ≥ piso_j al 1e-9 relativo, sobre los que despachan de verdad. La igualdad exacta entre el piso y el mayor de los despachados **no** puede ser guarda, porque el filtro de polvo del despacho y la tolerancia del nivel no comparten base: se comprueba la desigualdad que la cobertura necesita. La parte del vendedor ya no puede ser negativa.
- **El motor, `core/ems_p2p.py`.** `SolverParams.despacho_vendedores` pasa a `"piso"`, y `ALIAS_DESPACHO = {"merito": "costo"}` traduce el nombre viejo con un `UserWarning`. La tupla del trabajador **no cambia de longitud** (38 campos): el shim de 32 a 36 rellena `"piso"`, y una tupla que traiga `"merito"` resuelve como `"costo"` con aviso. `HourlyResult` gana `piso_marginal`, `vendedores_no_despachados`, `renta_inframarginal`, `parte_juego` y `retiros_reposo`, que `CAMPOS_REPOSO` y `_anota_reposo` propagan. **La participación no cambia de criterio (D67):** se cuenta en `retiros_reposo`, y en la rama en que todos quedan a pérdida la hora **nombra** a sus retirados, solo por la vía del reposo, para que el almacén y la compuerta los vean; esa rama vuelve antes de fijar el reparto, de modo que no mueve ninguna cifra. `tol_participacion(metodo, piso)` deja la holgura del retiro igual a la de la cobertura del núcleo por la vía del reposo (relativa al piso, unos 7e-7 con piso 700) y conserva la absoluta de 1e-9 en la acoplada y la alternada, cuyos literales se calcularon con ella.
- **`main_simulation.py`.** `--despacho-vendedores {piso,costo,llenado,merito}`, con defecto `piso` y aviso `[D64]` al traducir el alias. `cuenta_retiros_reposo` y `salida_d38`, extraída para probar la línea sin datos reales, hacen que `codigo_de_salida` devuelva 3 ante **cualquier** retiro (D67). `columnas_reposo` escribe los cuatro campos nuevos en todas las vías, y la línea `[D48]` termina con los retiros de la vía por reposo, que deben ser 0, y con la prima descompuesta en renta inframarginal y parte del juego.
- **La compuerta de salida**, `reformateo/documento/scripts/sonda/compuertas_matriz_reposo.py`. Dos identidades nuevas, `cero_retiros` (D67) y `prima_descompuesta` (D69). El volumen descuenta `vendedores_no_despachados`, que pasa a columna exigida, y `estado` falla si un vendedor aparece a la vez en esa lista y en `vendedores_excluidos`. La identidad `vendedores` usa el piso **de cada vendedor** tomado de los flujos, y no el del juego, que es lo que el teorema de cobertura afirma. Un almacén que no sea de la vía por reposo sale con código 2 y lo dice, en vez de fallar identidad por identidad.
- **Y un almacén sin ninguna hora resuelta también sale con 2.** Sin horas resueltas ninguna identidad mira nada y el detector de vía no puede decidir, de modo que la compuerta habría salido **en verde sin comprobar nada**, que es lo peor que puede hacer. El corte va antes del detector y de toda identidad, con el mensaje de que no ha comprobado nada y la pista de mirar `[C-190]`, `[D24]` y `[D38]` en el registro de la corrida. Lo fija `test_un_almacen_sin_ninguna_hora_resuelta_sale_con_2`, que ejerce las dos rutas, la excepción y el código 2 de `main`.
- **El lanzador y MONTAJE.** `matriz_reposo` y `barrido_sigma` pasan `--despacho-vendedores piso`; los mensajes del código 3 citan D67 y dicen qué mirar. `modelo_base/MONTAJE_SERVIDOR.md` lleva el encabezado de D48 a D69, la regla nueva, la orden, la tabla de identidades con las dos nuevas, la lectura de la línea `[D48]` y el retiro como causa del código 3.

### Estado

**Aplicada en el código, sin commit y sin corrida en el servidor.** En la máquina de trabajo, tras la ronda de arreglos de la revisión, **224 pruebas en verde** en 129 (s): 102 de `tests/test_reposo_mercado.py`, 78 de `tests/test_reposo_motor.py` (sin ningún salto, con los literales contra el código base del commit a392cbd) y 44 de `tests/test_compuertas_matriz_reposo.py`. Con el corte por almacén sin horas resueltas, añadido al documentar esta corrección, son **225 y 45**: `tests/test_compuertas_matriz_reposo.py` pasa 45 en 16,98 (s). La prueba dorada en 7 de 7, el preflight filtrado en 6 con 33 fuera, `bash -n` en verde y el seco de las dos acciones del lanzador con código 0, la bandera en los trece casos y un listado idéntico de 5 496 entradas. Nada escrito en `outputs/` ni en `graficas/`. La mecánica de la participación, que ya no muerde, se ejerce sustituyendo el núcleo por uno con el piso mínimo, es decir la regla de D62.

**Las horas reales, comparadas al bit sobre los literales** (huella `fe4488168ced426c`, sin cambio): once horas idénticas con `"piso"` (853, 874, 2120, 3804, 3086, 877, 12, 58, 540, 226 y 4766), por tres motivos comprobados uno a uno, y una que cambia, la 133 de E4, donde vende Udenar a 147,46 (COP/kWh) en vez de Cesmag a 330,15 y el excedente sube de 1 694,38 a 2 460,29 (COP). Con `"costo"` las once dan lo que daba el mérito por b_j; con `"llenado"` cambian la 4766 y la 133 de E4, con la parte del vendedor pasando de −0,262 y −0,441 a +0,512 y +0,216. Las horas 5631 y 5632 de E4 siguen `sin_ganancia` con las tres opciones. Las cifras están en H-91.

La revisión, **sin críticos**, comprobó a mano las nueve cifras de los ejemplos de Fable, los motivos de las once horas idénticas y el teorema de cobertura, que ejerció sobre su población adversaria sin una sola violación; la re-revisión la repitió después del arreglo, sobre **141 136 horas adversarias**, con el mismo resultado y un déficit peor de 7,0e-7 (COP) (`task-5a-rereview.md`). Pidió **dos importantes**. El primero era un defecto reproducible que habría abortado la matriz: la tolerancia del nivel de la caminata y el filtro de polvo del despacho no comparten base, de modo que un grupo de cierre justo en esa frontera dejaba el piso por encima del mayor piso de los que `_comprueba` cuenta como despachados, y la hora moría con un `ValueError` falso (56 de 141 082 horas adversarias, es decir el 0,040 %, y 16 de 120 000 con ofertas logarítmicas y los pisos reales, el 0,013 %). Se arregló comprobando la desigualdad que la cobertura necesita, más que el piso sea el de algún vendedor que puede vender; en 60 000 horas por población, los 54 falsos positivos desaparecen y las que lanzan por «oscila» quedan igual (74 y 142). El segundo era el docstring de la tolerancia, que daba por resuelto el vendedor marginal minúsculo: ahora dice que es una guarda de redondeo, con la pérdida máxima medida de 1e-9 relativo y 2e-9 (kWh) en 40 000 horas, y que el corte del vendedor de polvo está entre 1e-8 y 1e-7 (kWh) con 10 (kWh) de base. Los cinco menores quedaron atendidos: una sola tolerancia de participación por vía, el almacén de otra vía con código 2 y su prueba, la nota de MONTAJE sobre los almacenes del 15 de septiembre, los `AssertionError` de las rutas del núcleo convertidos en fallos en voz alta y M-K con `PARA_EN_FALLO=0`.

**Lo que queda por medir en el servidor:** M-I, los trece casos con el núcleo nuevo comparados con la tabla medida de D2 al 0,1 (kWh); M-J, la descomposición de la prima y el peso del vendedor marginal minúsculo; M-K, las horas de compradores cortos en que el mérito por piso y por b_j despachan distinto; y M-B reformulada, c_j = piso_j frente a b_j (H-89, H-91). El recuento de `c136`, que con el piso marginal pasa de 520 a 1 019 horas en 4 000 sintéticas, se remide allí: la cifra de 451 horas de H-90 es de D62. Las opciones `costo_vendedor` y `piso_juego` de la vía acoplada (D68) no entran aquí y no bloquean la matriz.

## C-200 · La bolsa y el techo de escasez: fechas reales en la caché, ningún mes inventado en el techo y un contrato al bit con el módulo MTE (H-92; informe del módulo del 2026-09-17, defectos 1 a 8)

**2026-09-18 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

El módulo de la plataforma MTE fijó el núcleo sobre 05ed9c3 e importa dos funciones de `data/xm_prices.py`: `apply_creg101066_ceiling`, en cada hora que resuelve, y `get_b_for_real_data`. Su informe del 17 de septiembre documentó ocho defectos, con reproducciones.

1. **La caché de bolsa llevaba fechas fabricadas** y, desde el 30 de julio de 2025, estaba corrida de 1 a 7 días (H-92).
2. **El techo perdía el último mes del horizonte**: el instante final se truncaba a la fecha y `load_creg_ceiling` le restaba un día entero, de modo que un horizonte que acababa el día 1 de un mes antes de la medianoche lanzaba `KeyError`.
3. **El techo inventaba los meses fuera de la tabla**: la interpolación con `limit_direction="both"` propagaba el mes más cercano, sin aviso ni marca. La tesis dependía de ello: su horizonte empieza el 4 de abril de 2025 y la tabla empezaba en julio, de modo que abril, mayo y junio salían con el techo de julio.
4. **El vector b**: un nombre de institución mal escrito caía en silencio al valor por defecto (4a), el relleno no llevaba el ajuste por irradiancia (4b), el docstring nombraba valores que la función no devuelve (4c) y `capacity_kw` se aceptaba y se ignoraba (4d).
5. **El techo no validaba la serie de bolsa**: un NaN pasaba el mínimo.
6. **Una fecha con zona lanzaba `TypeError`**, y nada decía que la fecha sin zona es hora local.
7. **El lector de CSV rellenaba con la mediana** lo que faltara, y un rango del todo fuera devolvía un vector de NaN en vez de `None`.
8. **El recorte medio por mes promediaba sobre todas las horas**, no sobre las recortadas.

### Qué cambia

Van en dos commits, P1 con lo que importa el módulo y P2 con la caché.

**P1, lo que ve el módulo (defectos 2 a 6).**

- `load_creg_ceiling` recibe el final del horizonte **exclusivo y con su hora**, y el último mes es el del último instante anterior a él. Un horizonte vacío es un `ValueError` claro. **Un mes anterior al primero o posterior al último con valor en la tabla es un `ValueError`**; solo se interpolan los huecos interiores (`limit_area="inside"`), sobre el calendario mensual completo de la tabla, de modo que el techo de un mes no depende de la ventana pedida. Con `return_info=True` dice qué meses salieron de su fila y cuáles se interpolaron, y `apply_creg101066_ceiling` lo copia en `diag` (`meses_tabla`, `meses_interpolados`).
- **La tabla `data/precios_escasez_creg.csv` gana abril, mayo y junio de 2025**, con los valores oficiales del API de XM (`PrecEscaInf`, `PrecEsca` y `PrecEscaSup`, entidad Sistema, un valor único en todos los días de cada mes): 928,33, 891,22 y 858,79 (COP/kWh) de techo superior, a dos decimales como el resto de la tabla en P1. Las tres métricas reproducen al redondeo las filas de julio, agosto, septiembre, octubre, diciembre y enero, lo que fija qué columna es cada una. Con estas filas cambian 7 horas de abril y 1 de mayo del horizonte de la tesis, que antes se recortaban a 865,22 (COP/kWh).
- **Se quita el `warnings.filterwarnings("ignore")` que `xm_prices.py` ejecutaba al importarse**, porque callaba todos los avisos del proceso que lo importara, incluido el módulo MTE. Ninguna prueba dependía de él. `main_simulation.py`, `data/xm_data_loader.py`, `data/preprocessing.py` y tres módulos de `analysis/` conservan un filtro igual, de modo que en una corrida de `main_simulation.py` los avisos siguen callados: no se tocaron, porque quedan fuera de esta corrección.
- `apply_creg101066_ceiling` rechaza una serie vacía o con valores no finitos, NaN o infinitos (defecto 5), sin tocar la comprobación del techo de H-50. **Una fecha sin zona sigue siendo hora local de America/Bogota y no se reinterpreta; una con zona se convierte a America/Bogota y se le quita la zona** (defecto 6). Solo se pide techo para los pasos desde la vigencia de la resolución.
- `INVERTER_BY_AGENT` pasa a constante del módulo, y **un nombre que no esté escrito exactamente igual es un `ValueError`**, con la sugerencia si solo cambia la capitalización (4a). Se revisaron todos los llamadores antes de endurecerlo: `main_simulation.py` y los trece casos de la matriz (SINU incluido, que excluye a Udenar antes de nombrar), las sondas y las pruebas pasan los cinco nombres canónicos o un prefijo de ellos; los nombres sintéticos («Agente n», «A1») solo aparecen en ramas que toman b del caso base. El relleno lleva el mismo ajuste que los demás (4b). **El ajuste por irradiancia no cambia**: solo el docstring, que ahora dice {241,07 × 4; 225,00} (4c). `capacity_kw` desaparece y la irradiancia pasa a ir por nombre (4d).
- `modelo_base/empaquetar.py` lleva la tabla de techos al servidor: con la vieja, el código nuevo se niega a correr el horizonte de la tesis.

**P2, la caché (defectos 1, 7 y 8).**

- `download_via_api` pide al API de XM directamente (POST a `servapibi.xm.com.co/hourly`, `PrecBolsNaci`, Sistema), porque pydataxm destruye la fecha. **Lee la fecha del dato, con formato estricto, y nunca la reconstruye por posición**; los bloques son de 28 días sin solape; una fecha repetida se descarta si es idéntica y falla si no; el calendario se exige completo, con 24 horas por día; y no recorta nada. Una petición por segundo como mucho, cada una impresa y anotada en `registro` si se da. Cualquier fallo es un error, y con `use_api` ya no cae a los precios sintéticos.
- `_save_csv` escribe la hora real de cada precio, a un temporal que luego sustituye al fichero. La caché conserva su formato y el redondeo a dos decimales de siempre.
- `get_pi_bolsa` usa la caché canónica **solo si cubre el horizonte entero, por fecha**; si no, descarga ese horizonte a una caché con el horizonte en el nombre, que `.gitignore` excluye. La canónica solo se reescribe a mano, con `python data/xm_prices.py --regenera-cache INICIO FIN --registro FICHERO`.
- `load_xm_prices` no rellena: devuelve `None` si el fichero no tiene ninguna hora del horizonte o lo cubre en parte, y con `estricto=True`, que usan los CSV explícitos, falla nombrando las horas que faltan. Una hora fuera de 1 a 24 o repetida con otro precio es un error. `_adj` falla si la serie es más corta que el horizonte (defecto 7).
- En `diag["by_month"]`, `delta_mean` es el recorte medio de las horas recortadas, y `delta_total` la suma del mes (defecto 8).
- **La caché `data/precios_bolsa_xm_api.csv` se regeneró del API** el 2026-09-18, del 2025-04-04 al 2026-01-31: coincide con la descarga del módulo en las 7 272 horas, con diferencia máxima de 0,00499 (COP/kWh), y sus 2 808 primeras horas son idénticas byte a byte a la vieja (H-92).
- **La tabla de techos pasa a los valores del API de XM a precisión completa**, en los diez meses (abril de 2025 a enero de 2026) y los tres niveles, con la métrica y la fecha de descarga (2026-09-18) en cada fila y la fuente anterior en la nota. Noviembre de 2025 venía redondeado a enteros del boletín de prensa: el techo superior pasa de 829,00 a 829,27159, el intermedio de 659,00 a 658,63049 y el inferior de 332,00 a 331,88241 (COP/kWh). El resto se mueve 0,0064 (COP/kWh) como mucho: el redondeo a dos decimales, salvo el techo intermedio de julio, que estaba truncado y no redondeado. Las pruebas que fijaban los valores redondeados usan ahora los oficiales; las de la tabla de una fila del módulo no cambian, porque esa tabla es del módulo.
- **`data/precios_bolsa_generados.csv` se retira**: nadie lo leía y era una copia de la caché corrida. El CLI de `xm_prices.py` ya no lo escribe, ni en `data/` ni en ninguna parte.

### Estado

**Aplicada en el código, sin commit.** 35 pruebas nuevas en `tests/test_xm_prices_contrato_modulo.py` (P1) y 21 en `tests/test_xm_prices_cache_bolsa.py` (P2), en verde junto con las de precios existentes, `tests/test_creg101066_ceiling.py` y `tests/test_cal17_xm_audit.py`: 78 en total. **El invariante del aviso al módulo está probado al bit contra 05ed9c3, cargado como módulo aparte**: `get_b_for_real_data(5, [...])` y la llamada exacta del módulo, veinticuatro horas desde medianoche con una tabla de una sola fila, el día 1 y el último de julio, noviembre y diciembre de 2025 y de enero y mayo de 2026, con precios bajo el tope, sobre él y en él; y veinticuatro horas desde medianoche con la misma tabla de la tesis en las dos versiones, cada día de abril de 2025 a enero de 2026. Las pruebas del reposo (228), las que leen la bolsa por la sonda (185), las de los escenarios que registran avisos (60) y el preflight filtrado (6) siguen en verde, y la compuerta de CAL-46 da el camino horario idéntico al de HEAD en 683 números, con P1 solo y con P1 y P2. P1 se probó también solo, en un árbol limpio con la caché vieja. El caso sintético de `main_simulation.py`, sin `--data real`, termina con código 0 y no pasa por la caché ni por el API. Nada escrito en `outputs/` ni en `graficas/` salvo los registros de las descargas.

## C-201 · La rama cuantal del reposo: en las horas frágiles el núcleo publica el reposo con μ = 1, y todo lo demás queda al bit (H-93; D71)

**2026-09-19 · tipo: `codigo` · aplicada, con pruebas**

### Qué hacía el código

El núcleo del reposo (C-197, C-199) publicaba en toda hora la forma cerrada, que es el límite μ → 0⁺ de la regularización entrópica: prioridad estricta por precio del lado de los compradores y por piso del lado de los vendedores. En 216 de las 15 363 horas con mercado de la matriz del 17 de septiembre ese límite no es un reposo estable de la dinámica regularizada con μ = 1 (H-93): los compradores en su techo, a menos de unos siete pesos por kilovatio hora del interior, no recibían nada, y M-D daba cuatro valores propios inestables en esas horas. Nada en el resultado de la hora lo decía.

### Qué cambia

- **El núcleo, `core/reposo_mercado.py`.**
  - `resuelve_reposo(..., mu_cuantal=MU_CUANTAL)` es el único argumento nuevo, con nombre y con defecto `MU_CUANTAL = 1.0`. `valida_mu_cuantal` lo exige finito, y rechaza un booleano o un texto. Admite 0, o al menos `MU_CUANTAL_MIN = 1e-6` (ronda 1, M-1): por debajo, la bisección no resuelve la exponencial y la respuesta fallaba en voz alta hasta en horas no frágiles. **Con `mu_cuantal=0.0` el bloque nuevo no corre y la hora es la de antes, al bit.**
  - `_respuesta_cuantal(E, cap, z, mu)` resuelve min(cap_k, e^{(z_k − C)/μ}) con Σ = E por bisección en C, con el corchete del censo, parada en hi − lo ≤ 1e-13·máx(1, |hi|) y tope de 200 vueltas. Después **reparte el resto E − Σ entre las entradas no topadas**, en proporción a su valor, lo que deja la suma a un ulp; sin eso, el residuo de la dinámica en el punto cuantal sería 6e-5 en vez de 7,5e-11. Falla en voz alta si algún valor no es finito, si la suma se aparta de E más de 1e-9 relativo o si alguna entrada pasa su capacidad.
  - **La detección.** En toda hora con mercado se calcula la respuesta cuantal de los dos lados con los precios del reposo cerrado: la de los compradores de `dentro` con sus precios y la de los vendedores que pueden vender con menos su clave de despacho. El apartamiento es la mayor diferencia con el reparto cerrado, relativa a E; **frágil si pasa de `TOL_FRAGIL_REL = 1e-3`**, el umbral del censo. En una hora no frágil no se asigna nada.
  - **El lado comprador.** Con vendedores cortos y dos o más compradores, `_estados_reposo` vuelve a enumerar los estados (T, N, F) con la respuesta cuantal como función de servicio, un argumento nuevo `sirve` cuyo defecto es `_sirve`, y se elige con `_elige_paso3`; sin estado del paso 3, la regla del paso 5 declarada con el servicio cuantal. **El lado vendedor**, con compradores cortos: lo despachado es la respuesta cuantal de los que pueden vender. S, el piso del juego, quién entra y la caminata no se tocan. El estado de precios elegido es el de la forma cerrada en las 216 horas reales; en horas sintéticas adversarias la reenumeración eligió otro en 33 de 1 588.
  - **Con `"costo"`, la rama no está definida cuando despacharía sobre el piso del juego** (ronda 1, I-1). `_sin_despacho_sobre_el_piso` lanza un `ValueError` de D71 si la respuesta cuantal del lado vendedor diera energía, sobre el polvo de redondeo, a un vendedor con el piso por encima del piso del juego. Con `"piso"` y `"llenado"` no puede ocurrir. El mensaje manda comparar por costo con `mu_cuantal=0.0`; antes saltaba D63, con otro nombre.
  - **La clave del lado vendedor es la del despacho**, `_clave_cuantal`: el piso con `"piso"`, que es la de D64 y la del diseño; el costo b_j con `"costo"`; y una clave común con `"llenado"`, cuya respuesta cuantal es el llenado mismo. Así la forma cerrada sigue siendo el límite μ → 0⁺ con las tres opciones.
  - `ReposoHora` gana `regimen_cerrado`, `apartamiento` y `mu_cuantal`, y `REGIMENES` gana `"cuantal"`, su décimo miembro. En las horas no frágiles y sin mercado, `regimen_cerrado` es el régimen.
  - `_comprueba` llama a `_comprueba_cuantal`, que verifica: la coherencia de los campos nuevos; que la suma del reparto y la de lo despachado sean E a 1e-12 relativo en la hora «cuantal»; y **la condición del reposo cuantal**, que π_i − μ·ln q_i sea la misma entre los compradores no topados a 1e-9·máx(1, |π|), y −c_j − μ·ln s_j entre los vendedores. Las identidades de antes siguen valiendo tal cual.
- **El motor, `core/ems_p2p.py`.** `SolverParams.mu_cuantal = 1.0`, validado como `mu_entropia`. **La tupla del trabajador pasa a 41 campos**: el campo 41 es `mu_cuantal`, y una tupla de 40 se rellena con 1,0, el defecto de producción, como D64 rellenó el despacho con `"piso"`. `HourlyResult` gana los tres campos, con valores neutros fuera de la vía por reposo (`""`, 0,0 y 0,0), y `CAMPOS_REPOSO` y `_anota_reposo` los propagan, también por la recursión de la participación.
- **`main_simulation.py`.** `--mu-cuantal` (defecto 1; 0 apaga la rama; entre 0 y 1e-6, error antes de cargar nada), con aviso `[D71]` si se pide sin `--metodo reposo`. La línea `[D48]` de inicio dice la rama y su μ, `columnas_reposo` escribe las tres columnas nuevas en la tabla de horas del almacén y la línea final cuenta las horas «cuantal» con los demás regímenes. `core/almacen.py` no cambia: escribe lo que recibe.
- **Fuera del núcleo.**
  - `censo_fragiles.py` mide sobre la forma cerrada (`mu_cuantal=0.0`); guarda contra el almacén con la forma con que se escribió, que un almacén de la rama declara con su columna `mu_cuantal`; cruza cada hora con el núcleo (`discrepancias`, que debe quedar en cero); y publica la sensibilidad al umbral y a μ.
  - M-D (`medicion_estabilidad.py`) decide «frágil» con la forma cerrada, corre las horas frágiles del lado vendedor con el costo en el piso (`b_vend = piso_j`, D68), publica la mayor parte real fuera de las direcciones neutras y acepta `--mu-cuantal` para analizar el punto cerrado.
  - `preparacion.py` lleva el régimen cerrado y el apartamiento en la referencia, y `tolerancias` decide la rigidez por el régimen cerrado; `corre_mediciones.py` y `veredicto.py` se lo pasan. `selecciona_horas.py` da por buena una hora cuyo almacén traiga el régimen de la forma cerrada, para no descartar las frágiles de un almacén anterior a D71.
  - Los textos de la regla del 2 % en `censo_fragiles.py` y `medicion_mu.py` pasan a diagnóstico: con D71 la decisión es por hora.
- **La compuerta de salida**, `compuertas_matriz_reposo.py`, acepta el régimen «cuantal» y gana la identidad `cuantal`: «cuantal» si y solo si el apartamiento pasa de 1e-3, con dos épsilon del float32 de holgura; un régimen cerrado de mercado y μ > 0 en la hora «cuantal», y el cerrado igual al régimen fuera de ella. Informa sin fallar de las horas «cuantal» con `n_soluciones` distinto de 1 y de las del lado vendedor, y exige las tres columnas nuevas: un almacén anterior a D71 sale con código 2 y lo dice.
- **La comparación de matrices**, `compara_matriz_reposo.py --por-hora`: `horas_distintas` separa las horas con otras entradas, como las de la caché nueva de bolsa (H-92), de las horas con las mismas entradas y el mercado distinto, al bit por defecto, y lista las segundas con los regímenes y la energía que cambia de manos, que mide el lado comprador, ½·Σ_i |Δq|.

### Estado

**Aplicada en el código, sin commit y sin la matriz repetida.** En la máquina de trabajo, con el `.venv` del lock (Python 3.13.7, numpy 2.4.4, scipy 1.17.1, pyarrow 24.0.0):

- **Al bit contra HEAD (f21205e)**, con el núcleo de HEAD cargado aparte. Con la rama apagada, idéntico en las 21 797 horas probadas: 240 literales con sus doce pares de opciones, 6 000 al azar y 15 557 de los almacenes. Con la rama encendida, idéntico en todas las no frágiles.
- **Las 216 horas**, sobre los trece almacenes (`tests/test_rama_cuantal_almacenes.py`, 6 pruebas, que se salta si no están). La forma cerrada reproduce las 15 363 horas de mercado. La rama publica «cuantal» exactamente en las 216 del censo, con el apartamiento del censo al 1e-9 relativo, un solo estado y el de la forma cerrada, y 48,5 (kWh) que cambian de manos (19,53 en E0, 0,82 en K1, 20,51 en SINU). La comparación hora a hora entre una matriz cerrada y una cuantal con las mismas entradas da exactamente las 216, al bit en float32, y también frente al almacén guardado del 17, con 1e-5 (kWh) de holgura por pareja.
- **La hora 12 literal**, con los esperados del diseño: q̃ = (0,97295922; 0,97295922; 4,78605365), P, precios y S al bit los cerrados, parte del vendedor 0,487739 y captura 0,823774.
- **M-D.** La hora 12, estable en el punto cuantal (−0,615, residuo 7,5e-11) e inestable en el cerrado (+6,84); los tres sintéticos del diseño, el del lado vendedor con el costo en el piso. La muestra de 50 horas de E0, 50 de 50 estables y quietas con la rama.
- **Las suites**, en verde tras la ronda 1 de la revisión: `test_reposo_mercado.py` (132), `test_reposo_motor.py` (89), `test_compuertas_matriz_reposo.py` (56), `test_compara_matriz_reposo.py` (26), `test_arnes_consenso.py` (154), `test_dinamica_regularizada.py` y `test_xm_prices_contrato_modulo.py` (250), `test_rama_cuantal_almacenes.py` (6), las dos horas rápidas de `gate_reposo_cero_dinamica.py` y el preflight filtrado (6). La prueba al azar de la cobertura sortea ahora la mitad de los b uniformes entre 220 y 230 (COP/kWh), con lo que habría destapado el defecto de `"costo"`. La comprobación del arnés frente al motor sigue en 30 IDÉNTICO. Nada escrito en `outputs/` ni en `graficas/`.
