...
tres subsecciones, porque es donde el dato cambia de paso y de forma. La
...
tomando el promedio entre si de los datos duplicados. En el horizonte de estudio solo quedan 27 en cada frontera.
...
La regla para determinar que instante de los datos duplicados usar no tiene consecuencia. En los \num{6696} instantes repetidos de las dos fronteras las
...
\begin{figure}[H]
 ...
 \caption[El censo de los instantes repetidos]{El censo de los instantes
 repetidos en las dos fronteras de medición. Arriba, la fracción que
 llevaba ocurrida cada frontera a lo largo del archivo, con la banda del
 horizonte sombreada y el anillo sobre el punto en que se cierra; bajo
 ella, un carril por frontera en el que cada marca es un día con algún
 instante repetido, y las altas son las de dentro del horizonte. Abajo a la
 izquierda, el reparto por institución en escala logarítmica, que es la que
 admite los tres órdenes de magnitud que separan a la UCC del resto. La
 masa se acumula en marzo de 2026, es decir, tres meses después de que el
 horizonte se cierre, y los 27 que caen dentro se reparten en solo ocho
 días, uno de ellos común a las cinco instituciones.
 \fuente{elaboración propia desde \file{MedicionesMTE\_v3/}, columnas de
 fecha y de potencia activa total de los diez medidores que el modelo lee;
 horizonte de \num{6144} horas.}}
...
Solo la segunda supone algo, y ese supuesto no está escrito en ninguna
parte del cargador. La media horaria ignora las muestras que faltan, de
modo que la estimación es lo que la agregación hace por defecto y no un
paso que alguien programara. Se declara aquí porque de otro modo nadie lo
revisaría.

La otra opción es contar solo los minutos medidos y no añadir nada por los
...
criterio y no como parte de la alineación con la normativa. El articulado y su alcance exacto quedan en
el Anexo~\ref{anx:repro}.
...
La serie horaria que sale de la primera etapa trae algo que no puede ser
cierto. Es la demanda ya transportada, ordenada y expresada en horas y en
kilovatios, pero con el valor sin tocar, y en ella la de tres de las cinco
instituciones baja de cero durante varias horas al día. Un edificio no
consume energía negativa. La Figura~\ref{fig:prep-negativa}
...
Udenar encabeza esa columna de la Tabla~\ref{tab:prep-tipos}, y el
...
\begin{detallebox}
El inversor cumple dos papeles dentro del pipeline y se confunden con
facilidad. La generación $G$ que consume el modelo proviene
...
gratuito, y lo que cuesta se puede medir. En
...
La última etapa no altera ningún valor: le pone zona horaria al índice. Es
la más corta de las seis y la que más fácilmente pasa inadvertida, y sin
embargo encierra una suposición de la que dependen dos argumentos
anteriores.
...
horas por detrás, ese máximo aparecería entre las 16 y las 17. La
subsección anterior ya usa esa alineación como comprobación final del
capítulo.
...
respuesta honesta no es que las decisiones sean neutrales: lo que decide
es hacia dónde apuntan.
...
Hay una quinta que apunta al revés, y se nombra aquí en vez de
...
hora. No se elige por su signo: truncar queda desmentido por el contador del
propio equipo y va en contra del criterio del regulador, según la
subsección~\ref{sub:prep-incompleta}. Y el supuesto se comprobó aplicando la
