...
El modelo liquida energía activa. De las setenta y dos magnitudes que
entregan los equipos toma dos, y el Capítulo~\ref{sec:dato} explica por
qué esas dos bastan para simular un intercambio de energía. Queda por
...
La regulación colombiana la factura cuando el consumo reactivo inductivo
...
\begin{table}[H]
    ...
    \bottomrule
    \end{tabular}
    \fuente{elaboración propia sobre \file{MedicionesMTE\_v3/}; cargo a la
    tarifa de transmisión y distribución del nivel de tensión 2.}
\end{table}
...
fila no admite la misma lectura que las demás. Elaboración propia.}
...
Udenar, sobre el circuito principal, el \pct{25,2} de las horas registra
...
el \pct{61} se queda entre cero y \uni{0,5}{kW}, con una mediana de
\uni{0,38}{kW}: es sencillamente un ramal pequeño. El caso de Mariana en el
circuito secundario es de otro orden, con el \pct{90,5} de las horas por
...
La distinción importa para leer la tabla, porque sus dos primeras columnas
...
menor, porque \uni{3495}{kvarh} de los \uni{8851}{kvarh}, es decir el
\pct{40} del exceso facturable, nacen en horas que el recuento porcentual
deja fuera, y son en su mayoría las de exportación de Udenar.

\begin{detallebox}
Conviene precisar qué series son estas, porque el
Capítulo~\ref{sec:prep} somete el dato a diez transformaciones antes de
...
La distancia entre las dos lecturas se puede medir, y resulta ser el
tamaño de lo que cuesta generar. Contra la demanda reconstruida, las horas
...
generación propia incumplen entre el doble y veinticinco veces más de lo
que incumplirían por su consumo real. El fotovoltaico no aumenta su demanda
...
La Figura~\ref{fig:dato-reactiva-series} traza la serie completa de las
diez fronteras, con la línea del umbral sobre cada una, de modo que el
...
\begin{figure}[H]
 ...
 \fuente{elaboración propia sobre \file{MedicionesMTE\_v3/}.}}
 \label{fig:dato-reactiva-series}
\end{figure}

\notafig{La escala se recorta en \num{2,0}, lo que deja fuera el
\pct{0,7} de las horas. Las diez series tienen huecos, entre 103 y 126
horas de las \num{6168} del horizonte, con rachas de hasta dos días; el
Capítulo~\ref{sec:prep} explica cómo se rellenan. Elaboración propia.}
...
La Universidad CESMAG merece una lectura aparte. Su razón mediana diaria
vale unos \num{0,78} en abril y mayo, sube a alrededor de \num{1,0} en
...
sistema que empeora a mitad de horizonte y se queda así es un hecho que
interesa a quien lo opere.
...
allí solo el \pct{3} de las horas registra consumo. Elaboración propia.}

La Figura~\ref{fig:dato-reactiva-tiempo} añade el ciclo del día, que la
serie completa no deja leer. Mariana alcanza su peor momento al mediodía,
con \num{0,95}. Su potencia activa mediana cae de \uni{9,1}{kW} a las
nueve a \uni{6,2}{kW} a las trece, mientras que la reactiva baja solo de
...
\begin{trampabox}
Puesto en dinero, el exceso suma \uni{1877304}{COP} en el circuito
principal y \uni{1215258}{COP} en el secundario, llevado a la tarifa de
...
los dos mecanismos mejor situados en esa frontera}, que es de
\uni{1426142}{COP}. No por ello cambia el ordenamiento. Es un cargo por
...
contra la energía activa \emph{entregada al usuario}, de modo que un
mecanismo que reduzca la energía que cada comprador toma de la red baja
también el umbral y convierte en facturable una parte mayor del reactivo.
El cálculo se hace institución por institución, que es como aplica la
norma, y bajo un supuesto que conviene declarar: cada kilovatio hora
...
potencia. Este modelo no representa la red, y ya se declaró que tampoco
afirma proximidad eléctrica entre los participantes.
...
baja de \uni{1877304}{COP} a \uni{303654}{COP}, casi la sexta parte.
