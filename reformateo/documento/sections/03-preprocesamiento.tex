...
Un valor se retira cuando supera el umbral
...
que combina dos criterios porque ninguno basta por separado. El primero es
una cerca de tipo Tukey con un multiplicador de 5 en vez del 1,5 clásico,
deliberadamente permisivo para que solo retire valores físicamente
inverosímiles. El segundo es un piso situado un \pct{20} por encima del
percentil 99,5, y es el que protege los picos operativos legítimos.

Cuál de los dos manda depende de la forma de la serie. La cerca fija el
umbral en 16 de las 20 series, y el piso en las otras cuatro, todas de
demanda: Mariana y CESMAG en las dos fronteras. El caso de CESMAG mide lo
que está en juego. Sin el piso, su demanda perdería \num{451} horas bajo
M1, que concentran el \pct{20.7} de su energía, y \num{927} bajo M3, que
concentran el \pct{52.5}. Con el piso tiene cero atípicos en M1 y dos en
M3.

Lo que hace peligrosa la cerca en CESMAG no es el rango intercuartílico
pequeño por sí solo. El HUDN tiene el menor de todo el estudio,
\uni{0.318}{kW} bajo M3, y su umbral lo fija la cerca sin que marque nada,
porque su carga es tan plana que nunca se acerca. El mecanismo es la
combinación de una base muy estable con picos operativos raros de gran
amplitud, es decir, un perfil de dos modas: la cerca se calibra sobre la
base y deja fuera el pico legítimo.
...
\begin{figure}[H]
 ...
 \caption[El umbral de atípicos y lo que retira]{A la izquierda, la hora que
 el umbral retira con mayor holgura entre las que quedan aisladas, que es la
 de la Universidad Mariana el 1 de octubre de 2025: entra con
 \uni{39.9}{kW}, excede en un \pct{13.7} el umbral de \uni{35.1}{kW} y sale
 con \uni{28.2}{kW}, interpolada entre las lecturas de las 8 y las 10 horas.
 A la derecha, el criterio en las diez series, con el eje en múltiplos del
 percentil 99,5 de cada serie: la barra es la mitad central de las lecturas,
 la línea llega hasta el máximo observado, el círculo marca la cerca de
 Tukey y el rombo el umbral que se aplica. El piso del percentil es la
 vertical en 1,2, común a las diez filas, de modo que manda el piso
 justamente en las filas cuya cerca cae a su izquierda, que son las de rango
 intercuartílico más estrecho.
 \fuente{elaboración propia desde el caché de estados intermedios del
 preprocesamiento; criterio de limpieza declarado en
 \file{data/xm\_data\_loader.py}.}}
 \label{fig:prep-umbral}
\end{figure}

\notafig{La distancia entre la masa de la serie y su umbral es lo que
acredita el criterio, y solo en cuatro de las 20 series, todas de demanda,
hace falta el piso del percentil para conservarla. Bajo M3 las cifras del
caso son equivalentes, con una lectura de \uni{10.3}{kW} sobre un umbral de
\uni{9.1}{kW}. Elaboración propia desde el caché de estados intermedios.}

\subsection{La escalera de los huecos}
\label{sub:prep-huecos}

Lo que queda como faltante, sea porque el equipo no registró o porque el
umbral acaba de retirarlo, pasa por una cascada de tres tratamientos. La
cascada no clasifica los huecos por su longitud: se aplica hora a hora
dentro de cada uno, de modo que un mismo hueco puede recibir los tres.
...
horas y después hacia atrás otras 24. El valor que se repite no es el que
cabría esperar: hacia adelante se propaga la tercera hora interpolada, que
es un valor fabricado, y no la última lectura del equipo. Y se repite un
escalar, de modo que la serie queda plana durante todo el tramo en vez de
recuperar el ciclo diario.

Entre los tres, la cascada cierra cualquier hueco de hasta 51 horas. Lo
que sobreviviera se imputaría con cero, pero ese cuarto tratamiento no
llega a actuar en este horizonte: el hueco más largo mide 49 horas, en la
demanda de la UCC bajo M3.

La cascada toca \num{808} horas repartidas entre las 20 series. De ellas,
\num{11} son los atípicos que retira el umbral, \num{233} quedan
interpoladas y \num{575}, el \pct{71.2}, se rellenan repitiendo un valor.
...
\begin{figure}[H]
 \centering
 \includegraphics[width=0.95\textwidth]{figuras/f3_06_escalera_huecos_m1.png}
 \caption[La escalera de los huecos]{Tres huecos reales de la demanda del
 Hospital, con el eje horizontal en horas desde el inicio de cada uno, que
 es la magnitud de la que depende el tratamiento. El de tres horas lo
 cierra entero la interpolación. El de 13 sale con sus tres primeras horas
 interpoladas y las diez restantes arrastradas, y la meseta no se queda en
 la última lectura observada, \uni{9.9}{kW}, sino en la tercera hora
 interpolada, \uni{10.1}{kW}, es decir, en un valor que ninguna hora
 observada tuvo. El de 42, el más largo de esta frontera, añade el arrastre
 hacia atrás desde la lectura que reabre la serie. Abajo, cuántas horas
 trató la limpieza en cada institución y por qué mecanismo.
 \fuente{elaboración propia desde el caché de estados intermedios del
 preprocesamiento; cascada de limpieza declarada en
 \file{data/xm\_data\_loader.py}.}}
 \label{fig:prep-huecos}
\end{figure}

\notafig{El arrastre es el \pct{71.2} de todo lo que la limpieza trata, de
modo que la mayor parte de lo rellenado es una meseta y no una rampa. El
relleno con cero no aparece en la figura porque no ocurre: no es una
decisión que se tome, sino un recurso que nunca hizo falta. Bajo M3 el
peldaño mayor es el hueco de 49 horas de la Universidad Cooperativa de
Colombia, que tampoco alcanza las 51 de la cascada. Elaboración propia
