...
**Corregido por H-90 (2026-09-17).** Que el término de competencia «no entra
en el gradiente, en ninguna forma» es cierto para la derivada del bienestar
propio de cada comprador, que es lo que mide la compuerta. Pero la ec. 23 del
documento extenso (H-61) deriva la **suma** de los bienestares, y en esa suma
el precio de un comprador aparece en la competencia de los demás: su derivada
da −(Σ_{k≠i} β_k)·q_i, la forma del código de la autora. La aptitud de ese
código es, término a término, la derivada del lagrangiano de la suma de
bienestares con su restricción. Quedan retiradas, por tanto, cuatro frases de
esta sección, todas para la forma del código:

- que la competencia y la señal de restricción no son el gradiente de nada
  («no es el gradiente de nada», y «tampoco»)
  (el diagrama de «Lo medido»);
- que el término de competencia aparece «restado a mano, no derivado»;
- que la dinámica «le añade dos términos que no son gradientes»;
- y que una función de bienestar que case con la dinámica «no correspondería a
  ninguna fuente»: la hay, el lagrangiano de la suma de los bienestares del
  documento extenso.

La forma agregada que usa esta traducción (CAL-49), que es la que midió esta
sección, es la del código sin el factor (I − 1) (síntesis §2.1, punto 10), de modo que
para ella la derivada de la suma vale salvo ese factor cuando las β son
iguales. Se sostiene lo medido (el término
de pago es el gradiente del documento extenso y no el de la versión
arbitrada, con el factor de 7 778) y no cambia ninguna cifra. El enunciado de
la compuerta se corrigió en el mismo sentido, sin tocar sus cálculos.
...
## H-82 · El oráculo del Anexo 4 confirma que C1 liquida el crédito como dice la norma

**Estado: registrado el 2026-09-14, subproyecto 2, paso 1 (D19, D32, D33). Comprobación local antes de las trece corridas del servidor.**

Se construyó un oráculo (`reformateo/documento/scripts/sonda/oraculo_anexo4.py`, función `liquida_mes`) que recalcula la liquidación del Anexo 4 de la Resolución CREG 101 072 con aritmética propia, escrita en un bucle explícito directamente desde el texto de la norma, sin llamar a `reparto_anexo4`, `deduccion_art25` ni `tramo_permuta` de `core/opciones_externas.py`. Se comparó, institución por institución y mes por mes, contra lo que liquida el motor (`run_c1_creg174`) sobre los MISMOS datos de generación y demanda de la frontera M1: cinco instituciones por nueve meses, 45 filas institución-mes por pasada (`tests/test_oraculo_anexo4.py` deja la comprobación como compuerta con los dos factores, uno y siete, de modo que el numeral 2 y el tramo del exceso con datos reales también quedan en ella). Se corrió dos veces, con la generación por uno y por siete (`data.escalado.lee_factor`), para ejercer los dos numerales del artículo 25 por capacidad: hasta 100 kW (numeral 1, solo el componente de comercializar Cv) y de 100 kW a 1 MW (numeral 2, Cv+T+D+PR+R).

**Factor 1 (numeral 1, capacidad 17,55 kW por planta; `outputs/run_2026-09-14_oraculo_x1.log`).** Ninguno de los 45 meses-institución alcanza la hora de corte hx: la exportación acumulada del mes nunca iguala la importación total, de modo que todo el excedente queda como crédito y el tramo del exceso a bolsa no se ejerce en la base. Diferencia máxima entre el oráculo y el motor: 2,3e-10 (COP) de dinero, que es ruido de punto flotante por el orden de suma, y cero (kWh) de energía agregada por institución (auto, crédito y exceso, cada una contra el total que ya trae el motor por agente). La consola la imprime redondeada como 0,000000; la cifra citada es la del CSV.

**Factor 7 (numeral 2, capacidad 122,85 kW por planta; `outputs/run_2026-09-14_oraculo_x7.log`).** 39 de los 45 meses-institución alcanzan la hora de corte, tanto en el oráculo como en el motor y en la MISMA hora en las 45 filas (ningún caso necesitó el matiz del empate exacto: el motor corta en la primera hora con exceso positivo y la norma dice «iguala o sobrepasa», pero las dos coinciden al bit aquí). Diferencia máxima: 1,9e-9 (COP) de dinero, de nuevo ruido de punto flotante, y cero (kWh) de energía. Es decir, con el numeral 2 y el tramo del exceso a bolsa realmente ejercidos, el motor sigue liquidando exactamente lo que dice la norma.

Los seis meses-institución de la pasada por siete que tampoco cortan son todos de UCC (abril, mayo, agosto, septiembre, octubre y noviembre de 2025) y no son un defecto: es la misma regla de la base, aplicada a una institución cuya exportación mensual no alcanza a su importación mensual ni siquiera con la generación escalada. Las dos corridas dejan `outputs/oraculo_anexo4/oraculo_anexo4_x1.csv` y `..._x7.csv`, con energías, hora de corte del oráculo y del motor, dinero de cada tramo del oráculo, dinero del motor y su diferencia, fila por institución y mes. Todavía no hay factura real (D33); cuando llegue se añade su columna a la comparación.

---

## H-83 · La sonda de H-79 en el servidor: con la generación actual el reparto depende del horizonte del acoplado; con la generación por siete, no

**Estado: registrado el 2026-09-14, subproyecto 2 (D25, D26, D35). Medido en el servidor antes de la matriz de trece corridas.**

La sonda de H-79 (`reformateo/documento/scripts/sonda/reparto_vs_integrador.py`) resolvió veinte horas con mercado en E0 (generación por uno) y veinte en E4 (generación por siete), cada una con tres variantes de la vía acoplada: la de producción (tolerancias relativa y absoluta de 1e-6, horizonte de 0,05), la tolerancia relativa apretada a 1e-7 con la absoluta en 1e-6 (H-51), y el horizonte doble (0,1). Los criterios se fijaron antes de medir: 1 punto porcentual de la tajada del vendedor y 0,1 % del excedente.

| Caso | Variante | Excedente | Cambio del excedente (%) | Tajada del vendedor (%) | Cambio de la tajada (puntos) | Precios en cota (%) |
|---|---|---|---|---|---|---|
| E0 | producción | 5979,725 | 0,00 | 62,87 | 0,00 | 0,0 |
| E0 | relativa 1e-7 | 5979,716 | 0,00 | 62,87 | 0,00 | 0,0 |
| E0 | horizonte doble | 6187,226 | 3,47 | 59,01 | −3,86 | 0,0 |
| E4 | producción | 98042,535 | 0,00 | 12,940 | 0,000 | 19,048 |
| E4 | relativa 1e-7 | 98042,535 | 0,00 | 12,940 | 0,000 | 19,048 |
| E4 | horizonte doble | 98042,535 | 0,00 | 12,798 | −0,142 | 19,048 |

**En E0 el reparto y el excedente dependen del horizonte.** Doblarlo baja la tajada del vendedor casi cuatro puntos y sube el excedente un 3,5 %, muy por encima de los dos criterios; apretar la tolerancia no mueve nada. Es decir, con la generación actual una parte de las horas no ha llegado al estacionario en el horizonte de producción, y cuánto se integra decide cuánto se lleva el vendedor. **En E4 no depende de nada:** con la generación por siete el excedente empuja los precios hacia la cota (19 % de los casos) y el equilibrio llega antes; la tajada del vendedor es de apenas 12,9 %. La señal local de una sola hora a ×7 (H-79: la tolerancia movía el excedente 0,40 %) no se confirma con veinte horas.

**Consecuencia para la matriz.** La unión de los dos veredictos es solo la palanca del horizonte: las trece corridas llevan `--horizonte-max-acoplado 0.4`, es decir la parada por estacionario de D26 con el presupuesto de evaluaciones de D36, y no llevan la tolerancia relativa de D35. Las cifras de reparto calculadas con el horizonte fijo de producción en casos como E0 cargan una dependencia del orden de cuatro puntos, que la matriz corrige.

**Costo medido por variante.** La tolerancia relativa cuesta prácticamente lo mismo que producción (por ejemplo 44,0 (s) frente a 43,8 (s)); el horizonte doble, entre 2,2 y 2,8 veces más, en línea con el factor de 2,56 con que se calibró el presupuesto. E0 tardó 29 (min) en un solo proceso, que es el diseño de la sonda. El detalle hora a hora está en `SALIDAS_SERVIDOR/sonda79/sonda79_E0.csv` y `sonda79_E4.csv`, y los veredictos en `veredicto_E0.json` y `veredicto_E4.json`.
...
**Estado: registrado el 2026-09-14, subproyecto 2. Detectado por la sonda de H-79 en el servidor, antes de la matriz.**

La hora 4184 con la generación por siete, que en la máquina de trabajo es la hora con que se calibró el presupuesto de D36, falló en el servidor. Se reprodujo aislada, con la misma receta de la calibración (vía acoplada, tolerancias de 1e-6, horizonte de 0,05, 150 puntos):

| | Servidor | Máquina de trabajo |
|---|---|---|
| Python, numpy, scipy | 3.10.12, 2.2.6, 1.15.3 | 3.13.7, 2.4.4, 1.17.1 |
| Entradas de la hora (vendedores, compradores, excedentes y déficits netos, techo, piso) | idénticas | idénticas |
| Resultado | falla en t ≈ 1,03e-4 tras 15 775 evaluaciones: el paso se hace tan pequeño que «t + h = t», aparece un NaN y LSODA termina con «Unexpected istate» | termina en 141 (s) con 1 292 398 evaluaciones y llega al estacionario |

Es decir, con datos idénticos el resultado depende de la versión de scipy (y de numpy, que evalúa el lado derecho de la ecuación): la implementación de LSODA cambió entre esas versiones. Es el mismo aviso de LSODA que dejan en el servidor los barridos de `test_analysis_numeral2` por la vía alternada. En la máquina de trabajo no se ve porque en Windows el Fortran escribe fuera del fichero redirigido.

**Consecuencia.** Con el entorno viejo, la matriz habría dejado sin mercado una parte de las horas de mucho excedente (una de veintiuna en la muestra de E4), y sus cifras no se habrían podido reproducir en la máquina de trabajo. `requirements.txt` dejaba las versiones libres, de modo que cada máquina instalaba lo que su Python admitía (scipy 1.16 en adelante pide Python 3.11 o superior).

**Decisión.** `requirements-lock.txt` fija las versiones exactas de la máquina de trabajo, con Python 3.13, y el servidor corre con ese mismo entorno. Antes de la matriz se repiten, con el entorno nuevo, las compuertas, la hora 4184 aislada y la sonda de H-79, porque su veredicto (H-83) se midió con scipy 1.15.3.

**Con el entorno igualado, la hora sigue sin comportarse igual.** En el servidor, ya con Python 3.13.7, numpy 2.4.4 y scipy 1.17.1 (instalados con `uv`; el entorno viejo queda en `.venv_py310`), la misma hora con las mismas entradas ya no falla al segundo, pero tampoco termina: pasó de 15 (min) al 100 % de un núcleo y se detuvo a mano, frente a los 141 (s) de la máquina de trabajo. Es decir, con versiones idénticas la diferencia que queda es la plataforma (Linux frente a Windows, y sus bibliotecas matemáticas); no está medido cuál pieza. La hora es un caso límite: en el tramo donde antes aparecía el NaN, el sistema es tan rígido que una diferencia en el último decimal decide si el integrador pasa, se arrastra o se rompe. En la matriz una hora así vence el plazo de 15 (min) y queda anotada en `[D24]`; si pasan del 1 % de las horas de mercado, D38 detiene la cadena. Queda por medir cuántas horas se comportan así en el servidor.

**Censo de una semana de E4 en el servidor (22 al 28 de septiembre, elegida porque contiene la hora 4184; entorno nuevo, palanca del horizonte, 31 procesos; `modelo_base/logs/censo_e4_semana.log`).** De 168 horas, solo 35 tienen mercado: con la generación por siete, de día todos venden y de noche todos compran. Una hora venció el plazo (la 80 de la semana, es decir la 4184) y otra no terminó su primera vuelta del integrador (la 104, el 26 de septiembre a las 8:00, anotada en `[D37]`). La parada por estacionario se comportó como se diseñó: 27 horas pararon por estacionario, 2 por tope y 4 por presupuesto. La corrida salió con el código 3 de D38: una de 35 horas de mercado vencida, es decir 2,86 %, por encima del 1 %. Si la semana fuera representativa, E4 completo tendría del orden de 1280 horas de mercado y unas 36 vencidas, de modo que la matriz se detendría en E4; la semana está sesgada a propósito, así que la tasa real queda por medir.

**Hipótesis de la causa, por comprobar.** La máquina de trabajo es un AMD Ryzen 5 3600, sin AVX-512: numpy usa hasta el nivel `X86_V3` (AVX2) y OpenBLAS rutinas de tipo Haswell. El Xeon Platinum 8452Y del servidor tiene AVX-512, y con él numpy y OpenBLAS eligen otras rutinas para `exp`, `log` y las sumas, que redondean distinto en el último decimal. Si es eso, forzar en el servidor las rutinas de la máquina de trabajo (`NPY_DISABLE_CPU_FEATURES` y `OPENBLAS_CORETYPE`) debe reproducir la hora 4184 con el mismo número de evaluaciones del integrador (1 292 398).

**La hipótesis no se confirma.** Con `NPY_DISABLE_CPU_FEATURES="X86_V4 AVX512_ICL AVX512_SPR"` (numpy queda en `X86_V3`, como la máquina de trabajo) y `OPENBLAS_CORETYPE=Haswell`, la hora siguió sin terminar y el tope de 400 (s) la cortó. Además, leído el lado derecho del sistema acoplado (`core/coupled_ode_convergence.py`, `_rhs`), no usa `exp` ni `log`: solo sumas, productos y divisiones, de modo que lo único que puede diferir entre máquinas es el orden de las sumas y la contracción en FMA que haga cada compilador. Y el sistema tiene términos que crecen sin cota (los multiplicadores `lam_ub` y `bet_ub`, proporcionales a sí mismos, y el replicador de `P`, cuadrático), capaces de una explosión en tiempo finito, que es lo que dice el aviso «t + h = t» cerca de t ≈ 1,03e-4. La prueba que decide es de sensibilidad en una sola máquina: mover las entradas un ulp y ver si la hora pasa o explota.

**Causa raíz: la hora está mal condicionada, no la plataforma.** Pruebas de 2026-09-14 (`outputs/run_2026-09-14_perturba_hora_local.log`, `..._anatomia_explosion.log`, `..._disparo_base.log`, `..._disparo_gn0mas.log`; en el servidor, `/tmp/perturba_servidor.log`):

- La huella de las entradas es la misma en las dos máquinas (`678c7a72873ca47b`): los datos son idénticos bit a bit.
- En la misma máquina el resultado es determinista: la hora base dos veces da exactamente 1 292 398 evaluaciones.
- Moviendo una sola entrada un ulp, en la máquina de trabajo la hora explota en seis de las siete perturbaciones que se pudieron leer (NaN hacia la evaluación 16 400 a 17 000, t ≈ 1,06e-4, P hasta 1e149); una de ellas incluso termina con el integrador diciendo «éxito» y el estado entero en NaN. En el servidor, la misma perturbación de `dn[0]` termina bien: el mismo experimento cae de un lado o del otro según la máquina.
- **El mecanismo.** El comprador 0 tiene un déficit de 0,12 (kWh) y el arranque le asigna 7,77 (kWh), 64 veces su necesidad. Su multiplicador de demanda crece de 0,1 a 3e4 en un microsegundo (`VEL_GRAD` = 1e6, por el factor 10 del bloque de vendedores) y su filtro llega a 3e7, de modo que la fuerza sobre la columna 0 de P es del orden de 1e7 y la lleva a cero en decenas de nanosegundos. La dinámica de replicador (dP = P·(F − F̄)) conserva P ≥ 0 solo en aritmética exacta: el integrador cruza el cero. **Incluso la corrida base que termina bien tiene P negativa en 1 291 155 de sus 1 292 398 evaluaciones** (del orden de −1e-23) y una excursión hasta −0,885 (kWh) en t ≈ 1,06e-4; se recupera. En la perturbada, la misma excursión no se recupera: con P negativa el replicador se alimenta a sí mismo y explota en tiempo finito. Que se recupere o no lo decide el último decimal.

Es decir, lo que falla es que el solucionador acoplado no respeta P ≥ 0 dentro de la dinámica: el precio sí se proyecta a su banda en cada evaluación (`_rhs`, líneas 427 a 429 desde C-192, «matching solve_buyers», que recorta después de cada paso), pero la oferta P solo se recorta a cero a la salida, después de integrar. Las horas afectadas son las que tienen un comprador con déficit muy pequeño frente a su asignación inicial, justo las de mucho excedente.

**Tabla completa del servidor (tope de 900 (s) por corrida).** La base, dos veces, explota idéntica (NaN en la evaluación 15 773, t ≈ 1,034e-4); de las diez perturbaciones, tres terminan y siete explotan (con las dos bases, nueve de doce). En las dos máquinas la hora explota casi siempre: sobrevivir es la excepción, y la base de la máquina de trabajo cae de ese lado. Además, con scipy 1.17 el integrador no se detiene al aparecer el NaN: siguió unas catorce millones de evaluaciones en 15 (min) sin avanzar en el tiempo, de modo que en la matriz cada hora así ocupa un trabajador hasta que la corta el plazo.

**Experimento del arreglo (`outputs/run_2026-09-14_experimento_proyeccion.log`; el motor no se tocó: una copia en memoria del acoplado con una sola línea cambiada, la lectura de P en `_rhs` con un piso de 1e-10, el mismo del arranque).**

| Corrida | Estado | Evaluaciones | Precio de equilibrio | Lo que recibe cada comprador (kWh) |
|---|---|---|---|---|
| base original (la que en esta máquina sobrevive) | éxito | 1 292 398 | 711,134 · 456,569 · 254,566 | **0,2319** · 9,638 · 13,5585 |
| base con piso | éxito | 1 155 804 | 711,134 · 456,569 · 254,566 | **0,1218** · 9,638 · 13,5585 |
...
**Corregido por H-87 (2026-09-16).** Dos afirmaciones de esta sección son falsas y quedan retiradas: que la hora 109 «converge» a dar todo el excedente a un solo comprador (a t ≤ 2 era una estancia en un punto de silla, del que sale hacia t = 12), y que «el transitorio se ve más igualitario que el equilibrio» (es al revés: el único punto de reposo es el reparto igualitario, y las instantáneas largas son esquinas). Lo demás se sostiene, con el matiz de H-87 sobre el precio medio.

**Matizado por H-90 (2026-09-17).** Que «el único punto de reposo es el reparto igualitario» vale solo con todos los compradores interiores (y en el reparto con topados); con compradores excluidos o cuando la suma de precios no cabe sobre el piso, el reposo es otro (ver la nota al final de H-87).

---

## H-87 · El reparto de un vendedor entre varios compradores no converge: oscila alrededor de un reposo igualitario, y el precio medio guardado es un invariante

**Estado: registrado el 2026-09-16, a partir de una sonda analítica local (guiones en el borrador de la sesión, `scratchpad/equilibrio/`) sobre la campaña de convergencia del servidor (`SALIDAS_SERVIDOR/entrega_convergencia_2026-09-16/`).**

La campaña de convergencia mostró que ningún horizonte asequible fija el reparto por institución, y que el problema solo aparece en horas con **un vendedor y cuatro compradores**. La sonda derivó las condiciones de estacionariedad del lado derecho del acoplado (`core/coupled_ode_convergence.py`, `_rhs`), las contrastó con `Documentos/copy/JoinFinal.m`, y verificó con números lo que afirma. Se marca lo demostrado por derivación y lo verificado con números; lo conjeturado se dice.

**1. El sistema completo no tiene punto fijo, y eso es del modelo.** Los multiplicadores de la restricción que muerde llevan un «+1000» en su dinámica (igual en `JoinFinal.m`), de modo que crecen sin límite: en la hora 109, λ = 1e4·t exactamente. La estacionariedad solo existe en la proyección sobre el reparto P y los precios π, donde los filtros hacen de precios sombra.

**2. Para un vendedor, el reparto tiene un único punto de reposo: el llenado por niveles.** Cada comprador recibe min(D_i, L), con L tal que la suma da la energía E del vendedor; es decir, el mismo reparto para todos, salvo los compradores que necesitan menos, que reciben su déficit. Los compradores topados quedan en su techo de precio y los libres en un precio común. **Es un cero del lado derecho del motor**: |dP| ≤ 7e-10 y |dπ| ≤ 2e-11 en las horas 109 y 130. Techo, piso, generación límite, λ, θ, η y b no mueven ese reparto: solo mueven precios y velocidades.

**3. Ese reposo es un foco débilmente inestable, y la dinámica oscila a su alrededor sin llegar nunca.** En la hora 109 los autovalores del sistema reducido son 5,21e-4 ± 0,9167i y 1,52e-4 ± 0,4950i, con periodos de 6,85 y 12,7 unidades de tiempo; una función de tipo Lyapunov crece en toda trayectoria que no esté en el reposo (demostrado sin compradores topados). Con el integrador de producción, arrancado en el reposo con una perturbación del 10 %, la amplitud se mantiene entre 0,036 y 0,041 (kWh) durante 42 unidades: ni converge ni crece de forma medible. **El promedio temporal sí coincide con el reposo**: en la hora 109, sobre t ∈ [11,5; 58,3], el reparto medio es 0,304 · 0,299 · 0,305 · 0,302 (kWh) frente a 0,302 del reposo, y los precios medios 728,39 · 728,47 · 728,39 · 728,45 frente a 728,42 (COP/kWh).

**4. Las esquinas (todo a un comprador) son puntos de silla, no equilibrios.** La trayectoria pasa temporadas largas en ellas. Hora 109 integrada hasta t = 80 (verificado sobre la trayectoria guardada; compradores Mariana · UCC · HUDN · Cesmag):

| t | Reparto (kWh) | Suma de precios (COP/kWh) |
|---|---|---|
| 2 | 0 · 0 · 0 · 1,209 | 2 913,699 |
| 10 | 0 · 0 · 0 · 1,209 | 2 913,699 |
| 12 | 0,509 · 0,164 · 0,469 · 0,066 | 2 913,699 |
| 30 | 0,531 · 0,177 · 0,501 · 0 | 2 913,699 |
| 47 | 0,002 · 0,020 · 0,002 · 1,185 | 2 913,699 |
| 58 | 0 · 0,006 · 0 · 1,202 | 2 913,699 |
| 70 | 0,568 · 0,003 · 0,638 · 0 | 2 913,699 |

En otras horas la estancia en la primera esquina dura del orden de 13/E unidades de tiempo (entre 4,7 y 17,5 en las medidas); la hora 152 de K1 recorre las cuatro esquinas en fila. **Los residuos del reparto de 1e-8 a 1e-16 que la campaña vio a horizonte 1,6 en las horas 83, 109, 152, 157 y 158 son estancias en la silla, no convergencia**, y el criterio de D47 para justo ahí.

**5. Por qué Cesmag gana las instantáneas.** El arranque de precios proporcional a la banda (CAL-47, H-32, H-45) pone a Cesmag unos 37 (COP/kWh) por encima de los otros tres, cuya banda es la misma; su participación crece deprisa y a t ≈ 0,4 ya se lleva min(D_Cesmag, E). Con el arranque a partes iguales, los tres de banda común arrancan idénticos y siguen idénticos para siempre, porque la dinámica no ve el déficit salvo como tope: es la degeneración de la hora 152 descrita en H-86.

**6. El precio medio guardado es un invariante, no un resultado del mercado.** La suma de los precios de los compradores se conserva por construcción desde que el jugador virtual se clava en el techo mayor (hacia t ≈ 1e-3), y la media que guarda el almacén es esa suma entre el número de compradores. Depende solo de las bandas y del número de compradores: **728,42 (COP/kWh) en las 26 horas de un vendedor y cuatro compradores de las seis configuraciones de la campaña** (verificado, entre 728,422 y 728,425), cualquiera sea el reparto. Con dos vendedores y tres compradores sale 725,99; con tres y dos, 719,58. **Es decir, la media aritmética de los precios por comprador no se puede citar como precio del mercado**; si hace falta un precio, es el ponderado por la energía. El volumen transado sí es un resultado firme (lo fuerza la condición de que se venda todo el lado corto). **El excedente total no es invariante**: en las horas de un vendedor de la semana se mueve entre un 7 % y un 31 % con el reparto, porque la banda de Cesmag (84,5 (COP/kWh)) es 2,2 veces la de los otros tres (38,4).

**7. Lo que es del modelo y lo que es de la traducción.** Del modelo: el replicador del vendedor, la aptitud del comprador que baja con lo que recibe, los filtros con «+1000» y la conservación de la suma de precios. De la traducción: el peso de barrera del jugador virtual (H-46) es lo que lo congela y conserva esa suma; con el peso de `JoinFinal.m` los cuatro precios caen al piso ya a t = 0,05 y el reparto queda congelado en el arranque, es decir un continuo de repartos. También son de la traducción el arranque de precios por banda, el techo por comprador, el piso de 1e-10 en P, la proyección en las cotas y el arranque factible.

**8. El reposo se calcula directo.** Para un vendedor, en forma cerrada: ordenar los déficits, topar a quien necesita menos que el nivel, repartir el resto por igual. Cuesta microsegundos por hora, frente a entre 1e4 y 3e7 evaluaciones del integrador. Para varios vendedores: los totales por comprador salen del mismo llenado por niveles, pero **el reparto por pareja es un politopo de transporte y no es único**; el motor conserva las proporciones del arranque factible, que equivale a P_ji = S_i·S_j/E. No conviene un buscador de raíces sobre el lado derecho: el sistema completo no tiene raíz, y las condiciones reducidas tienen raíces espurias en cada cota de precio.

**Lo que queda abierto.** (a) En las horas con compradores topados (130 a 132 de E0), que el promedio temporal coincida con el llenado por niveles está conjeturado, no verificado. (b) Horas de varios vendedores con dos o más compradores libres: no aparecen en la semana medida. (c) La forma de competencia `matrix` de `JoinFinal.m` invierte el signo de la aptitud y probablemente vuelve atractoras las esquinas (conjetura). (d) **El precio por comprador con que se liquida no lo fija el modelo** (el nivel de los libres se hereda del arranque, y el topado paga su techo con ahorro cero): hace falta una regla de precio declarada, y es decisión del autor.

**Consecuencia.** Ningún horizonte ni criterio de parada fija el reparto por institución, porque la dinámica no tiene a dónde llegar: oscila. Repetir la matriz con cualquier horizonte no resuelve las cifras por institución. La salida es declarar el reparto del reposo (llenado por niveles), que es el único punto estacionario y el promedio temporal de la dinámica, y declarar una regla de precio para la liquidación.

**Corregido por H-90 (2026-09-17).** Tres afirmaciones de esta sección quedan acotadas o retiradas, y con ellas la frase heredada por la nota de H-86. Lo demás se sostiene; las horas de esta sección van en la numeración de la semana del 5 de mayo (H-90).

- **§2 y «Consecuencia»: el llenado por niveles no es el único reposo con un vendedor.** Lo es con todos los compradores interiores, y el reparto lo es también con topados. Con compradores excluidos el reposo es otro: en la hora 3804 de la matriz la dinámica converge a dar toda la energía a Cesmag a 725,48 (COP/kWh), con los tres compradores de techo 711,13 (COP/kWh) en su techo y sin energía, donde el llenado daría 1,215 (kWh) a cada uno (`evaluador2-report.md` §3.1). Y en el régimen de la suma que no cabe sobre el piso la dinámica admite varios reposos (síntesis §2.3, punto 3; `evaluador2-report.md` §3.1): se aplica la regla declarada de D53, pendiente de M-C. La salida de «Consecuencia» pasa a ser el reposo por regímenes de H-90, con el presupuesto de precios declarado (D50).
- **§3: el promedio temporal no está demostrado igual al reposo.** Que coincida en la ventana [11,5; 58,3] depende de la ventana: el promedio acumulado desde t = 2 se aparta del reposo el 0,7 %, el 10,7 %, el 1,2 % y el 5,3 % de E a T = 40, 60, 90 y 108 (`evaluador2-report.md` §4.2). El reposo se publica en forma cerrada, no como promedio de la dinámica.
- **§8: la forma cerrada «ordenar, topar y repartir» queda sustituida** por los regímenes de H-90 §6 (prioridad por precio, grupo marginal, topados, excluidos, D53, compradores cortos y un comprador). El emparejamiento por pareja, que aquí queda indeterminado, lo fija la exploración entrópica en el de rango uno.

El punto (a) de «Lo que queda abierto» se responde en parte en la síntesis: con topados, el reposo tiene a los topados en su techo y a los libres en un precio común, y el sistema reducido, arrancado del estado del motor en t = 1,6, se acerca a él de forma monótona (síntesis §2.1, punto 4). Falta confirmarlo con el motor: en la hora 130 de la semana los libres no se estabilizan (`evaluador2-report.md` §4.3).

---

## H-88 · El precio del juego de Chacón es el mínimo que cubre el costo del vendedor, no un precio de escasez; con las tarifas colombianas ese mínimo es el piso

**Estado: registrado el 2026-09-16, a partir de una revisión del artículo base, de su código y de su propio caso (guiones en el borrador de la sesión, `scratchpad/revision_chacon/`), motivada por la pregunta del autor de si el juego debería subir el precio cuando hay más necesidad.**

**Lo medido sobre la matriz.** En horas de E0 con un vendedor, la parte del ahorro que se lleva el vendedor es 0,64 a 0,68 sea la necesidad de los compradores menos de 2 veces o más de 50 veces la oferta (correlación de Spearman 0,05). En el experimento natural E0 frente a K1 (misma oferta, demanda de todos los compradores duplicada; 297 horas de un vendedor en los dos; escasez mediana de 3,6 a 11,5), el precio medio guardado sale idéntico (730,79 (COP/kWh)) y la parte del vendedor baja de 0,667 a 0,537. El precio ponderado sube de 734 a 742 solo por composición: la energía va más a compradores de techo alto.

**Lo que reporta Chacón** (`Documentos/Modelo_Base_Sofía_Chacon.pdf`). Los compradores fijan los precios «according to their demand and risk aversion» (p. 1 y 2); el término de competencia de la ec. 14 expresa «the urgency to acquire the demanded energy» (p. 5); y sobre la ec. 16: «given that buyers decide the price and can rationally propose the minimum possible, the constraint (16) is introduced to cover the generation costs» (p. 5). En sus resultados, un comprador paga un precio alto y los demás quedan en el piso: Tabla III (14:00, excedente), el comprador 4, «having the highest demand, pays the highest price», 1 250; Tabla V (22:00, déficit), el comprador 1, que cubre toda su demanda, paga 380,30 y los demás 114; Tabla VI, con β3 de 0,1 a 10 su precio pasa de 114 a 402. La versión arbitrada concluye que «el precio determinado es mayor cuanto más alta sea la demanda». En ningún lugar afirma que la escasez de la comunidad suba el nivel del precio o la parte del vendedor.

**Lo que hace su modelo en su caso** (verificado con sus tablas y con la traducción literal de `JoinFinal.m`, que coincide con el motor al centésimo en la forma `matlab` + peso `precio` + arranque a partes iguales):
- El precio medio ponderado por energía es **el costo del vendedor b**: en la Tabla V, H = 225,20 · 1,381 = 311,0 y el precio del comprador que paga es 114 + (311,0 − 114 · 1,381)/0,5767 = 380,3, con ponderado 225,1. La parte del vendedor es (b − piso)/(techo − piso) = **0,098**.
- **Esa parte no cambia con la escasez**: con todas las demandas escaladas y la oferta fija, ΣD/G de 0,61 a 76, el ponderado queda entre 224,5 y 226,5 y la parte entre 0,097 y 0,099. Cambiar la oferta tampoco lo mueve.
- **Pedir más no sube el precio unitario**: el comprador que paga cubre el costo total, así que si recibe más energía su precio baja (481,2 → 406,0 (COP/kWh) al pasar su necesidad de ×0,8 a ×1). Lo que la demanda decide es **quién** paga la prima, no cuánto vale el kWh.
- **Lo que sí mueve el nivel es el costo b**: b = 600 da ponderado 600,1 y parte 0,428; b = 1 200 da 1 250 y parte 1,0; b por debajo del piso deja a todos en el piso con parte 0.

**Sus tablas salen de su código, no de la ec. 24 tal como está escrita.** Con el peso de barrera sobre el jugador virtual (ec. 24, la forma que la traducción adoptó por H-46), su caso no reproduce las tablas: la suma de precios queda fija en 5 000, los precios entre 960 y 1 100, la parte del vendedor en 0,77, y no converge. Con el peso de precio de `JoinFinal.m` se reproducen el patrón, el nivel y la parte del vendedor. Es decir, **H-46 eligió la ecuación del documento extenso (H-61) sobre el código, y es el código el que produce los resultados que ella reporta**. La conservación de la suma de precios de los compradores que describe H-87 es de la traducción: en su código se conserva la de compradores más virtual, no la de los compradores solos.

**Sobre nuestros datos.** El costo nivelado de la generación solar (b = 241 (COP/kWh)) está por debajo del piso de la permuta (unos 693 (COP/kWh), CAL-47), de modo que la ec. 16 nunca muerde. **El modelo de Chacón aplicado fielmente a las tarifas colombianas deja todos los precios en el piso y la parte del vendedor en 0**; el reparto queda congelado en el arranque (con el arranque a partes iguales, el llenado por niveles). La parte del vendedor de 0,5 a 0,7 que reportaba la producción no es resultado del modelo: sale del peso de barrera junto con el arranque de precios por banda (H-46, H-32, H-45). Ninguna otra pieza (ancho de banda, techo por comprador, arranque de la oferta, competencia, λ, θ, η, horizonte) crea ni destruye un traslado de la escasez al precio; λ y θ ni siquiera entran en la dinámica de `JoinFinal.m`.

**Consecuencias.** (a) La aptitud del comprador depende de lo que recibe, de su precio, de β y del multiplicador de costo del vendedor; no contiene la demanda, la oferta ni la escasez. Un precio que suba con la escasez no está en las ecuaciones de Chacón: exigiría un término nuevo (por ejemplo el déficit sin cubrir, D_i − q_i, o el multiplicador de capacidad en la aptitud del comprador), y sería una extensión declarada de la tesis, no una restitución del modelo base. (b) La salida fiel del modelo sobre nuestros datos es precio en el piso, es decir, todo el ahorro para los compradores; H-46 la descartó por «precios pegados al piso», pero es la respuesta legítima del modelo cuando la permuta ya cubre el costo del sol. (c) Activar la prima de Chacón exigiría un costo del vendedor entre el piso y el techo, sin justificación con el costo nivelado real; equivale a fijar el precio a mano.

**Lo que queda abierto.** Quién paga la prima en su código es frágil (la reproducción elige al comprador 2 y su Tabla V al 1; ella ajustaba τ y escalas por hora); la hora de las 14:00 solo se reproduce en parte; las horas con excedente son muy rígidas bajo su forma; y las horas reales con varios vendedores no se probaron con su configuración (se infiere que el costo del vendedor más caro fijaría el precio).

---

## H-89 · Con vendedores sobrantes y costos distintos, el reposo despacha por orden de mérito

**Estado: HIPÓTESIS DERIVADA, registrada el 2026-09-17 y pendiente de la medición M-B. Sale de la síntesis final sobre el juego de precios (`.superpowers/sdd/2026-09-16-sonda-equilibrio/fable-report.md`, §2.2) y de la revisión del núcleo del reposo (`.superpowers/sdd/2026-09-16-reposo/`). Ninguna de sus afirmaciones está comprobada con la dinámica: se registra como hipótesis, no como hecho.**

**La hipótesis.** Cuando los compradores son el lado corto, es decir cuando sobra oferta, y los vendedores tienen costos lineales distintos, el reposo del juego regularizado (H-90) no reparte la demanda entre los vendedores por llenado por niveles, sino por **orden de mérito**: primero el vendedor de menor costo, hasta su excedente, y después los de costo igual, a partes iguales con tope en lo que cada uno ofrece.

**De dónde sale (derivado a mano, no medido).** Con la exploración entrópica del replicador del vendedor, el reposo pone la oferta P_ji en proporción a exp(F_ji/μ), con F_ji = (π_i − β_i) − (b_j + λ_j) + constante (síntesis §2.1, punto 7). El costo lineal b_j entra en ese exponente; el piso del vendedor, no. Cesmag tiene el costo más bajo: 210 frente a 225 (COP/kWh) en la tabla de calibración (síntesis §2.2), que el motor recibe como 225 frente a 241,07 (COP/kWh) tras el ajuste por irradiancia de `data/xm_prices.py` (literales de `tests/test_reposo_mercado.py`). Una diferencia de costo de ese tamaño es mucho mayor que μ = 1 (COP/kWh), de modo que en el límite μ → 0⁺ de la forma cerrada la prioridad es estricta: Cesmag primero y los demás, de costo igual, por llenado. La síntesis la identifica con la segunda contribución del artículo arbitrado, que describe su gestor de energía como «reactivo a los costos de los generadores» (`revision-chacon-report.md` §2).

**Lo que ya se midió no la confirma ni la descarta.** La medición de las variantes de consenso vio que, con los compradores como lado corto (horas 153, 154 y 156 de la semana de E0), los vendedores se repartían a partes iguales con tope en su oferta (`consenso-report.md` §3). Según la síntesis, esa validación comparaba los totales por comprador, que con compradores cortos son sus déficits con cualquier despacho, y no el reparto entre vendedores; y el propio informe de consenso dejó sin medir ese reparto en E4 (§5, punto 4). No consta en las mediciones una hora que enfrente las dos lecturas.

**La consecuencia visible.** El núcleo implementa la hipótesis como despacho por defecto (D52), con el llenado de vendedores como alternativa para que M-B decida sin cambiar código (C-197). En la hora 133 de E4 (2025-04-09 13:00, en la numeración de la matriz; no es la 133 de la semana de E0, H-90) compra solo UCC y pueden vender Udenar, Mariana, HUDN y Cesmag (los tres últimos, retirados en la corrida guardada). Por orden de mérito despacha solo Cesmag, con costo 225 (COP/kWh); como el piso del juego es el de los vendedores que despachan (D62) y un único comprador paga el piso (D54), **UCC paga 330,15 (COP/kWh) a Cesmag mientras Udenar, con piso 147,46 (COP/kWh), no vende**. Con el llenado de vendedores despachan los cuatro, 1,0481 (kWh) cada uno, el piso del juego vuelve a ser el de Udenar y la parte del vendedor sale −0,4406 antes de la participación (`tests/test_reposo_mercado.py`, prueba de la hora 133 de E4; `task-1-fix-report.md`). Es decir, con la hipótesis el orden lo decide el costo nivelado, que difiere poco entre instituciones, y no la alternativa de cada vendedor, que en esa hora va de 147,46 (COP/kWh) (Udenar) a 415,10 (COP/kWh) (Mariana y HUDN); Cesmag, con 330,15 (COP/kWh), tiene la segunda más baja (literales de la hora en `tests/test_reposo_mercado.py`).

**La conjetura sobre la desviación de 0,354.** El primer evaluador midió, sobre la matriz del 15 de septiembre, que en las horas de compradores cortos con Cesmag vendiendo el reparto guardado entre vendedores se aparta del llenado en 0,354 de la energía de la hora, de forma constante (229 horas en E4 y 304 en E5), y lo dejó sin explicar (`evaluador-report.md`, riesgo 8 y §7). La síntesis conjetura que esa desviación es el orden de mérito. Es una inferencia sin comprobar.

**Qué la decide.** La medición M-B de la síntesis (§11): 20 horas de E4 y 10 de E5 con Cesmag entre los vendedores y sobrante total mayor que el déficit, integradas con la exploración entrópica (aceleración k = 100 y 1 000, hasta teq 160), comparando el reparto **entre vendedores** con (a) el orden de mérito y llenado entre iguales y (b) el llenado de vendedores. Se acepta (a) si |ΔP_ji| ≤ 1e-3·E. Si sale (a), D52 queda confirmada y se comprueba si explica el 0,354 (`scratchpad/evaluador/split_vendedores.py`); si sale (b), se revisa si el multiplicador de capacidad λ_j absorbe b_j y se adopta el llenado.

**Hasta entonces.** Las cifras por institución de las horas de compradores cortos con costos distintos, que abundan en E4, E5 y las noches, dependen de esta hipótesis y se citan con esa salvedad. El régimen de compradores cortos lleva el 16,3 % de la energía de E0 (censo del núcleo, H-90) y el 64 % de la de I1 (`task-1-review.md`). En E0 la hipótesis toca pocas horas: en el mismo censo, que no incluye a los vendedores retirados, 7 de las 130 horas de compradores cortos tienen vendedores de costos distintos.

**Reformulado por H-91 (2026-09-17).** El mérito deja de ser por costo nivelado y pasa a ser **por costo de oportunidad**, es decir por el piso piso_j de cada vendedor (D64): el despacho ordena por la alternativa, no por b_j. El motivo no es que la hipótesis de arriba se haya desmentido, sino que con un solo piso de juego y pisos heterogéneos el mérito por b_j dejaba a vendedores cobrando bajo su alternativa, y la participación los retiraba con su energía (H-91). El costo nivelado b_j se conserva como **opción de comparación** (`--despacho-vendedores costo`), y **M-B se reformula**: ya no enfrenta el mérito al llenado, sino c_j = piso_j a c_j = b_j en la aptitud del vendedor de la dinámica regularizada, con las mismas 20 horas de E4 y 10 de E5. La hora 133 de E4 que ilustra esta sección cambia de resultado: con el mérito por piso vende Udenar a 147,46 (COP/kWh), y el excedente pasa de 1 694,38 a 2 460,29 (COP).

**Y con ella queda superada la frase de «La consecuencia visible» que describe el llenado.** Decía que con el llenado de vendedores despachan los cuatro, «el piso del juego vuelve a ser el de Udenar y la parte del vendedor sale −0,4406 antes de la participación». Eso es de la regla anterior, con el piso en el mínimo de los que despachan (D62). **Con D63 y D64 el modo `llenado` pone el piso del juego en 415,1011 (COP/kWh), que es el máximo de los despachados, y la parte del vendedor en +0,216**, es decir positiva y sin necesidad de la participación (`task-5a-report.md`, comparación sobre los literales). El reparto de 1,0481 (kWh) por vendedor sigue valiendo. La prueba de `tests/test_reposo_mercado.py` que se citaba para el −0,4406 afirma hoy lo contrario, porque se actualizó con C-199: lo que queda de aquella cifra es el registro de lo que daba D62.

**La conjetura sobre la desviación de 0,354 sigue siendo conjetura** y no la toca esta reformulación: se comprueba igual cuando M-B decida.

---

## H-90 · El bloque de precios redistribuye un presupuesto; el nivel es una primitiva del mecanismo, y la exploración entrópica da un reposo al que la dinámica llega

**Estado: registrado el 2026-09-17. Reúne la síntesis final sobre el juego de precios y su evidencia (`fable-report.md`, `consenso-report.md`, `evaluador2-report.md` y `revision-chacon-report.md`, en `.superpowers/sdd/2026-09-16-sonda-equilibrio/`), las decisiones D48 a D62 (apartado 9 de `docs/superpowers/specs/2026-09-13-motor-conforme-norma-design.md`; ADR 0059; CAL-53, registrada como ADR 0060) y la implementación y revisión del núcleo del reposo (`task-1-report.md`, `task-1-review.md` y `task-1-fix-report.md`, en `.superpowers/sdd/2026-09-16-reposo/`; C-197). Corrige H-44 y acota H-87. Cada afirmación dice si está derivada a mano de las ecuaciones, medida con un guion o inferida.**

H-87 mostró que, con un vendedor y varios compradores, la dinámica del modelo base oscila sin llegar, y H-88, que el nivel del precio del modelo base es la cobertura del costo del vendedor, que con las tarifas colombianas queda bajo el piso. Las dos cosas tienen una causa común y una salida: el bloque de precios solo redistribuye un presupuesto, de modo que el nivel no lo decide la negociación; con una exploración entrópica la dinámica llega, en las horas medidas, a un reposo único; y ese reposo se calcula en forma cerrada.

**Dos avisos de lectura.**

- **Documento extenso y versión arbitrada.** Las ecuaciones 14, 16, 23 y 24, el Algoritmo 3, la sección III-E y las tablas del caso de Chacón que se citan aquí son del documento extenso de la maestría (`Documentos/Modelo_Base_Sofía_Chacon.pdf`), no del artículo de la revista (H-61); la versión arbitrada no tiene jugador virtual (H-46). La síntesis, el spec y los informes de `.superpowers/` llaman «artículo» a ese documento extenso.
- **Dos numeraciones de horas.** Las horas de la semana de E0, K1 y E4 del 5 al 11 de mayo de 2025 (campaña de convergencia, consenso y segunda evaluación) se cuentan desde el 2025-05-05 00:00; las de la matriz, desde el 2025-04-04 00:00, es decir 744 horas antes. La 109 de la semana es la 853 de la matriz (2025-05-09 13:00), la 130 es la 874 y la 133 es la 877 (2025-05-10 13:00). Aquí van en numeración de la semana la 109, 130, 133, 135, 152, 155 y 159 de E0, la 152 de K1 y la 55 de E4; van en numeración de la matriz la 540, 877, 2120, 2317, 3062, 3086 y 3804 de E0, la 133 de E4, que no es la 133 de la semana (2025-04-09 13:00), y las 5631 y 5632 de E3, E4, E5 y P1.

**1. La aptitud del comprador es la derivada de la suma de los bienestares (derivado).** La ec. 23 del documento extenso deriva, respecto del precio π_i de cada comprador, la suma de los bienestares de la ec. 14 de todos los compradores, y no el bienestar propio. En esa suma π_i aparece en el término de competencia de cada uno de los demás, y su derivada da −(Σ_{k≠i} β_k)·q_i, con q_i la energía que recibe i: es la forma `matlab`, la línea activa de `JoinFinal.m` (línea 189). Término a término, la aptitud del código (`pagos − compe + trestris`) es la derivada del lagrangiano de esa suma: el pago da −π_gb·q_i/(π_i + 1); la competencia de los demás, −(Σ_{k≠i} β_k)·q_i; y la restricción de cobertura del costo del vendedor (ec. 16), +Σ_j σ_j·P_ji (síntesis §2.1, punto 1, derivado sobre `_rhs` y las ec. 23 y 24). La forma agregada que usa esta traducción (CAL-49) es la del código sin el factor (I − 1) (síntesis §2.1, punto 10).

**2. Por eso el bloque de precios solo redistribuye (derivado y medido).** El bloque de precios es un replicador sobre un presupuesto fijo: la suma de los I + 1 precios, los de los compradores y el del jugador virtual, se conserva por construcción. Como la suma de bienestares decrece con cada precio (H-40), la dinámica no mueve el nivel: lo reparte, y quien recibe más energía cede precio. Qué queda del nivel lo decide el peso del jugador virtual:

- **Con la barrera de la ec. 24 del documento extenso**, la de esta traducción (H-46), el virtual sube a su techo y se congela, y la suma de los precios de los compradores queda invariante: S = suma de los arranques de los I + 1 jugadores − techo mayor. Medido: 728,42 × 4 = 2 913,70 (COP/kWh) en la hora 109 de la semana de E0 y en las 26 horas de un vendedor y cuatro compradores de la campaña de convergencia (H-87 §6).
- **Con el peso de precio de `JoinFinal.m`** (línea 211), el virtual absorbe el presupuesto y arrastra los precios al piso; solo la ec. 16 los sube, y con el costo nivelado bajo el piso de la permuta no muerde (H-88).

**3. Con la barrera, el nivel lo fija el arranque (medido; el umbral del final, medido e inferido).** El precio común de los compradores libres es piso + [Σ_i a_i·banda_i − (1 − a_v)·banda_mayor]/I, con a_i y a_v la fracción de la banda en que arrancan los compradores y el virtual; hoy, a_i = a_v = I/(I + 1). En la hora 109 de la semana de E0, con cinco arranques, el nivel que da el motor coincide con la fórmula: 728,4248 (COP/kWh) con el arranque de hoy, 717,6653 con los compradores a la mitad de la banda y el virtual en su techo, 707,1057 con todos a la mitad, y 695,2615 frente a 695,2617 con todos a un tercio. Sobre la matriz, la suma guardada se aparta de la fórmula como mucho 0,006 (COP/kWh) en las 741 horas de un vendedor de E0, 0,004 en las 585 de K1 y 0,008 en las 468 de SINU. Y la misma dinámica, arrancada por debajo del 29,7 % de la banda (umbral de la hora 109), termina en el piso (`evaluador2-report.md` §2.2).

Es decir, **el nivel del precio no lo produce la negociación: es una primitiva del mecanismo**, como el costo del vendedor en el caso de Chacón. La negociación decide quién recibe energía, quién paga más y quién queda fuera. El presupuesto del Algoritmo 3, conservado por la ec. 24, es la única regla de nivel que está en el modelo, y D50 la adopta generalizada a la banda, S = Σ_i [piso + σ_I·(techo_i − piso)] con σ_I = (I − 1)/I, sin el interruptor de C-136, y la barre (σ ∈ {0, 1/2, σ_I, 1}). Con bandas iguales el vendedor se lleva (I − 1)/I del excedente: nada con un comprador, la mitad con dos, tres cuartos con cuatro (derivado; `evaluador2-report.md` §3.3). Que esa fracción coincida con la puja de equilibrio de una subasta de primer precio es coincidencia de fórmula, no derivación (síntesis §2.3, punto 7).

**4. Corrección de H-44 (derivado).** H-44 sostiene que el término de competencia «no entra en el gradiente, en ninguna forma» y que, igual que la señal de restricción, «no es el gradiente de nada». Lo primero vale para la derivada del bienestar propio de cada comprador, que es lo que mide su compuerta: en el bienestar propio no aparece el precio propio dentro de la competencia. Pero la ec. 23 deriva la suma, y ahí sí aparece. Para la forma del código, «no es el gradiente de nada» es falso: la competencia es la derivada de la competencia de los demás, y la señal de restricción, la del término de la restricción. Se sostiene lo medido en H-44: el término de pago es el gradiente del documento extenso y no el de la versión arbitrada (factor de 7 778), y las tres formas dan la misma derivada del bienestar propio. **No mueve ninguna cifra; mueve el relato de fidelidad**: la dinámica del modelo base sigue la derivada de un objetivo colectivo con su restricción, no la del bienestar de cada comprador. CAL-49 se conserva porque el reposo no depende de la forma de competencia (D60). H-44 lleva la nota que remite aquí, y el enunciado de `tests/gate_h44_aptitud_gradiente.py` se corrigió sin tocar sus cálculos: su salida numérica es idéntica.

**5. La exploración entrópica hace que la dinámica llegue (medido).** Con un término de exploración entrópica en el replicador del vendedor, −μ·P·(ln P − ⟨ln P⟩) con μ = 1 (COP/kWh), la dinámica con la barrera llega a un reposo y se queda en las horas medidas (variante V3a de `consenso-report.md`). Es un apartamiento declarado (D49), análogo en espíritu al filtro que la autora añadió en la sección III-E de su documento extenso para mejorar la convergencia. El criterio de consenso se fijó antes de medir: en dos controles seguidos, |q − q_reposo| ≤ 1e-3·E (kWh) y |p − p_reposo| ≤ 0,05 (COP/kWh). Las horas con topados o con varios vendedores solo se pudieron integrar con aceleración, es decir multiplicando por k el bloque de compradores y el replicador del vendedor, lo que no cambia los ceros del lado derecho; esos resultados se leen en tiempo equivalente (teq).

- **Tipos de hora.** El informe de consenso la da por alcanzada en los siete tipos de hora integrados (109, 130, 133 y 155 de la semana de E0; 2120 de la matriz de E0; 152 de K1; 55 de E4), con aceleración en las cuatro de topados o varios vendedores. No todos cumplen su criterio con la misma holgura: la 133 seguía a 13,4 (COP/kWh) de la forma cerrada en teq 10 y figura en la semana entre las que solo se acercan, y en la 55 de E4 los precios estaban entre 487,2 y 487,6 (COP/kWh) en teq 10, frente a 487,42 (COP/kWh) de la forma cerrada. En la semana de E0, 13 de 20 horas coinciden con la forma cerrada de esa medición (|Δq| ≤ 6,3e-4 (kWh), |Δp| ≤ 0,03 (COP/kWh)), 5 se acercan y 2, la 152 y la 159, no llegan en local. Esa forma cerrada (`semana.py`) no tiene orden de mérito ni la regla de la suma que no cabe, de modo que no es la del núcleo (síntesis §2.3, punto 6).
- **Sensibilidad a μ** (hora 109 de la semana). Con μ = 0,05 no llega a t = 40; con 0,3 llega a t = 80 (1,3 millones de evaluaciones); con 1, a t = 40 (97 449 evaluaciones); con 2, a t = 80 el reparto está a 9,7e-7 (kWh) del llenado; con 5, a 1,6e-4 (kWh), más lento por sobreamortiguado (`consenso-report.md` §3; `scratchpad/consenso/s10_mu.log`). En todos los casos el reposo es el mismo, 728,42 × 4 (COP/kWh), llenado y parte del vendedor 0,715: **μ solo cambia la velocidad**.
- **Momento.** Una vez en reposo no se mueve: en la hora 109 y en la 152 de K1, los puntos de t = 40 y t = 80 coinciden a 1e-7 (kWh) y 1e-6 (COP/kWh), y con aceleración teq 40, 80 y 160 coinciden a 1e-7 (kWh) (síntesis §1).
- **Arranque de la oferta.** No depende: con la oferta a partes iguales se llega al mismo reposo en la 109, la 152 de K1 y la 2120 (3,8e-12 (kWh); síntesis §2.1, punto 7).
- **Arranque de los precios.** Sí depende, porque fija el presupuesto (punto 3): con los precios a la mitad de la banda el nivel es 707,11 (COP/kWh) en la 109 y en la 152 de K1 (parte 0,289), y la 2120 cambia de régimen; al 90 % de la banda, una esquina con Cesmag recibiendo todo a 748,74 (parte 0,663); al 10 %, piso y llenado. Por eso el presupuesto se declara y se barre.
- **Emparejamiento de rango uno (derivado).** En el reposo regularizado P_ji es proporcional a exp(F_ji/μ), que se factoriza, de modo que el emparejamiento de varios vendedores con varios compradores es de rango uno, P_ji = s_j·q_i/E. De los emparejamientos que el replicador puro deja indeterminados (H-87 §8), la regularización selecciona el de máxima entropía, que coincide con el arranque factible. Medido en la hora 133 de la semana de E0: el cociente entre lo que venden Udenar y Mariana es 12,98, el de sus ofertas.
- **Costo de integrar.** Las horas rígidas no se pueden integrar en producción: la 135 de la semana de E0 con k = 100 gasta 2,08 millones de evaluaciones y 360 (s) hasta teq 160, y la 152 con k = 100, 1,8 millones sin terminar (síntesis §2.1, punto 8). Por eso el reposo se calcula en forma cerrada y la dinámica queda para validarlo (D48).
- **Sin demostración general.** La estabilidad descansa en un argumento lineal derivado (el término añade rozamiento), no demostrado; lo decide M-D.

**6. El reposo en forma cerrada, por regímenes (derivado; implementado en C-197).** En cada hora el volumen es el lado corto, E = min(Σs, Σd) (D-7), el presupuesto es S (D50) y cada comprador con déficit queda en su techo, interior a un precio común ℓ o en el piso del juego. La especificación es la de la síntesis (§5.1), con las decisiones de la revisión:

1. **Prioridad por precio.** El vendedor sirve primero al precio más alto; a igual precio, por llenado por niveles.
2. **Llenado del grupo marginal.** Los compradores interiores comparten ℓ = (S − Σ techos de los que están en su techo)/|N| y reciben lo mismo, con tope en su déficit.
3. **Topados.** Los de techo por encima de ℓ quedan en su techo y reciben min(d_i, nivel), nunca más que uno del grupo marginal.
4. **Excluidos.** El comprador cuyo techo queda bajo ℓ se queda en su techo sin energía.
5. **La suma no cabe sobre el piso (D53, corregida).** Si ningún conjunto de interiores cumple, en orden de déficit creciente cada comprador se satura en su techo mientras quepa el presupuesto; el primero que no cabe queda interior con el resto, y los demás, en el piso. La suma de precios se conserva.
6. **Compradores cortos.** Cada comprador recibe su déficit; los vendedores despachan por orden de mérito, **como hipótesis pendiente de M-B** (D52, H-89), y los precios siguen la regla 5 con todos los compradores como candidatos. **Sustituido por H-91 (D63, D64):** el mérito es por piso_j, no por b_j, y el despacho lo resuelve la caminata competitiva de D65.
7. **Un comprador.** Paga el piso (D54). **Precisado por H-91 (D63, D66):** D54 se conserva, y ese piso es ahora el del vendedor marginal, no el mínimo.

El emparejamiento es de rango uno y la liquidación, al precio marginal uniforme de la hora que conserva el ingreso del reposo (D51). Las cuatro primeras reglas son la forma cerrada de la segunda evaluación, contrastada con el promedio del sistema reducido con topes, y la evidencia es desigual por régimen (`evaluador2-report.md` §3.1): en cuatro horas con topados, diferencias de energía entre el 0,08 % y el 3,6 % de E y de precio de hasta 2 (COP/kWh); en tres horas de excluidos, coincidencia exacta; en dos horas de régimen mixto, hasta el 4,6 % de E; y con todos interiores, una hora oscila un 0,63 % y otras tres quedaron en una esquina todo el intervalo. Validar los regímenes es lo que hace M-A. El llenado por niveles de H-87 §2 es, por tanto, el reposo solo con todos los compradores interiores y en el reparto con topados, no en los regímenes de excluidos ni de la suma no cabe.

**D53, corregida.** La versión anterior de D53 («el de mayor déficit queda marginal en el piso») describía un segundo lazo del prototipo, que pone al grupo marginal en el piso y a los demás en su techo sin conservar la suma. Con el presupuesto de hoy del motor (`c136`), la regla corregida reproduce al centésimo los promedios de la dinámica en las horas 540, 2317 y 3062 de la matriz de E0, y a 1,4 (COP/kWh) en la 3086, donde la dinámica oscila; el segundo lazo quedaba a entre 24 y 44 (COP/kWh) en las cuatro (`task-1-report.md`). Los valores esperados que el encargo del núcleo traía para las horas 3086 y 877 salían de ese lazo y se retiraron.

**Censo de E0 por régimen.** `scratchpad/tarea1/censo_E0.py` llama a `resuelve_reposo` con las opciones por defecto (presupuesto `sigma`, despacho `merito`, liquidación `uniforme`) sobre el almacén de E0 de la matriz del 15 de septiembre (`SALIDAS_SERVIDOR/entrega_matriz_2026-09-15/`, tabla `agentes`), con los vendedores y compradores guardados y sin reincorporar a los vendedores retirados. La primera corrida fue anterior a D61, D62 y la exclusión de vendedores (`task-1-report.md`); repetida el 2026-09-17 con el núcleo revisado (Python 3.13.7, porque el entorno virtual no tiene pyarrow), da **los mismos porcentajes**: 1 126 horas con mercado, todas con un único reposo, ninguna de régimen mixto ni sin ganancia, y 4 062,8 (kWh) en total.

| Régimen | Horas | Energía (kWh) | Energía de E0 (%) |
|---|---:|---:|---:|
| Topados que reciben | 332 | 1 674,2 | 41,2 |
| Todos interiores | 312 | 676,4 | 16,6 |
| Compradores cortos | 130 | 662,2 | 16,3 |
| La suma no cabe sobre el piso | 74 | 578,8 | 14,2 |
| Excluidos | 226 | 384,2 | 9,5 |
| Un comprador | 52 | 87,0 | 2,1 |

Estos porcentajes no se comparan con los de la segunda evaluación (29,5 % todos interiores, 48,4 % topados, 13,0 % excluidos y 9,0 % suma no cabe), que cuentan solo las horas de un vendedor corto con reposo único y usan el nivel heredado del arranque.

**El caso de Chacón a las 22:00.** Con el presupuesto del Algoritmo 3 literal (S = 5 000), el núcleo da precios de 833,33 × 3 y 1 250 × 2 (COP/kWh) y reparto [0,3037 × 3; 0,262; 0,208] (kWh), en régimen de topados con un único reposo: exactamente la predicción que la síntesis derivó a mano. Liquidado, p_u = 975,14 (COP/kWh) y parte del vendedor 0,758 (`task-1-report.md`). No son sus tablas (un comprador en 380,30 y los demás en 114), que salen del peso de precio de su código (H-88). Que la dinámica regularizada llegue ahí en su caso sigue sin integrarse: a t = 0,3 los precios iban por 947,6 × 3, 1 037 y 1 120 (`consenso-report.md` §3); lo decide M-G.

**7. Lo que salió en la revisión del núcleo** (`task-1-review.md` y `task-1-fix-report.md`; decisiones del autor D61 y D62).

- **Horas sin ganancia posible (D61).** En las horas 5631 y 5632 (24 de noviembre de 2025, 15:00 y 16:00), el piso de todos los vendedores, 791,36 (COP/kWh), supera el techo de todos los compradores, 720,12 (COP/kWh): ningún precio deja a las dos partes mejor que con la red. El núcleo lanzaba un error que habría detenido la matriz. Ahora la hora sale sin mercado, anotada con su causa: una hora en E3 y las dos en E4, E5 y P1, con el 0,05 %, 0,22 %, 0,51 % y 0,22 % de la energía de cada caso. Un comprador activo con techo bajo el piso del juego queda excluido de la hora (ninguno en los trece casos).
- **El piso del juego es el de los vendedores que despachan (D62).** Con compradores cortos, el piso del juego era el mínimo de los vendedores activos, y a menudo lo fijaba uno que no despacha: el precio quedaba bajo el piso del que vende y la parte del vendedor salía negativa sin aviso en 8 de los 13 casos (hasta −4,4 en N1). En la hora 133 de E4 (numeración de la matriz) despacha solo Cesmag y el piso era el de Udenar, 147,46 (COP/kWh), con parte −0,452. Con D62 el núcleo despacha primero y toma después el piso de los que despachan: 330,15 (COP/kWh) y parte 0 (H-89). Afectaba a 45 horas de E1, 57 de E2, 74 de E3, 41 de E4, 29 de E5, 19 de I1, 107 de N1 y 42 de P1, y a ninguna de E0, K1, CV2, P2 y SINU. **Sustituido por H-91 (D63, D64):** tomar el mínimo de los que despachan sigue dejando a los vendedores de piso alto bajo su alternativa, y la participación los retira con su energía; el piso del juego pasa a ser el del **vendedor marginal**, y el despacho, por piso_j creciente. En esa misma hora 133 de E4 vende Udenar a 147,46 (COP/kWh), y el excedente sube de 1 694,38 a 2 460,29 (COP).
- **Vendedores sin ganancia posible.** El vendedor activo cuyo piso supera el techo de todos los compradores activos queda fuera antes del orden de mérito; sin esto, el mérito podía despachar a un vendedor sin ganancia mientras otro de piso bajo no vendía. En los trece casos solo toca la hora 5631 de E3, que no cambia de régimen: salen Mariana y Cesmag (piso 791,36 (COP/kWh) frente a techos de 720,12 (COP/kWh)), vende HUDN (piso 681,54 (COP/kWh)), E baja de 5,729 a 0,852 (kWh), p_u = 700,83 (COP/kWh) y parte 0,5.
- **Tras la participación emulada, ninguna parte del vendedor negativa.** Con varios vendedores despachados de pisos distintos, el piso del juego es el menor, y quedan partes negativas antes de la participación (por ejemplo, 68 horas y el 10,8 % de la energía en E0; 700 horas y el 60,8 % en E2). Con la restricción de participación **emulada por la revisión** encima, que es el paso 7 y en producción va en el motor, ninguna parte del vendedor queda negativa y ningún vendedor despachado queda bajo su piso en los trece casos, con a lo sumo una vuelta de retiro. Con rango uno todos los vendedores cobran el mismo precio medio, de modo que tras el retiro la parte es no negativa por construcción. Falta comprobarlo con el motor conectado: es la compuerta que hereda C-198. **Sustituido por H-91 (D63):** el piso del juego ya no es el menor de los despachados sino el del vendedor marginal, de modo que **no hay partes negativas que corregir**: ninguna aparece antes de la participación, y las cifras de arriba (68 horas y el 10,8 % de la energía en E0; 700 horas y el 60,8 % en E2) son el registro de lo que daba D62. **Y lo que faltaba comprobar ya está resuelto:** el motor conectado lo ejerce con la compuerta de cero retiros de D67, implementada y probada en C-199, y con el teorema de cobertura dentro del núcleo.

**8. El modo `c136` y H-32 con dos compradores.** El núcleo conserva, solo para comparar con la dinámica, el presupuesto de hoy del motor (`c136`, copia literal del acoplado con el interruptor de C-136). Con dos compradores y el interruptor sin disparar, S = Σ techos − techo mayor = techo menor, que queda por debajo de dos veces el piso: es H-32, y el núcleo lanza un error. Son 451 horas de ocho casos (CV2, E3, E4, E5, I1, N1, P1 y P2). La revisión, que contó 464 antes de D62 (la diferencia sale de I1 y N1), les midió esta energía: I1 9,2 %, P2 7,5 %, CV2 6,4 %, N1 4,9 %, E4 2,2 %, E5 1,1 %, P1 0,5 % y E3 0,1 % (`task-1-review.md`, punto 2). En esas horas M-A y M-C no pueden usar `c136`. El presupuesto de producción no tiene el problema: con σ_I ningún caso lanza error. **Las 451 horas son de D62 y hay que remedirlas (H-91):** `c136` lanza cuando S < I·piso, y el piso marginal es mayor o igual que el mínimo, de modo que rechaza más horas, no menos. En 4 000 horas sintéticas con pisos heterogéneos el núcleo pasa de 520 a 1 019 horas rechazadas, 419 de ellas en común. **El recuento sobre los trece casos con la regla nueva se hace en el servidor**; hasta entonces la cifra de 451 no se cita como si fuera la de hoy. La matriz no usa `c136`.

**9. Lo que queda abierto.** Las mediciones M-A a M-H de la síntesis (§11), en el servidor con el entorno de `requirements-lock.txt`. No bloquean la matriz: deciden si cada régimen se publica como reposo verificado o como regla declarada, con el criterio de al menos el 95 % de las horas de cada régimen dentro de tolerancia.

- **M-A.** La forma cerrada frente a la dinámica regularizada, por régimen.
- **M-B.** El orden de mérito con compradores cortos (H-89). **Reformulada por H-91 (D64):** ya no enfrenta el mérito al llenado, sino **c_j = piso_j a c_j = b_j** en la aptitud del vendedor de la dinámica regularizada, con las mismas 20 horas de E4 y 10 de E5.
- **M-C.** La regla de la suma no cabe sobre el piso: el mismo reposo con cuatro arranques, coincidente con D53.
- **M-D.** La estabilidad: el jacobiano en el reposo cerrado, en 50 horas.
- **M-E.** La sensibilidad a μ y los empates de techos o costos a menos de 3μ.
- **M-F.** La matriz de trece casos con el reposo, con sus compuertas de salida.
- **M-G.** El caso de Chacón a las 22:00 y a las 14:00.
- **M-H.** El peso de las horas de un comprador y los retiros con la liquidación nueva (D54 se reabre si pasan del 5 % de la energía). **Respondida en parte por H-91.** El disparador **se cumple**, y no en un caso sino en ocho: la energía de las horas con un solo comprador con déficit es el 62,7 % de la transada en E5, el 60,1 % en P1, el 57,1 % en E4, el 53,6 % en E3, el 40,2 % en E2, el 23,1 % en N1, el 21,6 % en E1 y el 10,8 % en SINU (`task-2-fix2-report.md`, columna «% 1c»; los otros cinco casos quedan entre el 0 % y el 2,4 %). La reapertura de D54 se resolvió con **D66: D54 se conserva, reinterpretada**, porque el comprador único paga el precio competitivo del lado vendedor, que es el piso marginal y ya no el mínimo; con σ_1 = ½ solo como fila del barrido. **La segunda mitad de M-H, los retiros, pasa a ser la compuerta de cero retiros** de D67, que la matriz ejerce en cada caso.
- **M-I, M-J y M-K, añadidas por H-91.** M-I, los trece casos con el núcleo nuevo, comparados con la tabla medida del contrafáctico al 0,1 (kWh). M-J, la descomposición de la prima y el peso del vendedor marginal minúsculo. M-K, cuántas horas de compradores cortos despachan distinto el mérito por piso y el mérito por b_j, y cuánta energía y cuánto excedente mueven.

**Consecuencia.** El mercado se publica en su reposo, calculado en forma cerrada (D48, CAL-53); el nivel del precio se declara como parámetro del mecanismo y se barre; y la no convergencia de la dinámica original pasa a ser un hallazgo del método de solución (actividad 4.2), no del mercado.

---

## H-91 · Con pisos distintos por vendedor, anclar el juego al piso mínimo retira energía comerciable; el piso del vendedor marginal la recupera

**Estado: registrado el 2026-09-17, con la regla nueva decidida por el autor (D63 a D70) e implementada en el núcleo, el motor, la compuerta de salida y el lanzador (C-199). Cierra lo que C-198 dejó abierto. Sale de tres piezas de `.superpowers/sdd/2026-09-16-reposo/`: la medición del motor sobre los trece almacenes de la matriz del 15 de septiembre (`task-2-fix2-report.md`), la medición de la energía que se pierde por el retiro (`medicion-retiro-report.md`) y el diagnóstico con la especificación de la regla (`fable-retiro-report.md`), más la implementación, su revisión y su re-revisión (`task-5a-report.md`, `task-5a-review.md`, `task-5a-fix-report.md` y `task-5a-rereview.md`, de donde salen las 141 136 horas adversarias, el déficit peor de 7,0e-7 (COP) y los tres peores déficits por modo de despacho). Cada afirmación dice si está medida, derivada a mano o inferida.**

C-198 dejó abierto un hecho incómodo: con el motor conectado al núcleo del reposo, la restricción de participación retiraba vendedores y con ellos energía que algún comprador habría pagado. La medición encontró que el retiro no es del mecanismo de defensa sino de lo que defiende: **con un solo piso de juego y pisos heterogéneos, el emparejamiento de rango uno paga a todos los vendedores el mismo precio medio, de modo que el de piso alto queda bajo su alternativa por construcción**. La salida no es quitar la participación, sino definir el piso del juego como el del vendedor marginal.

### Lo medido

**1. Cuánta energía retira la participación** (medido; motor sobre las entradas de los trece almacenes de la matriz del 15 de septiembre, en float32, `task-2-fix2-report.md`, tabla; los totales y los porcentajes, de `medicion-retiro-report.md` §A2, columna «nueva»). La pérdida es la energía comerciable que el retiro deja sin transar, es decir la que algún comprador con déficit sin cubrir y techo sobre el piso del retirado habría comprado.

| Caso | Transada (kWh) | Pérdida por retiro (kWh) | % de la transada | De ella, en horas de un comprador (kWh) |
|---|---:|---:|---:|---:|
| CV2 | 3 699,7 | 892,4 | 24,1 | 86,2 |
| E0 | 4 006,0 | 586,1 | 14,6 | 86,2 |
| E1 | 10 624,5 | 5 977,4 | 56,3 | 3 255,1 |
| E2 | 10 782,8 | 6 488,8 | 60,2 | 3 915,0 |
| E3 | 10 722,8 | 4 364,0 | 40,7 | 2 370,5 |
| E4 | 10 499,1 | 2 308,6 | 22,0 | 1 540,6 |
| E5 | 8 271,8 | 993,0 | 12,0 | 732,2 |
| I1 | 17 421,5 | 134,3 | 0,8 | 0,0 |
| K1 | 3 712,1 | 6,4 | 0,2 | 0,0 |
| N1 | 4 660,9 | 1 413,4 | 30,3 | 235,7 |
| P1 | 1 430,0 | 399,6 | 27,9 | 216,8 |
| P2 | 30 854,9 | 1 289,2 | 4,2 | 603,1 |
| SINU | 818,7 | 99,0 | 12,1 | 46,0 |

En las horas de un solo comprador toda la energía se paga al piso (D54) y la parte del vendedor es exactamente 0 en los trece casos.

**2. El defecto no es de la vía nueva: la acoplada pierde lo mismo** (medido; `medicion-retiro-report.md` §A1 y §A2, sobre los mismos almacenes con la vía acoplada del 2026-09-15). La pérdida de la vía vieja queda dentro del ±2 % de la nueva en E1, E2, E3, N1, P1, I1 y K1, e idéntica en CV2 (892,4 (kWh) en las dos); pierde más en E4, E5 y P2 (3 574,7 frente a 1 289,2 (kWh)) y menos en E0 y SINU. En las horas de un comprador la vía vieja ya pagaba casi el piso: la parte del vendedor, ponderada por energía, quedaba entre 0,000 (P2) y 0,114 (P1) según el caso, frente a 0 exacto en los trece con el reposo. Que se retire más se leyó del código, no se midió: su piso es el mínimo de todos los vendedores activos, no el de los que despachan.

**3. La pérdida tiene dos causas, y solo dos** (medido; `medicion-retiro-report.md` §B, que asigna cada kWh perdido a un retirado y ordena los prefijos por piso decreciente). La primera es **la hora de un solo comprador** (D54), que pone el precio en el piso mínimo y retira en la primera vuelta a todos los de piso mayor. La segunda es **un precio uniforme por debajo del piso del retirado con varios compradores**. Una tercera causa vale 0 en los trece casos, y ninguna hora mezcla causas. El peso de la primera dentro de la pérdida es del 54 al 74 % en E1 a E5 y P1, del 47 % en P2 y SINU y del 10 al 17 % en CV2, E0 y N1. **El retirado está en el tramo de permuta en toda la pérdida salvo 22,9 (kWh) de E4**, lo que apunta a la deducción del artículo 25 como origen de la heterogeneidad de pisos; el tramo se infirió de la brecha entre techo y piso, no se leyó del almacén, y solo se pudo comprobar en E2 a E5, N1 y P1.

**4. En todas las horas con retiro existía un precio uniforme que cubría a todos** (medido; `medicion-retiro-report.md` §C). En las **4 886 horas con retiro** de los trece casos, el piso máximo de los vendedores despachados es menor o igual que el techo mínimo de los compradores servidos: siempre había un precio que dejaba a las dos partes mejor que con la red. La brecha entre ese piso máximo y el precio uniforme que la regla vieja fijó tiene medianas por caso de 1,7 (K1, que tiene una sola hora) a 262,4 (N1) (COP/kWh), percentiles 90 de hasta 580,7 (E3) y un máximo de 609,9 (E1, E2 e I1). Los tres rangos son de la columna del total de cada caso; las submuestras de un comprador y de varios dan otros, y no se mezclan. Es decir, la participación nunca probó el precio que sí cubría a todos: es todo o nada.

**5. Los dos contrafácticos recuperan el 100 %** (medido; `medicion-retiro-report.md` §D, con el motor de producción y el núcleo sustituido por una copia del borrador). D1 pone el piso del juego en el máximo de los pisos de los despachados; D2 ordena el despacho por piso_j y pone el piso en el del último despachado, es decir el marginal. Los dos dan **0 horas con retiro y 0 (kWh) perdidos en los trece casos**, sin excluir compradores y sin romper ninguna identidad, y la energía ganada es exactamente la pérdida de la base. Se pagan con ahorro de los compradores, en millones de (COP):

| Caso | Variante | Excedente | Ahorro de los compradores | Prima de los vendedores |
|---|---|---:|---:|---:|
| E1 | base | 1,363 | 1,020 | 0,343 |
| E1 | D1 | 1,362 | 0,610 | 0,752 |
| E1 | D2 | 1,669 | 0,882 | 0,787 |
| E2 | base | 2,118 | 1,724 | 0,394 |
| E2 | D1 | 1,872 | 0,735 | 1,138 |
| E2 | D2 | 2,541 | 1,371 | 1,170 |
| E3 | base | 3,840 | 3,314 | 0,526 |
| E3 | D1 | 3,441 | 1,846 | 1,595 |
| E3 | D2 | 4,215 | 2,817 | 1,398 |
| I1 | base | 6,154 | 3,009 | 3,144 |
| I1 | D1 | 5,366 | 1,394 | 3,973 |
| I1 | D2 | 6,167 | 2,838 | 3,329 |

D2 conserva más ahorro y más excedente que D1 en todos los casos donde difieren, y con D2 el excedente es **mayor o igual** que el de la base en los trece: mayor en doce y sin cambio en K1 (0,284 millones de (COP) en las dos), que es el caso donde solo se recuperan 6,4 (kWh).

**Cómo está medido, y qué falta.** Los dos contrafácticos se corrieron con el motor de producción **sustituyendo `resuelve_reposo` por una copia del borrador**, es decir con el lazo de punto fijo anterior y **no con la caminata competitiva de D65** que quedó implementada. Las cifras de esta tabla son, por tanto, una vista previa de lo que la regla adoptada debería dar, no la medida de lo que da. **Comprobarlo es M-F y M-I**, que comparan los trece casos del núcleo nuevo con esta tabla al 0,1 (kWh); es el riesgo 5 que dejó anotado el diagnóstico.

### El diagnóstico: el defecto es del mecanismo, no de la participación

Lo derivó Fable (`fable-retiro-report.md` §2) sobre las ecuaciones y el código, con la medición de arriba como evidencia.

- **La raíz es el ancla al piso mínimo con emparejamiento de rango uno.** La tesis dio a cada vendedor su propio piso (CAL-47, H-49), pero el juego conserva un único piso escalar, y D62 lo ponía en el mínimo de los que despachan. Con rango uno todos los vendedores cobran el mismo ingreso medio, p̄ = Σ_i π_i·q_i / E, que cumple p̄ ≥ piso mínimo pero no p̄ ≥ piso_j de cada uno. Si un vendedor despachado tiene piso_j > p̄, no hay reposo compatible con la participación: el núcleo devuelve el equilibrio de un mercado con una sola alternativa. Con bandas iguales, el vendedor de piso alto queda fuera cuando piso_j − piso_mínimo > σ_I·banda, y la heterogeneidad medida supera ese umbral con frecuencia.
- **D54 es el caso extremo de lo mismo:** con un comprador, σ_1 = 0 pone el precio exactamente en el piso mínimo, de modo que se retiran todos los de piso mayor en la primera vuelta.
- **El mérito por b_j (D52) alimenta el defecto.** No pierde energía por sí solo, pero sí excedente: b_j casi no varía entre instituciones (225 frente a 241,07 (COP/kWh)) mientras la alternativa va de 147,46 a 415,10 (COP/kWh) en la hora 133 de E4, donde UCC paga 330,15 (COP/kWh) a Cesmag mientras Udenar no vende a 147,46 (COP/kWh).
- **La participación no es la causa: la hace visible.** Es correcta como racionalidad individual, y es la que impide publicar un vendedor cobrando bajo su alternativa; lo que no puede hacer es subir el precio.
- **La dinámica tampoco lo repara** (derivado, leído sobre `_rhs`). La aptitud del vendedor es (π_i − β_i) − (b_j + λ_j) y la barrera usa el escalar del piso mínimo: **piso_j no aparece en ninguna aptitud**. Por eso la vía acoplada pierde lo mismo, y por eso la ecuación 16 del documento extenso no puede subir el nivel del presupuesto, solo redistribuirlo (H-90, punto 2).

La conclusión es que se trata de un defecto del mecanismo heredado del modelo base, que tiene un solo piso, y que se resuelve al definir cuál es el piso del juego.

### La regla adoptada (D63 a D67 y D69)

El piso del juego pasa a ser **el del vendedor marginal**, es decir el del último que hace falta para cubrir la demanda. Los vendedores despachan por su alternativa piso_j creciente, con llenado por niveles en los empates, y una caminata competitiva sobre los pisos define quién entra de cada lado y la energía E: p* es el menor nivel de piso que maximiza el mínimo entre la oferta con piso ≤ p y la demanda con techo ≥ p (D63, D64 y D65). La banda del juego queda [p*, techo_i], con el presupuesto σ_I y la liquidación uniforme sin cambio. Es la subasta de precio uniforme del mercado mayorista, la misma que justifica D51, y es la variante D2 de la medición, con el despacho en forma de subasta. **Con un solo vendedor, o con pisos iguales, es idéntica a la regla anterior.** D54 se conserva reinterpretada: el comprador único paga el precio competitivo del lado vendedor, que es el piso marginal (D66). La prima se publica descompuesta en renta inframarginal Σ_j (p* − piso_j)·s_j y parte del juego (p̄ − p*)·E (D69).

Los dos ejemplos que la fijan son sintéticos, derivados a mano por Fable (§6) y comprobados a mano por el controlador.

**S1, un comprador y tres vendedores con pisos distintos, vendedores sobrantes.** Demanda 10 (kWh) con techo 734 (COP/kWh); ofertas (3; 4; 2) (kWh) con pisos (150; 330; 415) (COP/kWh); E = 9 (kWh).

| | Regla anterior | Piso marginal |
|---|---|---|
| Precio | 150 (COP/kWh) | 415 (COP/kWh) |
| Quién vende | solo el de piso 150, tras dos retiros | los tres |
| Energía | 3 (kWh); se pierden 6 | 9 (kWh) |
| Ahorro del comprador | 1 752 (COP) | 2 871 (COP) |
| Prima de los vendedores | 0 | 1 135 (COP), repartidos (795; 340; 0) |
| Parte del vendedor | 0 | 0,283 |

El comprador ahorra más pagando casi el triple, porque compra 9 (kWh) y no 3.

**S2, dos compradores y dos vendedores, uno en bolsa y otro en permuta.** Ofertas (2; 2) (kWh) con pisos (350; 693) (COP/kWh); demandas (3; 3) (kWh) con techos (731; 777) (COP/kWh); E = 4 (kWh), I = 2, σ = ½.

| | Regla anterior | Piso marginal |
|---|---|---|
| Presupuesto y precio común | S 1 104 (COP), ℓ 552 (COP/kWh) | S = 2·693 + ½·(38 + 84) = 1 447 (COP), ℓ 723,5 (COP/kWh) |
| Energía | 2 (kWh) tras la segunda vuelta; se pierden 2 | 4 (kWh) |
| Ahorro de los compradores | 404 (COP) | 122 (COP) |
| Prima de los vendedores | 404 (COP) | 808 (COP) |
| Excedente | 808 (COP) | 930 (COP) |
| Parte del vendedor | 0,500 | 0,869 |

Con la regla anterior el vendedor de permuta cobra 552 (COP/kWh), por debajo de su piso de 693, y se retira. S2 es también el ejemplo del riesgo: **el ahorro de los compradores cae de 404 a 122 (COP)**, y la diferencia es renta de los inframarginales.

### Lo que da el motor con la regla nueva

Medido por el implementador de la tarea 5a y comprobado a mano por su revisión. **Las cifras de la matriz con la regla nueva todavía no existen**: las da M-F en el servidor. Lo que hay son horas reales sueltas y casos sintéticos.

- **De literales** (catorce horas reales sacadas una vez de los almacenes de la matriz del 15 de septiembre, con la huella `fe4488168ced426c`, que no cambió). **Once horas son idénticas al bit** a lo que daba la regla anterior: la 853, 874, 2120, 3804, 3086, 877, 12, 58, 540, 226 y 4766, en los cuatro pares de presupuesto y liquidación. Los motivos están comprobados uno a uno: siete tienen un solo vendedor; en la 877, la 12 y la 226 todos los pisos son iguales; y en la 4766 el vendedor de piso menor (Cesmag, 616,46 (COP/kWh)) cubre él solo la demanda de 1,7224 (kWh). En los tres casos el piso marginal coincide con el mínimo, de modo que la igualdad es estructural y no una casualidad.
- **De literales, la hora que cambia, y es el cambio buscado.** En la hora 133 de E4 (2025-04-09 13:00, numeración de la matriz) la caminata da el mismo volumen, 4,1924 (kWh), en los tres niveles de piso y toma el menor: **vende Udenar a 147,46 (COP/kWh)**, y no Cesmag a 330,15 como antes. El excedente de la hora pasa de 1 694,38 a 2 460,29 (COP), la captura de 0,689 a 1, y no hay retiros ni con una regla ni con la otra.
- **De literales, las opciones de comparación.** Con `despacho_vendedores="costo"` las once horas dan lo mismo que la regla anterior, incluida la 133 de E4. Con `"llenado"` cambian la 4766 y la 133 de E4: el piso pasa del mínimo al máximo de los despachados (616,46 → 677,2828 y 147,46 → 415,1011 (COP/kWh)) y la parte del vendedor, que salía negativa antes de la participación (−0,262 y −0,441), pasa a +0,512 y +0,216. Las horas 5631 y 5632 de E4 siguen `sin_ganancia` con las tres opciones (D61).
- **Sintéticas.** Cambian tres casos de prueba del núcleo, con el piso subiendo de 650 a 690 y de 620 a 690 (COP/kWh). En **4 000 horas sintéticas con pisos heterogéneos**, el modo `c136` rechaza el presupuesto por H-32 en 1 019 horas con el piso marginal frente a 520 con el anterior, 419 de ellas en común: es estructural, porque `c136` lanza cuando S < I·piso y el piso marginal es mayor o igual que el mínimo. **La cifra de 451 horas de H-90, punto 8, es de D62 y hay que remedirla en el servidor**; la matriz no usa `c136`.
- **Sintéticas, el lazo de la opción `"costo"`.** Con el piso máximo de los despachados el lazo de exclusión deja de ser monótono y oscila en 106 de 4 000 horas, es decir el 2,7 %; el núcleo lo dice en voz alta y la hora queda con su motivo. Con `"piso"` y con `"llenado"` no ocurre.

### El teorema de cobertura, y por qué la participación queda inerte

**Con el piso marginal, todo vendedor despachado cobra al menos su piso.** Es un teorema del emparejamiento de rango uno, derivado a mano y verificado aparte por la revisión: con rango uno todos los vendedores cobran el mismo ingreso medio p̄ = Σ_i π_i·q_i / E; todo precio de un comprador servido está dentro de [p*, techo_i] por construcción de la banda; luego p̄ ≥ p*, y p* es el mayor piso de los que despachan. La re-revisión lo ejerció sobre **141 136 horas adversarias** (pisos y techos sacados de la misma lista para forzar empates, ofertas de 0 a 50 (kWh) incluyendo 1e-12, las tres opciones de despacho): cero violaciones de cobertura, cero desajustes del piso marginal y cero desajustes entre lo despachado y E, con un déficit peor de 7,0e-7 (COP) sobre toda la población (`task-5a-rereview.md`).

Se apoya en el otro hecho que C-198 dejó anotado: **el piso del juego nunca sube tras un retiro**, medido en 40 000 horas al azar con 25 545 vueltas de retiro, ninguna subida y 462 bajadas. Los dos juntos cierran el lazo: si nadie cobra bajo su piso, nadie se retira; si nadie se retira, el piso no cambia. De ahí D67: **la restricción de participación se conserva como guarda que debe quedar inerte**, cero retiros pasa a ser compuerta de la matriz y un retiro detiene la corrida con el código de D38. Quitarla sería perder la alarma.

### Lo que queda abierto

- **M-B, reformulada.** La pregunta ya no es mérito por costo frente a llenado, sino **c_j = piso_j frente a b_j**: si el reposo de la dinámica regularizada despacha por la alternativa o por el costo nivelado. Se mide en el servidor con 20 horas de E4 y 10 de E5. De ella depende el relato de H-89, cuyo «costo» pasa a ser costo de oportunidad.
- **M-I a M-K, en el servidor.** M-I, los trece casos con el núcleo nuevo, comparados con la tabla medida del contrafáctico al 0,1 (kWh). M-J, la descomposición de la prima y el peso del vendedor marginal minúsculo. M-K, cuántas horas de compradores cortos despachan distinto el mérito por piso y el mérito por b_j, y cuánto excedente mueven.
- **M-H queda respondida en parte, y lo que respondió cambia D54 de sitio.** Su disparador era reabrir D54 si las horas de un solo comprador pasaban del 5 % de la energía. **Se cumple en ocho de los trece casos:** el 62,7 % de la transada en E5, el 60,1 % en P1, el 57,1 % en E4, el 53,6 % en E3, el 40,2 % en E2, el 23,1 % en N1, el 21,6 % en E1 y el 10,8 % en SINU (`task-2-fix2-report.md`, columna «% 1c»). La respuesta no es retirar D54 sino reinterpretarla: **D66 la conserva**, con el comprador único pagando el piso del vendedor marginal, que es el precio competitivo del lado vendedor y ya no el mínimo; σ_1 = ½ queda solo como fila del barrido. **La segunda mitad de M-H, los retiros con la liquidación nueva, pasa a ser la compuerta de cero retiros de D67.**
- **La renta inframarginal, con O3 de reserva.** El precio uniforme paga a los vendedores de piso bajo por encima de su alternativa, y eso cuesta ahorro a los compradores: −13,5 % en E1, −20 % en E2 y −5,7 % en I1 con la medición de D2, y de 404 a 122 (COP) en S2. Si al autor le pesa más que lo medido, la reserva es la regla de cobertura mínima (O3), que clava el precio en el piso marginal cuando σ_I·banda no alcanza; es una rama de una línea y no está medida (D70).
- **El vendedor marginal minúsculo sube el piso de toda la hora.** La tolerancia del nivel de la caminata es una guarda de redondeo, no un filtro: absorbe solo lo que queda por debajo de 1e-9 relativo, con una pérdida máxima medida de 1e-9 relativo y 2e-9 (kWh) en 40 000 horas. Con 10 (kWh) en el nivel bajo, un vendedor de 1e-7 (kWh) en un nivel alto lleva el piso de 616,46 a 693 (COP/kWh), es decir un 12 % de precio por una diezmillonésima de la energía; el corte está entre 1e-8 y 1e-7 (kWh). Es la subasta de precio uniforme haciendo lo que hace, y se mide en M-J.
- **En los modos `"costo"` y `"llenado"`, un vendedor despachado que el filtro de polvo descarta puede cobrar bajo su piso.** La re-revisión lo buscó a propósito, con 80 000 horas de un vendedor grande de piso bajo y de dos a cuatro de oferta entre 1e-12 y 1e-6 (kWh) con pisos separados: el peor déficit es de 4,9e-5 (COP) con `"costo"` y 4,7e-5 (COP) con `"llenado"` (`task-5a-rereview.md`). **Con `"piso"`, que es el modo de la matriz y el de producción, el peor déficit medido en esa misma búsqueda es de 1,2e-11 (COP)**, es decir redondeo: es imposible por construcción, porque pueden vender los de piso menor o igual que p\* y el ingreso medio nunca queda bajo p\*. No es una regresión, y no toca las cifras publicables: afecta solo a las comparaciones de M-K.
