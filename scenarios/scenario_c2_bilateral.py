...
def run_c2_bilateral(
    ...
    for k in range(T):
        ...
        # Autoconsumo: ahorro de cada prosumidor
        for n in prosumer_ids:
            autoconsumo = min(G[n, k], D[n, k])
            savings_gen[n] += autoconsumo * pi_gs
        ...
        if total_dem_cons > 0 and total_surplus > 0:
            ...
            for idx, i in enumerate(consumer_ids):
                # Ahorro del consumidor: habría pagado pi_gs, paga pi_ppa
                savings_cons[i] += ppa_delivered[idx] * (pi_gs - pi_ppa)
                # Déficit residual → red
                residual = max(0.0, deficits[i] - ppa_delivered[idx])
                grid_cost[i] += residual * pi_gs
    ...
    return {
        ...
        "params": {"pi_ppa": pi_ppa, "pi_gs": pi_gs, "pi_gb": pi_gb},
    }
